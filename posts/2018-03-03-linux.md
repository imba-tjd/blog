...
* /etc/ssh/sshd_config：PasswordAuthentication改成no就变成不许密码登录，只能用已有的RSA密钥验证
...
2. ssh-copy-id user@host 或 ssh user@host 'mkdir -p .ssh && cat \>\> .ssh/authorized_keys' \< \~/.ssh/id_rsa.pub
