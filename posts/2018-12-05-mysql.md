--- layout: post title: MySQL date: 2018-12-05 16:08:35.000000000 -06:00 type: post parent\_id: '0' published: true password: '' status: publish categories: - 数据库 tags: [] meta: \_oembed\_615f19451c11281683ec1f874693335d: "{{unknown}}" timeline\_notification: '1543997320' \_publicize\_job\_id: '24990396355' \_wpcom\_is\_markdown: '1' \_edit\_last: '119115352' \_oembed\_05b3d3f893dcc6a92887fe8755f8de0d: "{{unknown}}" \_oembed\_19daaf91d149d0c25197577adb8df042: "{{unknown}}" author: login: imbalancedweb email: imba.tjd@gmail.com display\_name: imba-tjd first\_name: '' last\_name: '' permalink: "/2018/12/05/mysql/" ---
...
* set foreign\_key\_checks=0：不检查外键约束
...
* 自增长：auto\_increment定义自增列；表的auto\_increment\_offset表示开始位置，auto\_increment\_increment表示每次自增的量；重置需要删除原列再添加，可以加个first放到第一位去
...
* repalce into：用法与insert类似，其实就是如果给的数据的**主键和unique约束**和已有的相同，就先delete再insert；最好写全否则没写的算default；执行SET col\_name = col\_name + 1相当于执行col\_name = DEFAULT(col\_name) + 1
...
* 建数据库时：default character set = utf8mb4 collate = utf8mb4\_unicode\_ci，每次连接时set names utf8mb4或者改数据库服务器参数
* Windows的命令行不乱码可能需要（未验证）：set character\_set\_results=gbk
