...
* 隐式转换
  ...
  * 与NULL运算：https://sqlite.org/nulls.html
...
* 数据类型
  * 存储类：INTEGER（物理上0-8B自适应，读取后为8B有符号）、REAL、TEXT、BLOB（它俩默认最大10进制1GB，有选项调到2进制2GB）、NULL
...
* Instance(Server) - DataBase(Catalog) - Schema - Table/View/...
  * DataBase是备份、事务的边界
  * Schema是逻辑组织，用于存放表、细粒度授权。不同Schema可以有同名表；Oracle每个用户默认自动创建一个Schema
  * MySQL没有Catalog，它的database就是Schema，可以用 库.表 二级查询所有库，可以直接join。其实其他数据库的三级也可以直接join
...
* 数据结构
  ...
  * B+树的数据都存放在叶子结点中，且之间又相连，刚好适合顺序遍历。B树数据在树里，不方便遍历
* 有大量重复值或者太宽不适合建立索引。区分度：COUNT(DISTINCT col1)/COUNT(*)在80%以上就可以
* 聚集索引/主键索引：一个表只能有一个，顺序与表的物理顺序相同，叶子储存数据值，不额外占用空间，不应频繁修改；自增对插入和密集读很友好，用UUID容易导致页分裂
* 非聚集索引(NONCLUSTERED)/二级索引/辅助索引
  * 相当于创建另一个表，顺序与原表物理储存顺序不同，叶子储存主键id
  * 多列索引/联合索引：一次性设置多个列。在一组AND中使用时要按顺序，不能有间隔、可以只用前面的、不等条件或排序要在最后
  * 用到的所有列都在索引中叫覆盖索引，不会查原表
  * 基于确定性函数的索引：SQLite PG MySQL8支持把表达式作为索引内容，如ON(a+b)、abs(x)，当查询时出现同样的表达式如WHERE a+b=xxx时就会使用
  * 有的主键允许用非聚集索引，但研究这个没意义
* 唯一索引(UNIQUE)：指数据是唯一的，不是只能创建一个；等值查询性能高
* 筛选索引/部分索引(MySQL除外)：创建非聚集索引时添加WHERE，维护开销更小，如用于大部分值为NULL的列创建非NULL的索引；如果查询时WHERE中有AND，创建索引一般要用OR
...
* 前缀索引(MySQL)：仅限字符串
* 位图索引(Oracle, PG)：某属性的相异基数(distinct cardinality)少又要过滤那一列时使用

### WHERE中索引失效

* 一般会尽量用索引，如果用到一半无法再继续了，会“回表”
* LIKE '%xxx'
* 列类型与操作数类型不同，包括字符集不同
...
* 对列进行加减乘除运算
* 复合索引没有按最左匹配原则
* 如复合索引a,b，过滤a>1 and b=2，a可以走索引二分，但a没有唯一确定；b无法用覆盖索引直接返回，但可以在索引中过滤，不用回表，称作“索引下推”(Index Condition Pushdown)。要尽量前面的判断相等，最后一项才能用不相等（或用来排序）
* 选择的列太多。避免SELECT*
* OR了不在索引里的列
* IN太多，少量的其实和=是一样的。看能不能换成BETWEEN
* 可能不会失效的某些情况
  * a,b索引，但a只有两种取值。可能改成a=v1 and b ... UNION a=v2 ...。SQLITE说必须经过ANALYZE才会做
  * a,b,c索引，ORDERBY a,c。会进行多次小型排序
...
* 锁
  ...
  * 按模式分：乐观锁（业务实现，加一个更新时间/Hash/Version列，读取的时候读入，写的时候检查是否一致）、悲观锁（SELECT FOR UPDATE）
* 死锁的解决办法
...
* SSL加密
  ...
  * 开启后仍要验证用户名和密码
  * 服务端第一次启动不能用ssl相关选项，否则不会自动生成证书
* show processlist 查看所有客户端连接
...
* 并发和线程安全
  * THREADSAFE=1（默认）下，官方说是“线程安全”的
  * =2时官方说只要没有两个线程同时使用同一个连接（及派生的对象如prepared_statement）就是安全的。每个线程新建连接用自己的没问题
  * =0一次只能有一个线程使用。官方CLI就是=0，可以多进程使用同一个数据库文件
...
* 查询schema元数据
  ...
  * EXPLAIN QUERY PLAN：以人类可读的方式显示查询计划
...
* journal_mode = DELETE/TRUNCATE/PERSIST/WAL/MEMORY。默认DELETE完成事务后就删除日志，TRUNCATE不删除日志文件只是清空，PERSIST在日志头部写零；这三种性能依次少量提升，这日志不在临时文件夹而是在数据库同级目录，相当于undolog；MEMORY不安全但也算能用，OFF无法ROLLBACK无意义
...
* mmap_size=xxx字节 能提高IO效率，但发生IO错误时不会由SQLite返回错误，而是产生信号，如果未捕获将导致崩溃；以及Win下无法VACUUM
* foreign_keys=1
...
* SQLITE_ENABLE_STAT4：有助于ANALYZE
