...
* 默认目录结构，apt装的是分散的。配置文件：/etc/nginx；网站根目录：/var/www/html；主程序：/usr/sbin/nginx(/sbin/nginx)；日志：/var/log/nginx；so模块：/usr/lib/nginx/modules(/usr/share/nginx/modules)；系统证书：/etc/ssl/certs和private；make install的装在`/usr/local/nginx`
* nginx -s stop(强行退出)、quit(正常退出)、reload(重载配置文件)、reopen(重新打开日志文件)；nginx -t只检测配置文件语法，-V显示版本信息和编译参数
...
* Linux下的相对路径是相对于/etc/nginx的路径，root除外
...
http {
    ...
    #limit_conn_zone $binary_remote_addr zone=addr:5m; # 定义连接域：https://nginx.org/en/docs/http/ngx_http_limit_conn_module.html
    #limit_conn addr 25; # 为指定域中的每个key（此处是IP的二进制）配置并发连接数，当连接请求数超出该值则拒绝；拒绝状态码默认503，可用limit_conn_status设置
    #limit_req_zone $binary_remote_addr zone=one:10m rate=10r/m; # 来不及处理的请求被延迟执行，直到它们的数量超过burst(漏桶的最大容量，默认为0)，即溢出(Nginx将拒绝该请求)
    ...
    server {
        ...
        listen [::]:80; # 地址可指定ip，可含*，一次只能指定一条；支持非https的http2
        ...
        error_page  404              /404.html; # 此指令产生的错误页是302跳转过去然后200加载成功的
        error_page  500 502 503 504  /50x.html;

        root /var/www/html; # 默认为/usr/share/nginx/html
        index index.html; # 可以有多个文件，适用于子目录
    }
}
```
...
http {
    ...
    ssl_dhparam certs/dhparam.pem; # 生成：openssl dhparam -out /etc/nginx/certs/dhparam.pem 2048
    ...
    ssl_stapling_verify on; # 好像有的教程没开下一条也能用
...
http.server { # 禁止ip和SNI不匹配的访问；也必须有证书指令否则会报emerg错误拒绝启动；需小心SNA泄露，可用自签edc证书
    ...
    return 444; # nginx的特殊状态码，代表直接断开连接
}
```
...
```conf
location = / {
    # =完全匹配，^~前半部分匹配；~大小写敏感正则匹配，~*大小写不敏感正则匹配，点号记得转义
}
location / { # 此处的斜杠指的是url中顶级域名之后的部分且不包括参数(?)；最长匹配原则，最后才匹配/
    try_files $uri $uri.html $uri/ =404; # 先尝试将请求视为文件，然后尝试补充后缀，然后是目录，然后回落到404；或者最后可以是@fallback，再在location @fallback里做别的操作；可以放外面
...
location /download/ {
    root /data; # 把另一个路径作为基路径，注意此时至少访问的真实路径为/data/download/
...
location ~ ^/blog/(\d+)/([\w-]+)$ {
    # /blog/20141202/article-name -> /blog/20141202-article-name.md
...
* `$scheme`：http或https，`$host`：原始主机名，含端口（若有），`$uri`：去掉参数、合并连续斜杠、解码了的路径，`$request_uri`含有参数的原始路径，`$args`：所有url参数不含一开始的问号，`$arg_abc`：url中?abc=xxx的值
...
* document_root：当前的root的值；realpath_root
* msec：当前unix时间戳
* remote_addr、remote_port：客户端的地址和端口；remote_user：http basic认证的用户名
...
* upstream中的keepalive指令不是和upstream的长连接，见<https://lanjingling.github.io/2016/06/11/nginx-https-keepalived-youhua/#二、nginx长连接——keepalive>
...
upstream backend { # 这里面的server指令并不是普通的server，因为context是upstream
    ...
    keepalive 8 # 缓存空闲连接的连接数，而不会限制新连接和总连接数
}
...
* https://nginx.rails365.net/chapters/1.html
