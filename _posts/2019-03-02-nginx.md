...
include /etc/nginx/modules-enabled/*.conf; # 该文件夹下有50-mod-stream.conf等文件，是/usr/share/nginx/modules-available下的的软链接，里面只有一行上一行
...
events {
    ...
    #accept_mutex on; #【默】设置网络连接序列化，防止惊群现象发生
...
```conf
http {
    include mime.types; # 文件扩展名与文件类型映射表；不知是否要写绝对路径/etc/nginx/mime.types
    ...
    sendfile on; # 开启高效文件传输模式；用来进行下载等IO重负载时应设为off
    tcp_nopush on; # 上一条开启时才有效，在一个数据包里发送所有头文件，而不一个接一个的发送
    #tcp_nodelay on; #【默】当需要及时发送数据时，就应为on
    keepalive_timeout 65; # http1.1长连接超时时间，默认75秒，与TCP的keepalive意义不同，见：http://blog.sina.com.cn/s/blog_e59371cc0102ux5w.html
    ...
    # include /etc/nginx/sites-enabled/*; # 里面的内容是sites-available的软链接
    server {
        listen 80; # 可指定ip，可用*，可一次指定多个；如果流量不从指定ip进入就不会hit；可加http2来支持非https使用
        #listen [::]:80 # 暂不知是否必须这样才能监听IPV6
        server_name  localhost; # 根据Host字段匹配，可正则匹配
...
* 匹配不到时使用listen中的default_server，不同端口能有不同的，V4和V6也可不同；如果还没有就是第一个listen到的
...
* 防止SNI不匹配时Subject Alternate Name泄露，比自签证书更高级，直接报无匹配的握手协议：https://github.com/QVQNetwork/ssl-patch/blob/master/nginx-config-nosni/README-zh_cn.md
...
http {
    ...
    #ssl_session_tickets off; # 默认是on，使用服务器集群且不希望部署共享票证密钥时可设为off
    ssl_buffer_size 4k; # 调小有利于减少首字节时间；默认16k，传大文件如图片则无需更改
    ssl_dhparam /etc/ssl/certs/dhparam.pem; # 生成（耗时约1分钟）：openssl dhparam -out /etc/ssl/certs/dhparam.pem 2048
...
http.server {
    ...
    # 也可以放在http中
    ssl_certificate      /etc/ssl/cert/fullchain.pem; # 或.crt
    ssl_certificate_key  /etc/ssl/private/privkey.pem; # 或.key
...
server { # 禁止ip和SNI不匹配的访问；也必须有证书指令否则会报emerg错误拒绝启动；需小心SNA泄露，可用自签证书
