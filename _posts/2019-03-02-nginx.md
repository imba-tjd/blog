...
http.server { # 禁止ip和SNI不匹配的访问；也必须有证书指令否则会报emerg错误拒绝启动；需小心SNA泄露，可用自签edc证书
    listen [::]:80 default server reuseport ipv6only=off deferred; # 每多监听一个端口都要加到这里，否则后几个参数不生效
...
proxy_cache_key  $host$uri$is_args$args; # 还可加$slice_range；默认是$scheme$proxy_host$request_uri;感觉没啥必要改
