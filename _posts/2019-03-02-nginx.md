...
http {
    ...
    ssl_early_data on;                # 开启0-RTT，但可能受重放攻击；一般要用proxy_set_header Early-Data $ssl_early_data;告诉上游
...
# 其它设置
proxy_buffering on;   #【默】开启缓冲功能，会尽快地把所有响应从上游都放到nginx中。若关闭或超出所有缓冲大小，收到响应后会将其同步传给客户端，此时proxy_buffer_size变为每次读取的长度。上游的响应头X-Accel-Buffering可动态控制此行为
...
proxy_max_temp_file_size 1024m; #【默】当proxy_buffers放不下上游的响应内容时，会将一部分保存到硬盘的临时文件中，默认在/var/cache/nginx下，这个值用来设置最大临时文件大小，设置为0相当于禁用此功能。与proxy_cache没有关系
...
proxy_hide_header、proxy_pass_header; # 控制服务器往上游传递的头，默认不会传递Date、Server等少数几个
...
proxy_ssl_name "api.cloudflare.com"; # 默认是上游的主机名
...
proxy_ssl_trusted_certificate # 如果上游用自签证书，反代端可通过此指令信任；一般的可用curl的证书 https://curl.haxx.se/docs/caextract.html
...
* 默认会遵循上游发送的Vary头，如果之后请求中该头指定的头的值发生了变化，会建立不同的缓存
* 默认会缓存上游发的gzip等压缩内容，如果客户端不支持此时可用gunzip模块；但应该无法把上游发的明文内容gzip给客户端

```conf
# 路径不能是proxy_temp；levels是目录深度，最高是2:2:2表示三级目录，每级有16*16个；inactive默认10m，超过时间没被访问就会被删除；keys_zone中1m可储存8千key
proxy_cache_path /var/cache/nginx/mycache levels=1:2 keys_zone=one:10m inactive=1d max_size=1g;
proxy_cache_key  $host$uri$is_args$args; # $slice_range；默认是$scheme$proxy_host$request_uri;感觉没啥必要改

proxy_cache       one; # 要用的zone；这条一般放在location里
