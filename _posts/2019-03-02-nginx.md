...
* 用途：Http静态资源服务器、反代负载均衡、邮件服务器、抗DDOS、TLS卸载
...
* nginx -t只检测配置文件位置和语法，-T还输出最终配置。-V输出版本信息和编译参数到stderr
* 在docker中运行要用-g "daemon off;"以前台的方式启动
...
#user www-data # worker的用户，默认nobody。master还是root的

#worker_processes auto;    # 默认1，够用作WebServer了。auto为虚拟cpu的数量。用作负载均衡时可调高。多个worker必须开reuseport
#worker_cpu_affinity auto; # 高并发下，设置cpu粘性，降低多核切换造成寄存器等现场重建损耗
#pid /var/run/nginx.pid;   # 记录每次启动的pid是什么，不是指定pid
...
http {
    ...
    default_type application/octet-stream; # 默认文件类型，nginx默认为text/plain，这个是apt的默认值
    #server_tokens off                     # 出错页面隐藏版本号，响应头不加Server，防止针对特定版本的攻击

    sendfile on;             # 传输文件不经过用户态，用gzip会自动禁用；用来进行下载等IO重负载时应设为off，或使用aio和directio
    tcp_nopush on;           # 上一条开启时才有效，在一个数据包里发送所有头文件，而不一个接一个的发送
    #tcp_nodelay on;         #【默】禁用Nagle算法（提出于1984年）
...
location = / # 精确匹配/
^~和无修饰符 # 前缀匹配
~大小写敏感正则匹配。~*大小写不敏感正则匹配，点号记得转义
不支持!~和!~*

多个location的查找顺序（只会选定一个）：
1. 在所有的=(精确匹配)中，使用最长匹配的。
2. 在所有^~中匹配最长的。
3. 按配置文件中的书写顺序，从上到下正则匹配，一旦有一条正则匹配成功就不会再继续，不管是否有更精确的。
4. 无修饰符最长匹配。内部无修饰符的处理和^~是一起进行的，只是当时把结果暂存，等正则失败再用

location @name 用于内部重定向，可以proxy_pass和error_page到它上面去


location / {
    # 先尝试将请求视为文件，然后尝试补充html后缀，然后是目录（去找index指令），然后404。对于未在path上区分静态和后端的场景，最后可用@fallback，内部转到location @fallback
    # 若不用此命令，相当于用了$uri
...
location ~ \.(gif|jpg|png)$ { # ~ ^/static/
    root /data/images/$1;
    #etag on;【默】
    expires 7d; # 可为max，会同时设定Expires和Cache-Control
    #tcp_nodelay off;
...
if ($request_method = POST ) { # 不等于用!=
    return 405; # Method not allowed。另一种限制HTTP方法的做法：limit_except语句，其中GET隐含HEAD
}
...
# 其它设置
proxy_buffering on;   #【默】开启缓冲功能，会尽快地把所有响应从上游都放到nginx中。若关闭或超出所有缓冲大小，收到响应后会将其同步传给客户端，此时proxy_buffer_size变为每次读取的长度。上游的响应头X-Accel-Buffering可动态控制此行为；设为off不影响proxy_buffer_size；对于基于长轮询(long-polling)的应用来说应关闭
proxy_buffer_size 4k; #【默】接收到的第一块响应的缓冲大小，用于存放响应头
...
* 负载策略路由指令
  * 默认是轮询，会把weight考虑进去
  * least_conn最小连接数、ip_hash、random。NGINX+还有个least_time
  * 自定义：hash $request_uri consistent
  * sticky指令提供另外三种方式保持指定的server，如根据cookie
...
#gzip_buffers 32 4k; #【默】缓存的数量和大小，第二个参数应和一个内存页大小相同。一个内存页(memory page)大小可用getconf PAGESIZE查看，Debian是4k
...
# 根据HTTP头的值决定发往哪个后端，不存在时用default
upstream backend { ... }
map $http_x_backend $backend { # 对应请求头 X-Backend
    default backend;
    "value1" backend1;
    "value2" backend2;
}
...
* map $a $b { v1 v2; default v3; } 当$a的值是v1时，将$b设为v2；如果无匹配则设为v3；均为任意字符串。可配合$http_xxx即读取客户端传入的HTTP头从而决定proxy_pass到哪里，v还可为upstream模块的值，因为map仅为字符串替换，proxy_pass支持upstream
...
* 数据准备阶段：数据从硬件到内核空间
* 数据拷贝阶段：数据从内核空间到用户空间
* 阻塞和非阻塞IO：数据准备阶段，应用程序是否阻塞等待操作系统将数据从硬件加载到内核空间
* 同步和异步IO：数据拷贝阶段，数据是否备好后直接通知应用程序使用，无需等待拷贝
* 同步非阻塞：如果无数据就直接返回错误。若处理的fd太多，多次系统调用也会大量消耗性能
* 多路复用：同时等待多个fd，当其中任意一个就绪时返回
  * select、poll、epoll：前两者只支持LT水平触发，当内核缓冲区里有数据没读完时，下一次还能触发；优点是可以逐步处理数据，缺点是可能导致频繁返回。ET边缘触发：如果没读完，下次wait时不会触发
* 零拷贝：mmap将内核空间映射到用户空间。sendfile：数据对用户空间完全不可见
* reactor模型：redis是单reactor，nginx是多reactor
...
* https://nginxproxymanager.com/ https://nginxui.com/zh_CN/ WebUI管理配置，本身有docker镜像
* https://web-check.xyz/
* Win版
  * 即使配置多个worker，也只有一个能工作
  * 不支持UDP，即不支持QUIC
  * 不支持IOCP，支持select和poll
* 域名用speedtest.xxx.yyy可能有提速效果
...
* traefik：必须配合docker，在使用者容器上加一些label从而声明路由，它自动发现
* HAProxy：传统负载均衡
* Envoy、APISIX：面向微服务和 API 网关的未来（限流、鉴权、监控；调整配置不按配置文件+CLI重载的模式）
* https://openlitespeed.org/ 有个WP缓存插件性能好。开源社区不活跃
