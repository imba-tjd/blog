...
http.server {
    ...
    add_header X-Content-Type-Options "nosniff" always; # 默认只有2xx和3xx会添加头，always指示任何状态码下都加
...
* rewrite的last表示完成rewrite，不做其他事；break表示中止rewrite，不再继续走流程，使用当前块进行处理；这俩重写不会影响客户端；redirect和permanent分别表示302和301
...
* 这种方式不支持https，不支持connect方式（curl -x失败）
...
# http_realip：当nginx自己处在反代后时，日志中记录的都是反代的IP。此指令可从X-Forwarded-For中从右往左找第一个不在set_real_ip_from范围内的IP作为$remote_addr，原来的变为$realip_remote_addr
...
* 错误代码400：请求方法不正确，如不带Host，请求方法乱写等（这个也可能是405）。会发到默认主机上所以没事
