...
http {
    ...
    server {
        ...
        error_page  404              /404.html; # 如果不加/，遇到404时会变成302跳转过去然后200加载成功，客户端无404；有/才直接真正404和指定文件；另一种语法是 error_page 404 =200 /empty.gif；还可以等于@fallback
...
* 防止SNI不匹配时证书泄露，能使浏览器报ERR_SSL_VERSION_OR_CIPHER_MISMATCH：https://github.com/QVQNetwork/ssl-patch/blob/master/nginx-config-nosni/README-zh_cn.md ；自签edc证书在浏览器中也能达到这个效果，只是也许未来会真正支持，而且这样其实是能获取到证书的，curl -k就能忽略证书错误：`openssl req -newkey ed25519 -keyout edc.key -sha512-256 -x509 -nodes -days 365 -out edc.crt -subj "/CN=localhost"`
...
proxy_bind $remote_addr transparent; # 标准用途是当有多个网卡时指定发送源；此行是文档中的一个例子，我以为是透明代理，但我测试使用后必定报502，好像要net.ipv4.ip_nonlocal_bind=1
...
# 然而我反代到自己的https报502，错误日志里发现是试图使用SSL3握手而失败。有可能是不支持TLS1.3
```
...
server.location /proxy {
    proxy_pass      http://backend; # 最后如果加反斜杠，效果是会把原来的/proxy部分去掉后再拼接过去
}
```
