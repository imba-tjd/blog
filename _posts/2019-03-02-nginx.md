...
* 默认目录结构，apt装的是分散的。配置文件：/etc/nginx；网站根目录：/var/www/html；主程序：/usr/sbin/nginx(/sbin/nginx)；日志：/var/log/nginx；so模块：/usr/lib/nginx/modules(/usr/share/nginx/modules)；系统证书：/etc/ssl/certs和private；make install会装在`--prefix`指定的地方，makefile中默认`/usr/local/nginx`但一般都会改成`/usr/share/nginx`
* nginx -s stop(强行退出)、quit(正常退出)、reload(重载配置文件)、reopen(重新打开日志文件)；nginx -t只检测配置文件语法，-V显示版本信息和编译参数；在docker中运行要用-g "daemon off;"以前台的方式启动
...
* include另一个配置文件的相对路径是相对于`--conf-path=/etc/nginx/nginx.conf`的路径，其余指令都相对于`--prefix=/usr/share/nginx`的路径；但编译时一般还会指定日志文件`--http-log-path=/var/log/nginx/access.log`和`--error-log-path=/var/log/nginx/error.log`，如果不重写就是用的这俩，重写且用了相对路径就还是相对于prefix的
...
http {
    ...
    server {
        # 一些listen的socket参数只需也只能设置一次，包括下面这俩和fastopen=3；而已知ipv6only必须重启nginx才能生效，reload无效，不知其它是否也是这样；另外ipv6only off会导致所有的V4地址以 ::ffff: 开头进行显示
        ...
        error_page 400 =200 /empty.gif; # 发生指定错误时改用另一种代码；但改写400好像非常不稳定，bug？
...
* 防止SNI不匹配时证书泄露，能使浏览器报ERR_SSL_VERSION_OR_CIPHER_MISMATCH：https://github.com/QVQNetwork/ssl-patch/blob/master/nginx-config-nosni/README-zh_cn.md ；自签edc证书在浏览器中也能达到这个效果，只是也许未来会真正支持，而且这样其实是能获取到证书的，curl -k就能忽略证书错误：`openssl req -newkey ed25519 -keyout localhost.key -sha512-256 -x509 -nodes -days 365 -out localhost.crt -subj "/CN=localhost"`
...
http {
    ...
    ssl_dhparam certs/dhparam.pem;    # 用于DHE和ECDHE，ECDH不用，TLS1.3好像是要用的；生成（约40分钟）：openssl dhparam -out path 4096
...
http.server {
    ...
    add_header Strict-Transport-Security "max-age=15768000; includeSubDomains; preload" always; # 半年（182天多）HSTS
...
http.server { # 禁止ip和SNI不匹配的访问；也必须有证书指令否则会报emerg错误拒绝启动；需小心SNA泄露，可用自签edc证书
    ...
    return 444; # nginx的特殊状态码，代表直接断开连接，会直接reset
}
```
...
* rewrite的last表示完成rewrite，不做其他事；break表示中止rewrite，不再继续走流程，使用当前块进行处理；redirect和permanent分别表示302和301
...
* `set $varname $1;`：声明变量（`$1`是正则匹配到的第一个分组，也可用普通字符串），另一种方式是用命名分组`(?<var>...)`。设置`$sent_http_name`可设置任意响应头
...
proxy_redirect off;             # 显形/隐形 URI，上游发生重定向时，Nginx是否同步更改uri
...
location / {
    # 如果proxy_pass没有uri（路径）部分（/也算有），则会传递$uri；单独用$request_uri和它的效果是一样的
    # 如果proxy_pass有uri，则会把location匹配到的部分去除，再把剩下的部分拼接上。称作“虚拟路径代理”
...
proxy_bind $remote_addr transparent; # 标准用途是当有多个网卡时指定发送源；此行是文档中的一个例子，我以为是透明代理，但我测试使用后必定报502，好像要net.ipv4.ip_nonlocal_bind=1；又看了文章说transparent是伪造请求源，则响应如果不经过Nginx就会出错，因为客户端会直接收到upstream的报文
...
upstream backend { # 这里面的server指令并不是普通的server，因为context是upstream
    # 负载策略指令：least_conn、ip_hash，或hash $request_uri consistent这种自定义策略；用了别的就不能用weight了
...
```conf
proxy_cache_path  cache/proxy_cache_path levels=1:2 keys_zone=pnc:300m inactive=7d max_size=10g;
...
* `location / { proxy_pass $scheme://$host$request_uri; }`，然后在hosts中设置域名为正代服务器IP即可
* 不要有server_name，其实是必须是default_server，因为真正要访问的网址就在Host字段中
* 必须有resolver，否则报502 Bad GateWay？使用ip访问正代也是502？
* 这种方式不支持https，不支持connect方式
* 支持connect方式的真正正代，不过需要自己编译：https://github.com/chobits/ngx_http_proxy_connect_module
...
limit_req_zone $binary_remote_addr zone=one:10m rate=10r/m; # 来不及处理的请求被延迟执行，直到数量超过burst(漏桶的最大容量，默认为0)溢出，拒绝该请求
...
location /download/ {
    ...
    limit_rate 128k; # 也可以使用$limit_rate
}
# 另有proxy_limit_rate
```
...
* 博客：http://blog.sina.com.cn/s/articlelist_1834459124_0_1.html https://blog.csdn.net/xxcupid/category_6411308.html
