...
* 操作符：d删除、y复制、c修改、v选中
* motion动作：w单词……
* 操作符 + motion：操作到哪里。一般的操作符必须指定到哪里
* 操作符 + 数字 + motion：重复n次动作
* x：单字符删除。3x删3个字符，不需要指定到哪里。或者x相当于dl。X相当于dh
* dd：特殊，删除一整行。2dd或d2d均可：删除2行。d3j删除下三行
...
* 删除当前行：dd。删除本段光标（及）右侧所有字符：d}或D。删除当前行后进入Insert模式：cc
...
* neovim
* https://helix-editor.com/ 具有Vim模式的终端编辑器，支持LSP
* https://github.com/lapce/lapce GUI
* https://zed.dev
* https://github.com/zyedidia/micro 终端编辑器
* 使用键盘操作浏览器页面：https://github.com/philc/vimium
* sublime
  * 虽然官网没写但能下到32位的：https://download.sublimetext.com/sublime_text_build_4200.zip
  * Tools - Install Package Control（最后一项）。不能删python33，4200说是加了一个disable_plugin_host_3.3设置，但实测没有
  * Preferences - Package Control - Install Package
  * 中文菜单：LocalizedMenu。Preferences - Languages。不要装ChineseLocalizations很久不维护了
  * LSP：添加包 LSP LSP-json。https://lsp.sublimetext.io/language_servers/ py：Pyrefly
* https://lnav.org 用于看大日志文件，避免爆内存。自动处理 .gz。指定文件夹可自动处理轮转
