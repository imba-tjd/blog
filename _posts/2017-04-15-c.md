...
* 非ISO标准修饰符：`"%2$d %1$d"`可以不按参数顺序输出，`"%m"`等价于`"%s",stderr(errno)`，会输出Success
* 标准输入默认是行缓冲，而标准错误是无缓冲
...
* sizeof('a')==4，~~也就是会提升为int~~单个字符字面量就是int类型，只不过如果超出char的范围就是实现定义；sizeof(char)==1
* 正确地声明函数原型，传参时不会类型提升
