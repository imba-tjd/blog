...
* &：无论前面的命令是否成功都会执行后面的命令，且视为一条语句，如用在if中
* 短文件名：六个字母加~1
* Ctrl+C：若在运行bat时使用，会发给当前正在执行的外部程序，同时发给运行那个bat的cmd，会问`Terminate batch job (Y/N)?`，如果此时再按一次Ctrl+C，相当于输N，会继续执行bat，只不过终止了那一个外部程序
* 输出：echo "123"会输出`"123"`，真的反人类。`echo.`无空格输出一空行，`echo.xxx`无论是否是echo off的，都会输出xxx
* 命令行参数解析：https://zhuanlan.zhihu.com/p/551435073 https://www.zhihu.com/question/1974854700256944776/answer/1974996354171418603

## 变量

* 声明：set a=10。等号两边都一定不能有空格和引号，否则会算作变量名和值的一部分。另一种选择是set "a=12&34"，允许有特殊符号，但值好像又不方便有引号字面量
* 使用：cmd1 %a%。如果值中有空格用cmd1 "%a%"，值中有引号难以处理
  * 显示变量：无参set显示所有环境变量和局部变量。set x：显示以x开头的变量
* 删除：set a=  无法创建值为空字符串的变量
...
* SETLOCAL ENABLEDELAYEDEXPANSION 变量延迟扩展：使用感叹号包裹变量。默认一条语句（包括用&连接和括号内换行）中的百分号的变量在这条语句之前就评估完毕了，如果在语句内又改变了变量，则用百分号时不会生效。另一种解决方式是用call并使用双百分号
  * 另一种情况：在CMD里设置了某变量，运行BAT脚本，则在运行脚本内任何代码前，该变量的值已经固定（替换完毕）
* 在环境变量里递归引用变量时，对顺序有要求，而顺序取决于字符序。如X=%X_1%，X_1=真实值，如果PATH=%X%，则无法解析，而把X改为A则可以
...
* FOR /R 可选起始目录 %%i IN (*.txt)：递归显示匹配了模式的文件的完整路径，set用(.)表示所有的。for /D 匹配文件夹
* 这些%%i支持参数扩展
...
* for /F in ("路径\*.txt") 也能搜索文件，但通配符仅限最后一节，无法在中间的路径里用
* 如果无权限访问目标文件夹，可能无报错
...
* call
  * 调用目标会等待退出，返回原位置继续；MS文档里说“without stopping the parent batch”，但只解读为不会“STOP”，而是会“PAUSE”
  * call bat2 params：调用第二个bat时永远用call，否则对于label有bug（见ss64）
  * call :label params，类似于函数，会修改%n。返回时必须exit /b 返回值或goto :EOF，不能单纯exit；调用者可配合||goto :错误处理。只在批处理中有用
  * MS文档说不能和管道、重定向符号一起使用
  * call exe感觉没什么意义，与直接运行一样。本来call bat感觉上也如此
  * 无论是否用call，调用bat都会使用当前ctx，即新建或修改变量执行完后对调用者也有效，感觉类似于bash的source
* start
  * 第一个参数如果用了引号，会被认为是title
  * 支持根据关联打开文件
  * 默认会新开一个控制台窗口：如果目标是内置命令或bat，相当于用cmd /k运行它；如果是外置命令，相当于Win+R运行它，运行完后窗口会关闭
    * 当在批处理中执行GUI程序时，默认会等待结束，而CMD交互式运行时却不会。用此命令可解决
...
* md或mkdir：自动创建中间文件夹
* mklink link target：需要管理员权限（Win10开DeveloperMode后不需要了）；默认为文件符号链接，/D用于目录，两者的target都可以为相对路径，且是相对于link的而非cwd的；target不能用斜杠，必须用反斜杠；/J只能用于目录且只能是绝对路径但无需管理员权限
...
* setx /m PATH "%PATH%;C:\tools"：永久修改系统级别环境变量。若修改用户级别，会将系统级别的重复一份；还会最多1024字符截断。注册表（用户）：HKEY_LOCAL_MACHINE\System\CurrentControlSet\Control\Session Manager\Environment
* shift：依次把%0~9往前移
...
* robocopy /R:0 /E src_dir dst_dir [file_pattern]
  * 默认重试一百万次，间隔30秒。/E复制子文件夹
  * /MT:n 使用多线程。/J无缓冲当复制大文件时用
  * src_dir必须为文件夹，如果想复制文件，在后面再写file_pattern的过滤器。/MOV完成后删除那些文件，/MOVE完成后删除src_dir。/L即dry-run
  * 支持跳过已存在的文件。可以看到进度。支持输出日志。支持“镜像”模式删掉dst中不存在于src的
* rstrui：系统还原对话框
* sc：配置服务。见Windows笔记
* schtasks：管理计划任务，见Windows笔记
* subst H: C:\Share：将目录或网络路径挂载为虚拟驱动器，也支持取消映射
* takeown /F 文件目录名支持星号 /R递归：修改所有者为当前用户
* tasklist：/FI过滤，支持 PID 进程名 服务名。/FO支持CSV输出。/M列出进程加载了的dll，/SVC列出进程对应的服务名。/m xxx.dll 列出加载了此模块的进程，注意无需完整路径，而是基本名，如同无参/M列出的那样
* tlist：显示线程信息
* wbadmin：备份工具，但无法通过它新建备份计划，即无法创建，只能执行和停止计划、删除备份、进行恢复
* whoami：无参使用显示`主机名\用户名`，用/ALL可显示所有用户及SID、组信息、特权信息
* wmic：已弃用，在win11上已被删除。使用Get-CimInstance代替，用Get-CimClass获得可用的Key
...
* eventvwr.msc 事件查看器
* hdwwiz.cpl 或 devmgmt.msc 设备管理器
* main.cpl 鼠标
...
* 一般UAC是双击的。不考虑传递命令行参数，可能有引号、空格，太复杂
* %~s0 表示短文件名，避免空格问题
* 如果卸载了ie则不生效

```bat
:: 获取管理员权限后把参数设置为::，把该行作为注释避免重复执行
