...
* md或mkdir：自动创建中间文件夹
* mklink link target：需要管理员权限；默认为文件符号链接，/D用于目录，两者的target都可以为相对路径；/J只能用于目录且只能是绝对路径但无需管理员权限
...
* wmic：已弃用
  ...
  * 编程使用时，name后的双引号不可省，推荐改成name like "xxx"，否则有奇怪的Invalid Query问题
* PathPing
...
* netstat -anop TCP；a显示listen的，n避免持续输出，o显示对应的PID，p指定协议。e查看统计数据
* route print
