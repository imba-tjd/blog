...
* goto :EOF：跳到最后一行，即退出
* call xxx.bat params
* call :label params，可相当于函数，会修改%n，退出时必须exit /b 返回值或goto :EOF
* start
  * 第一个参数如果用了引号，会被认为是title
  * 支持根据关联打开文件
  * 如果目标是内置命令或bat，相当于用cmd /k运行它；如果是外置命令，相当于Win+R运行它
...
* [Make SFX](https://revocue.cz/en/make-sfx/index.php)
* [EnigmaVB](https://enigmaprotector.com/cn/aboutvb.html)
