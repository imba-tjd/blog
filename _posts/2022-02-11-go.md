...
* REPL环境：https://github.com/x-motemen/gore https://github.com/cosmos72/gomacro
...
* `var i, j int = 1, 2`
  ...
  * 连续多个参数类型相同，可只写最后一个
  * 如果初始化了，可以省略类型
  * 函数内的简化声明：i := 1。对于x,y:=...只要有一个本作用域内不存在时就会shadow它们全部，因此a,err:=...;b,err:=...不会报错，但有_,err就会报错
...
* 指针：*T、new(T)、nil。无法加减运算，T可以是数组，能返回指向本地变量的指针
* 结构体
...
* 约束
  ...
  * 自定义类型约束：type Number interface{int|float32}
...
* ioutil：不应使用
* bytes.Buffer：相当于Stream或StringBuilder，零值可用，b.Write()/WriteString()往它里面写，b.WriteTo()写入另一个Writer，b.String()转换成字符串
...
buf := make([]byte, 2048)
for {
    n, err = r.Read(buf) // 不是返回[]byte，而是写入传入的[]byte参数
...
* log.Fatal(err)：相当于log.Print()再os.Exit(1)；log.Panic()类似
...
* 文件在同一位置以_test.go结尾，package名可以以_test结尾，则只测试公开的API
