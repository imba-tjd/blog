...
* REPL Shell：https://github.com/x-motemen/gore https://github.com/cosmos72/gomacro
...
* Build Tag：在文件第一行注释写// +build 标签名，构建时加-tags 标签名
...
* 类型定义：type status bool，两者是不同的类型，可以在status上定义方法；类型别名：type Text = string，两者是同一个类型，编译后会去掉
  ...
  * 新类型与原类型之间无法隐式转换，两者的方法也不共享（不同的方法集），（接口对象对他俩的）类型断言也不兼容
