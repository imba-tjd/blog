...
* `var i, j int = 1, 2`
  ...
  * 连续多个参数类型相同，可只写最后一个
  * 如果初始化了，可以省略类型
  * 函数内的简化声明：i := 1。对于x,y:=...只要有一个本作用域内不存在时就会shadow它们全部，因此a,err:=...;b,err:=...不会报错，但_,err就会报错
...
* 数组
  ...
  * 部分值语义：传参时会整个复制；可以直接==判断相等，如果长度不等会在编译期报错
  * 解包：在对象后加...，此处的点是真的三个点
* 切片
...
* error接口
  ...
  * 用多返回值特性作为最后一个返回值
  * 不发生错误时返回nil。因为接口的特性，不能一开始声明error接口变量，中间给它赋值，最后统一return
...
* 错误包装
  ...
  * errors.Is(err, ErrXX)：类似于判断哨兵错误，但可以自动递归处理包装的
...
* ioutil：不应使用
* bytes.Buffer
  * 相当于Stream或StringBuilder，零值可用。或用bytes.NewBuffer([]byte)
  * b.Write()/WriteString()/ReadFrom(r)往它里面写
...
* 将uintptr转换回Pointer一般来说是不正确的，因为uintptr的数字是不变化的，它代表的地址可能已经被GC移动了
...
* 等待
  ...
  * time.After(time.Second)：返回ch，可用在select中作为超时
...
//export echo  // 在.c中用 #include "_cgo_export.h" 能使用
func echo(n int) int {
  return n;
}
```
...
* https://golang.google.cn/doc/faq 看到Why is my nil error value not equal to nil?，对照 https://go-zh.org/doc/faq https://www.jianshu.com/p/4b6b8966b314 https://go.p2hp.com/doc/
...
* https://www.kancloud.cn/mutouzhang/go/596804
