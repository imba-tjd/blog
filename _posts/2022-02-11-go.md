...
* `var i, j int = 1, 2`
  ...
  * 连续多个参数类型相同，可只写最后一个
  * 如果初始化了，可以省略类型
  * 函数内的简化声明：i := 1。对于x,y:=...只要有一个本作用域内不存在时就会shadow它们全部，因此a,err:=...;b,err:=...不会报错，但_,err就会报错。又但是b,err其实只是给err赋值
...
* go mod vendor
* import的是模块名，可加/目录名，使用时是包名.函数名，与模块名和文件名无关；同一个包的不同文件具有相同作用域，即使是私有变量也不行
...
* 使用Fork的模块：`replace 老模块名无需版本 => 新模块名 latest`。这样无需更改原先import的路径
* 使用未发布的本地模块：先普通require目标模块名，版本号随便写但必须有，再加一条replace指向本地路径无需版本号；1.18后可用workspace
* 自动生成文档
...
* Time
  ...
  * 模板：如`2006-01-02 15:04:05`，关键是这个模板的具体值是固定的，2006等价于java的yyyy，15等价于HH，03等价于hh。预定义了一些格式模板常量
...
* net/netip
  * 1.18新增，不可变
  * netip.ParseAddr("1.1.1.1") netip.AddrFrom4([4]byte(1,1,1,1))
  * addr.Prefix(24) p.Contans(addr)
...
* 编译：go build -buildmode=c-shared -o echo.dll echo.go
* 在C中也可以直接使用Go导出的函数
