...
* 结构体
  ...
  * 组合/嵌套另一个结构体
    ...
    * b可以直接使用A的字段和方法（重名时才用b.A.x，且不使用b.x就不会报错），A的方法也能算进B实现的接口，这称作提升
...
* 映射
  ...
  * m["a"] = 1 可直接对不存在的key赋值，可以++。但若元素是struct，则不能直接更新其字段
...
* ioutil：不应使用
* bytes.Buffer：相当于Stream或StringBuilder，零值可用，b.Write()/WriteString()/ReadFrom(r)往它里面写，b.WriteTo()写入另一个Writer，b.String()转换成字符串
...
* sync包
  ...
  * WaitGroup：类似于信号量，多个goroutine同时写入某ch时用到。wg.Add(3)增加计数一般等于要使用的goroutine数，go func(){defer wg.Done()}()减少计数，主函数wg.Wait()，之后再关闭ch可避免多次关闭
  * Once：先在外部声明它，函数内闭包调用的once.Do(func)可保证func只会执行一次
  * Mutex：不可重入锁，当需要处理值但不消费时用到，m.Lock()，defer Unlock()。RWMutex读写锁，Lock()为加写锁，RLock()加读锁
  * Map：问题多，不要用
...
* Time
  ...
  * 格式化为字符串：t.Format("模板格式如2000/01/01")
