...
* 可以直接输出
* 大小固定
* 值语义：传参时会整个复制。可以直接==判断相等，如果长度不等会在编译期报错
* 解包：在对象后加...，此处的点是真的三个点
...
* 操作
  ...
  * 1.21 slices包：插入、删除、Index、Contains、排序、稳定排序、替换
...
* 常见错误：for的v在循环时地址不变，循环append它的&或[:]会添加重复的对象，或调用接收者为*T的方法时也要注意。预计1.22会更改
...
* 在func和函数名之间加“接收者”，同名函数可以有不同类型的接收者。接收者的类型是用type定义的，如函数无法作为接收者
