...
* ioutil：不应使用
* bytes.Buffer：相当于byte流，零值可用，b.Write()，读取用b.String()
...
* Context
  * 解决goroutine之间退出通知、元数据传递的功能
  * 上层任务取消后，所有的下层任务都会被取消。中间某一层的任务取消后，只会将当前任务的下层任务取消，而不会影响上层的任务以及同级任务
  * ctx, cancel := context.WithTimeout(context.Background(), 5 * time.Second); defer cancel() 还有WithCancel WithDeadline WithValue
  * 父节点用defer cancel()取消任务，子节点用select{case<-cxt.Done()}判断是否取消了
...
* net/http
  ...
  * 手动使用
    ...
    * req, err := http.NewRequestWithContext(context.Background(), http.MethodGet, "url", body(Reader)/nil)
    * req.Header.Add(k,v)
    * client.Do(req)
