...
* 操作
  ...
  * 1.21slices包：插入、删除、Index、Contains、排序、稳定排序、替换
...
* 常见错误：for的v在循环时地址不变，如果循环append它的&或[:]，添加的是重复的对象，或调用接收者为*T的方法时也要注意。预计1.22会更改
...
* m["a"] = 1 可直接对不存在的key赋值，可以++。但其它情况下要视为右值，如不能直接更新struct的字段，不能取地址
...
* 指针接收者*T：t.F()时应把t看作OOP的引用类型，使用字段也用点
