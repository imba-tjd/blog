...
* 运算符
  ...
  * 位运算符
    ...
    * &^位清除：将第二个操作数的1位置在第一个操作数中清零，其实就是&0位置。但没有=&^运算符
    * 如果跟字面量，要写 op类型(x)
  * 运算符优先级：暂时不学
...
* 指针：*T、new(T)、nil。无法加减运算。T可以是数组。能返回指向局部变量的指针，会自动放到堆里。1.26：允许new(值)，而不必先创建变量再&
...
* 组合/嵌套另一个结构体
  ...
  * 没有dynamic dispatch，被嵌套的结构体不知道外层的事：若A存在F1和F2，F1里会调用F2，B存在F2，则b.F1()调用的是A的F2
...
* 操作
  ...
  * 1.21 slices包：插入、范围和条件删除、Index、Contains、排序、稳定排序、替换、翻转。1.22：合并多个
...
* 不支持默认参数，不支持重载。一般将可选参数用一个*Config表示
  * Option模式：用于设定可选参数。先创建一个函数类型Option，参数是要设定的对象。再创建一些函数，参数为要设定的选项，返回Option，闭包设定它。最后在“构造函数”里读取最后一个可变参数依次传要设定的对象进去
...
* Mutex：不可重入锁，当需要处理值但不消费时用到，或处理共享资源，m.Lock()，defer Unlock()。RWMutex读写锁，Lock()加写锁，RLock()加读锁，写锁优先级更高。不能值拷贝
* Map：适合读多写少，无法实际删除。Store(k,v) Load(k) LoadOrStore() Range(func(k,v any){处理内容}) 普通的map如果全都只读也是安全的
...
* 等待
  ...
  * time.After(time.Second)：返回ch，可用在for-select中作为超时：当在select中创建时就是单次超时；当在外面创建，select中取出时就是总超时
    * 在1.22及之前，若设定的超时时间比成功的路径长很多，会导致内存泄漏，因为每次for都会创建定时器，但执行成功路径却不会停止定时，只会在到期后才销毁。1.23解决了
...
* Timer定时器：“继承”了Time。Ticker也是
* Duration
  * time.ParseDuration("1.5h10m1ms")
  * time.Hour + 10 * time.Second
  * elapsed := time.Since(开始的时间).Seconds() 代替计时时两个Now()后t2-t1，Seconds()结果是小数
* Time对象
  * 有两种作用。内部有Monotonic Clock，默认在做加减运算或比较时会使用，防止时钟倒退；序列化或按日期创建时自动不会用，手动去除用t.Round(0)
  * 也起日期的作用（Wall Clock），时间精度为纳秒
  * time.Now() 本地时间，带有时区。进行时刻相等比较：Equal
  * time.Date(年,月,日,时,分,秒,纳秒,time.UTC) 其它地区：time.LoadLocation("America/New_York" 或 "Local")
  * 提取日期和时间的部分：t.Date()、t.Clock()
  * 修改时区（时刻不变）：t.Local()、t.UTC()
  * 格式化为字符串：t.Format("模板")
  * 从字符串转换为时间：time.Parse("模板", "时间字符串") 匹配是严格的。当模板无时区时默认UTC，编程指定：ParseInLocation
  * 模板：如`2006-01-02 15:04:05`，关键是这个模板的具体值是固定的，2006等价于java的yyyy，15等价于HH，03等价于hh
    * 预定义了一些格式模板常量，最常见的是time.RFC3339。1.20提供DateTime DateOnly TimeOnly表示最常用的那种形式
      * 此处RFC3339可与ISO8601标准互换，对比：https://ijmacd.github.io/rfc3339-iso8601/ 另外RFC9557是3339的升级
  * Unix时间戳：time.Now().Unix() UnixMilli()
