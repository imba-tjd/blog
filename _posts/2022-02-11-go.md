...
* REPL Shell：https://github.com/x-motemen/gore https://github.com/cosmos72/gomacro 后者更好
...
* VSC
  ...
  * 调试控制台即时调用函数：`call err.Error()`，不加call会报错
...
* `var i, j int = 1, 2`
  ...
  * 连续多个参数类型相同，可只写最后一个
  * 如果初始化了，可以省略类型
  * 函数内的简化声明：i := 1。对于x,y:=...只要有一个本作用域内不存在时就会shadow它们全部，因此a,err:=...;b,err:=...不会报错，但_,err就会报错。又但是b,err其实只是给err赋值，x:=1;if true{x:=2}才会新建变量
...
* 控制台输出：fmt.Print(对象)、Println(对象)、Printf("模式", 对象)
...
* strings
  ...
  * Trim、TrimSpace、TrimPrefix 不是按字符移除
...
switch i := 1; i+1 { // 无条件的switch和switch true一样，case可用条件表达式，代替一长串ifelse
  case 1+1,3: ... // 自带break也支持手动用，顺序执行指明fallthrough；case后的可以是表达式；并列的case用逗号
}
```
...
* 隐式实现
  ...
  * 可看作多态、鸭子类型
  * 与函数指针相比，后者难以组合
  * 保证结构体实现了某接口的方式：`var _ S = (*I)(nil)`，如果没有实现会编译报错
...
* error接口
  ...
  * 用多返回值特性作为最后一个返回值
  * 不发生错误时返回nil。不要声明一个具体错误类型变量直接返回它，或者把具体错误类型变量传进另一个函数，因为接口的特性，它在本函数中可以为nil，但返回后就一定不为nil了
...
* 错误包装
  ...
  * var e ErrXX; if errors.As(err, &e)：类似于类型断言
  * 类型断言：switch err.(type){case *XXError: ...; default: ...}
...
* 使用未发布的本地模块：先普通require目标模块名，版本号随便写但必须有，再用一条replace指向本地路径；1.18后可用workspace无需replace：在项目的上一层运行go work init proj1 proj2，会生成go.work，无需提交进仓库
* 自动生成文档
...
buf := make([]byte, 2048)
for {
    n, err = r.Read(buf) // 不是返回[]byte，而是写入传入的[]byte参数；另有io.ReadFull(r,buf)当没读满
...
* channel
  ...
  * close(ch)关闭。只有发送方能关闭，一般情况下无需关闭。从已关闭的ch中取值永远能成功，如果缓冲用完了会返回零值，使用ok判断是否是真正的值
  * 读写nil的ch会永远阻塞，在select中可通过把某一ch设为nil来“关闭分支”
  * 生成器模式：一个返回ch的函数，里面声明一个ch、go func(){写入ch}()、返回ch
  * 利用ch通信/同步：比如一个函数创建并返回{msg,ch}，然后在另一个地方<-ch等待。调用者取出msg处理后ch<-通知创建者
  * Replica时取第一个：创建ch，for循环运行多个goroutine写入ch，返回<-ch，则取的是最快返回的那一个
* select
  ...
  * Done Channel模式：for{select{case<-done:return; case v:=<-ch:...}}，当done关闭时结束，done一般为<-chan struct{}
* sync包
  * 零值可用，普通地var声明即可创建；不要值传递进其他函数，要么闭包要么指针
  * WaitGroup：主线程wg.Add(3)增加计数，一般等于要使用的goroutine数。go func(){defer wg.Done(); 调用真正的函数()}()减少计数。主线程wg.Wait()等待计数归零，之后close(ch)可避免多次关闭。用于等待多个任务都完成，与信号量的剩余资源思想完全不同
  * Once：先在外部声明它，函数内闭包调用的once.Do(func)可保证func只会执行一次
  * Mutex：不可重入锁，当需要处理值但不消费时用到，m.Lock()，defer Unlock()。RWMutex读写锁，Lock()加写锁，RLock()加读锁
  * Map：适合读多写少。Store(k,v) Load(k) LoadOrStore() Range(func(k,v any){处理内容})
...
* Context
  ...
  * 取消
    ...
    * 父任务取消后，所有的子任务都会被取消。中间某一层的任务取消后，只会将当前任务的子任务取消，而不会影响上层的任务以及同级任务
    * 子层用select{case<-cxt.Done()}判断是否取消了，一般返回ctx.Err()
...
* Type
  * t:=reflect.TypeOf(x) 其中x是变量/实例。不创建变量就获得类型：如`reflect.TypeOf([]string(nil))`
  * t.Name() 如"int"等。切片和指针没有名字，返回空字符串
  * t.Elem() 如果x是指针 切片 数组 映射 Channel等，此函数相当于解引用
* Kind：例如定义了struct Foo{}，它的Type是"Foo"，Kind是reflect.Struct；指针的Kind是reflect.Ptr
* Value
  * i:=0; v:=reflect.ValueOf(&x); v.Elem().SetInt(1) 此处对指针的使用和普通的函数是一致的，不这样不能改变调用者的int。如果只读，可以不用指针
  * 创建值：reflect.New/MakeMap/MakeSlice/Append等，与new make等一样
  * 转换为具体类型：v.Int()/String()/Interface().(xxx)
...
s := "hello"
sHdr := (*reflect.StringHeader)(unsafe.Pointer(&s)) // Go的string是bytes指针和长度。切片为类似的SliceHeader
for i := 0; i < sHdr.Len; i++ {
    bp := *(*byte)(unsafe.Pointer(sHdr.Data + uintptr(i))) // 遍历切片时后者还要乘以unsafe.Sizeof(s[0])，因为int的大小不确定
}
...
* 一般工作区根目录的main.go用package main再定义func main()作为入口点，二进制名就是模块名。多二进制：cmd目录下多个目录再main.go，build时依次指定那些目录或用`cmd/...`表示通配，生成在cwd下
* 命令行参数用os.Args获得，0号是程序自己
* flag库
  * b := flag.Bool("参数名", 默认值, 帮助信息)，用*b使用
  * flag.Parse() 把命令行参数解析进之前指定的变量里
  * flag.Args() NArgs() 非命名参数及其个数，必须在Parse()后使用
  * if *s == "" {flag.Usage(); return}
  * 命名参数用单个横杠指定，自带-h
...
* 等待
  ...
  * time.After(time.Second)：返回ch，可用在select中作为超时：当在select中创建时就是单次超时；当在外面创建，select中取出时就是总超时
...
* 反序列化：var m Message; err := json.Unmarshal(byte[], &m)。如果不知道结构可以设为map[string]any
* 自定义JSON键名：使用struct的Tag，在字段后加`反引号json:"自定义名"反引号`，用"-"表示忽略。如果不指定，则序列化时大写（因为只会序列化公开的），反序列化时大小写不敏感
...
* Fatal(err)：相当于log.Print()再os.Exit(1)；log.Panic()类似
* New(writer, prefix, flag)：新建Logger
...
* net/netip
  ...
  * AddrPort：没啥用，基本上就是地址加端口的结构体
* 第三方HTTP客户端：https://github.com/imroc/req
...
* golang.org/x/sys/windows：封装了一些WinAPI
...
* 不能调用函数指针
* 并不快
* 目前不支持UCRT：https://github.com/golang/go/issues/51007
...
* https://golang.google.cn/doc/faq 看到Why does Go not have variant types? https://go.p2hp.com/doc/
* https://golang.google.cn/doc/effective_go 看到 多值返回
...
* https://zhuanlan.zhihu.com/p/352589023 https://zhuanlan.zhihu.com/p/450822514 https://zhuanlan.zhihu.com/p/64823525 https://zhuanlan.zhihu.com/p/276052046
* sort.Search
...
* https://github.com/langwan/chihuo/tree/main/golang
...
* https://github.com/golang/go/wiki/Articles https://github.com/golang/go/wiki/LearnConcurrency https://github.com/golang/go/wiki/LearnErrorHandling 以及Additional Go Programming Wikis
...
* https://github.com/go-gorm/gorm https://gitea.com/xorm/xorm https://github.com/ent/ent
