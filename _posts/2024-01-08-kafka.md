...
* 4.2：允许多个消费者读取同一个分区，类似于传统意义上的队列。之前如果某个分区拥挤而另一个分区空闲，没有办法，因为一个分区最多一个消费者

### 消息传递语义

* at-most-once：无需副本、生产者无需重试(或acks=0)、消费者先commitSync再处理消息（不能是Async，否则可能重复消费）
* at-least-once：消息不会丢失，但可能被重复发送
* 生产者：默认retries>0且acks=all时会自动开启幂等性enable.idempotence（但librdkafka要手动开启），可以不产生重复记录，且重发的消息的最终顺序保持了原本的顺序
* 消费者和自动提交
...
```
batch.size默认16KB，当消息满足此大小后会立即发出，否则再等linger.ms（默认0ms）后立即发出。根据测试，对于90个生产者100个分区90MB/s的写入速度，设置linger.ms=5的人工延迟反而能降低端到端延迟，从200ms到100ms
...
fetch.min.bytes（默认1）、fetch.max.wait.ms(默认500；librdkafka是fetch.wait.max.ms) 二者满足任意一个就会下一步处理。调大后一次性会多fetch一些，增大吞吐量，当重启后会重传更多数据，但对offset无影响
...
事务：spring.kafka.producer.transaction-id-prefix=kafka_tx. 设置后所有行为都必须在Spring事务中进行。一个事务中可以发送多次，如果一个失败了会回滚。会先提交数据库事务
对于普通的生产者自动幂等性，如果MQ回复的ack丢失，生产者又挂了，则会重新分配PID导致自动幂等性失效。若开启事务每个生产者配置不同的事务id，则会从broker中取回PID

spring.kafka.listener.ack-mode=默认BATCH。当spring.kafka.listener.async-acks=true时要改前者的值
```
...
* UI：https://www.kafkamagic.com/ https://www.conduktor.io/ https://github.com/provectus/kafka-ui https://github.com/tchiotludo/akhq https://github.com/didi/KnowStreaming kafkadrop
