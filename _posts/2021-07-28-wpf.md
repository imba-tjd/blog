...
class RelayCommand<T>: ICommand {
    ...
    // 两个public的方法，对应调用那两个成员，参数为T；当can为null时属于true
}
```
...
* 不依赖控件渲染获取DPI Ratio：(int)typeof(SystemParameters).GetProperty("DpiX", BindingFlags.NonPublic | BindingFlags.Static).GetValue(null) / 96
* 一些winuser.h中的函数：GetSystemMetricsForDpi AdjustWindowRectExForDpi SystemParametersInfoForDpi GetDpiForWindow
* 不学HwndHost和WindowsFormsHost

```xml
<dpiAwareness xmlns="http://schemas.microsoft.com/SMI/2016/WindowsSettings">PerMonitorV2</dpiAwareness> # 1703+，但好像默认只支持PM1
