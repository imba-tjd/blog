...
* 实际上是设置对象的依赖属性
* 优先级：动画 > 绑定 > 控件inline本地值 > Style Trigger > Template Trigger > Style Setter > 继承值（嵌套元素，具体某一DP是否可继承取决于MetaData）> 默认值。当某一条件不满足时会使用优先级更低的，而不是像event handler“永久”修改属性值
...
```xml
<Window.Resources>
<Style x:Key="MyStyle" TargetType="{x:Type Button}"> # 也可以直接写"Button"
...
<Button Style="{StaticResource MyStyle}"> # 其实也可以写<Button.Style>，仍要写TargetType
```
...
* 感觉是创建了一个新的控件，只是允许把老控件的属性绑定到新控件上
...
* 传播途径分为“隧道”和冒泡，前者一般以Pre开头，先执行前者。普通的.NET事件不会传播，等于RoutingStrategy.Direct
...
MouseUpEvent = Mouse.MouseUpEvent.AddOwner(typeof(ButtonBase)); // "共享"/"继承"：重用事件
base.RaiseEvent(new RoutedEventArgs(ClickEvent, this))  // 触发事件

<StackPanel Margin="3" Button.Click="DoSomething"> // 附加事件：利用冒泡，在高层次元素设置handler关联同一个
    <Button>Button 1</Button> <Button>Button 2</Button> <Button>Button 3</Button>
</StackPanel>
ButtonsPanel.AddHandler(Button.ClickEvent, new RoutedEventHandler(DoSomething)); // 在cs中添加handler
```
