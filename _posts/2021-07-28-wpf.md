...
* Prop={Binding 变量名, 选项=...}，实际上是创建了Binding对象并给它设置属性
...
* Model：对应实体，只有属性。M的实例放在VM里
* ViewModel：每个窗体或控件有一个object类型的DataContext。Binding的东西默认从这里取
  * 创建：`<Window.DataContext><local:MainWindowViewModel /></Window.DataContext>`
* 理论：只在界面上存在的属性就属于VM，如用户是否勾选本条数据。VM从M获取数据，转换为适合V使用的状态，再公开修改状态的Command。但VM不知道V如何使用它，特定于某一UI框架的东西不要放到VM，理想上改成UWP不需要改动VM。V使用时不要更改控件的属性，而是调用Command更改VM的状态
...
public class MainWindowViewModel: INotifyPropertyChanged {
    ...
    public int MyProp {
        ...
        set {
            myProp = value;
            OnPropertyChanged(); // 手动调用：PropertyChanged?.Invoke(this, new PropertyChangedEventArgs(nameof(MyProp)));
        }
    }
...
* 控件实现ICommandSource和以下三个属性，就支持命令
  * Command="{Binding xxxCommand}"
  * CommandParameter：不指定时为null
  * CommandTarget：要执行命令的对象，就是sender。必须为RoutedCommand才有效，不设置时默认为焦点

```c#
// 自定义一个为了满足ICommand接口要求的包装类，只需定义一次
class RelayCommand<T>: System.Windows.Input.ICommand {
    Action<T> _execute;
    Predicate<T> _canExecute;
    public event EventHandler CanExecuteChanged {
        add {CommandManager.RequerySuggested += value;} // 不推荐，在几乎所有UI交互时触发。如果CanXxx在VM里则不如下面手动触发，只有当依赖非VM的外部状态如Clipboard.ContainsText()时用它才有意义
        remove {CommandManager.RequerySuggested -= value;}
    }
    public RelayCommand<T>(Action<T> execute, Predicate<T> canExecute=null) {  _execute = execute; _canExecute = canExecute; }
    public bool CanExecute(object param) => _canExecute==null || _canExecute((T)param); // 此为相比于用事件的主要好处，当它返回false时，会自动禁用控件；但只有发生CanExecuteChanged时框架才会调用
    public void Execute(object param) => _execute((T)param);
    public void RaiseCanExecuteChanged() => CanExecuteChanged?.Invoke(this, EventArgs.Empty); // 一般做法，不用RequerySuggested，外部创建一个规定是否启用的属性CanXxx，set时调用此方法，创建cmd时传 _ => CanXxx
}
// 使用时：VM里
public ICommand LoginCommand {get;set} = new RelayCommand(act);
// V里
...
* https://github.com/windows-toolkit/WindowsCommunityToolkit 有CommunityToolkit.Mvvm。给字段加[ObservableProperty]就自动生成了一些代码。教程：https://mvvm.coldwind.top
* https://github.com/PrismLibrary/Prism 好像比较复杂，不过主力维护者一直在
* https://github.com/Caliburn-Micro/Caliburn.Micro
* https://github.com/canton7/Stylet 受Caliburn.Micro的启发，不过各种数据都很少，最后更新2021年2月
...
* CalcBinding XAML的增强
* https://github.com/microsoft/microsoft-ui-reactor 类似于Compose，用于构建WinUI3应用
...
* https://github.com/lepoco/wpfui Win11风格
...
* https://github.com/beeradmoore/dlss-swapper WinUI
