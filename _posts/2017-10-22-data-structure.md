...
* 高度为h的m叉树最多有(m^h-1)/(m-1)个结点，推导自等比数列求和；至少有h+m-1个结点，画图容易理解
* 具有n个结点的m叉树的最小高度为logm(n*(m-1)+1)向上取整；直接画图也不难
...
* 与树的区别：二叉树是有序树，即使某结点只有一个子结点也要分左右；二叉树的度不超过2即可因此可以为空，而对于度为2的有序树至少有三个结点
* 线索二叉树
  * 添加ltag和rtag标识是前驱(1)还是孩子(0)，某个结点的前驱/后继就是它中序遍历的前面/后面那个结点（如果是中序线索二叉树）
  * 树是逻辑结构但线索二叉树是一种物理/储存结构
  * 线索的数量：2n0+n1==n+1，但默认最左边和最右边的节点为null，要具体情况具体分析
  * 用处是先序/中序遍历时不再需要栈，但后序仍需要
