...
* 散列函数和散列地址
  * 直接定址法: H(key)=key或a*key+b，不会发生冲突
  * 除留余数法: H(key)=key%p，最好选择不太接近2的整数幂的素数，且小于数组长度
...
* 冲突处理方法
  * 开放定址法(open addressing)
    ...
    * 再散列法/双重散列：第一个发生冲突时利用第二个散列函数计算增量。Hi=(H1(key)+i*H2(key)) MOD m，且H2产生的数必须与m互素，当m为素数时保证Hash2更小即可
    * 伪随机序列法
  * 链地址法：把具有相同散列地址的记录(同义词)放在单链表中；其实属于开放散列(open hashing)，也可以用树；同一哈希储存多个值，无需探测；还可以建立公共溢出区
