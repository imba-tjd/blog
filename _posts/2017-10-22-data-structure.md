...
* Kruskal算法：最初把所有顶点看作一个单独的树。按权值从小到大排列边，如果选择了某条边以后可以减少一个非连通分量（不构成回路，或边的两个顶点属于不同的连通分量），则选择那条边以及两个顶点，否则不选。直到没有不连通分量。时间复杂度与顶点数无关，适合边稀疏的
...
* 可用于得到无序序列中k个最小元素（Top K），有序的用二分更好
* 堆是用来排序的，在查找时它是无序的
* 调整时都是非相邻的元素，缓存不友好
* 二项堆和斐波那契堆不是二叉树
