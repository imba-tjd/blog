...
* 亮度Brightness：类似于x += y。主要影响暗部层次。对于线性调整来说，过高->黑色发灰，过低->黑位压死(crush black)细节丢失
* 对比度Contrast：类似于x *= 增益。提升->更通透，如阴天有雾变为白天的感觉。过高->高光过曝、暗部压缩，原本白灰色的部分也白成一片
* 灰度Gamma：非线性亮度曲线调整，控制中间调。Gamma本身调高->画面越暗越浓郁。但NV滑块的灰度类似于1/γ，调小是模拟提高Gamma值
* 饱和度Saturation：调到0，变黑白。过大->塑料感，因为自然界没有那么浓的颜色
* 自然饱和度、NV数字亮丽：非线性饱和度增强
