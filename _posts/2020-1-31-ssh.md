...
* 安装：sudo apt-get install openssh-server。安装后开机就会自动运行，关闭用`systemctl disable sshd`
...
* -v显示详细信息，-vv和-vvv更加详细
* 修改PAM后不需要reload sshd，在下一次登录时会自动使用新PAM配置，使用错误的PAM配置覆盖后，下一次登录会立即失败
...
PermitRootLogin yes # 默认no，可设为prohibit-password。相关的还有AllowUsers和DenyUsers，支持限制地址。无法使用/etc/hosts.deny，它必须配合第三方库TCPWrappers
...
#无需设置：AEAD隐含了MAC，etm意思是encrypt-then-mac更安全。HostKeyAlgorithms当存在多个key时优先使用哪个算法的。PubkeyAcceptedKeyTypes似乎不生效。HostKey指定服务端密钥的位置，默认包含了/etc/ssh/ssh_host_ed25519_key
...
* https://github.com/PowerShell/Win32-OpenSSH msi安装后自动启动sshd和agent，配置和服务端密钥在%ProgramData\ssh。可以用windows的用户名和密码登录。实测密钥登录不成功
