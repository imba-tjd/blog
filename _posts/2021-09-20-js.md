...
* bin:{"myapp":"xxx"} 安装后会创建二进制入口，开发依赖在./node_modules/.bin/下
...
* i/install pkg@version，up/update按package的语义更新lock，un/uninstall
  ...
  * -g：安装到%AppData%\npm\node_modules
* ls：列出安装了的包，ll也显示包的描述，-a显示包内部的依赖。ou/outdated显示过时的包
* cache clear --force
...
* ts-node命令行在package里指定type:module时无法使用。ts-node-esm：必须要有tsconfig里设置module才能工作
...
* any(等于没声明类型，与其它任意类型运算都是它) unknown(可以任意赋值但无法访问任何属性，类型系统中的Top，所有类型的父类) never(类型收窄时的Bottom，是任何类型的子类型，表示不会到达，如函数只会抛异常时使用) void(无返回值的函数) enum(优先用const enum，不支持计算项，编译时移除)
* 数组：`Array<T>`或T[]、`ReadonlyArray<T>`或readonly T[]
* tuple：如`[number, string]`
* Type Aliases：type T = 其它类型声明。与interface相比不允许重新定义
* 函数类型：(user: User) => void。在object class interface中声明返回类型还是用的冒号
...
* 继承用extends，实现用implements。另一种扩充方法：多次定义同名接口
  * 支持多继承，多个父类有同名属性时会报错
* 构造函数类型声明：new()
* 也能声明属性：属性名:类型。可选属性：属性名?。只读属性：readonly，必须初始化
* 可索引类型：`[index: number]: any`、`[key: string]: any`。伪数组：IArguments
...
* https://github.com/tinyhttp/tinyhttp 类Express，ts
