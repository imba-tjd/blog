...
* substring()：两个参数都是索引，但end却是右开区间，负数当作0，两个参数中的较小者作为起始点，行为诡异一般不用。substr()第一个参数可为负会倒数，第二个参数是length，不在核心规范中但实际上都实现了。slice见数组的，一般用它
...
* length toUpperCase() toLowerCase() indexOf() trim() includes() startsWith() endsWith() padStart() padEnd() trimStart() trimEnd()
* 转换成字符串：+""或String()或.toString()，后者对于数字字面量要用两个点，转换数字时可指定进制，无前缀
* 字符串转数字
...
* indexOf() lastIndexOf() push(变参) pop() unshift()等于lpush shift() reverse()原地
* sort()：原地；无参调用隐式转成字符串按ASCII升序排序，导致[9,10].sort()为[10,9]。传`(a,b)=>a-b`后正常；此自定义排序函数应返回数值而不是bool
* splice()：从指定索引删除几个元素作为返回值，再从该位置添加元素。传单个参数等于将原数组在指定位置拆分成两个数组
...
* 高阶函数
  * map()：传`String`就能把所有元素变成字符串
  * reduce()：[a,b,c].reduce(f)相当于f(f(a, b), c)，第二个参数可指定初始值防止数组为空时报错；还有reduceRight()
...
Object.assign(Man.prototype, Animal.prototype) // Mixin，伪多继承，把Animal的方法加到Man上
...
* setTimeout() setInterval()
  * 第一个参数是要执行的回调函数，也可以是字符串表示的js代码
  * 第二个参数是毫秒。前者不传或传0并不是真的0秒后就执行，而是等本轮事件循环完成后再执行，可循环中避免大量设置DOM造成阻塞，而且浏览器可根据是否是当前tab进一步延迟以省电。后者计算的是开始的事件，不考虑执行时间；假如设置的100，但执行要50，感觉上就只间隔了50
  * 剩下的参数为回调函数的参数
...
* getElementByxxx() querySelector(All)()：后者为css选择器，注意FF的复制css路径不保证唯一。也可以在查询出来的节点上用
...
* hidden visibilityState：前者为bool，后者为visible和hidden字符串，都是只读的，窗口最小化 选项卡切换 锁屏就是不可见的
...
* 文本
  ...
  * textContent：标准化得更早一些，IE8不支持。差不多就是原样把所有HTML标记去掉，会得到style和script里的内容
  * 赋值时两者都会自动对内容进行HTML编码，尖括号变成实体，保证无法直接设置HTML元素
...
* remove() before() after() replaceWith()以自身为对象调用
* contains()：是否含有指定子节点
...
* className属性：对应CSS的Class，因为class是关键字
...
* outerHTML 给它赋值相当于在DOM中替换元素但原元素还是在内存中
* 大小
  * clientWidth clientHeight 含有padding和content，不含滚动条，html元素的这俩属性就叫viewport的大小
  * offsetWidth offsetHeight 加上边框。对于自然出现滚动条的元素，大小与scrollWidth差不多
  * clientTop clientLeft 边框宽度
  * scrollWidth scrollHeight 当内容超过其宽度时的实际宽度，即使overflow:hidden了也是，包括::before的大小
  * scrollTop scrollLeft 可写，拖动滚动条后元素被卷起的宽度
...
* dispatchEvent()、fireEvent()：添加/移除事件，后者用字符串即可，前者要自己new Event出来
* EventTarget.addEventListener(eventname, e=>...)：添加事件处理程序
...
* onload：HTML加载完成后触发，在这里取元素不用担心还没加载完导致找不到。body和img等几个也支持；注意不是document下的
...
* 现在一般用addEventListener，因为onXXX会覆盖之前绑定的处理器
...
* jQuery对象可认为是个HTML对象数组，用`[]`取出HTMLElement，get()无参全取出。设置它们的属性会应用到每一个数组元素，对于`[]`也不报错。一些函数传一个参数是查询，传两个是修改；还有一些函数无参调用是查询，传参是修改。查询时对于多个元素最好不要直接用，而是用.each(function(ndx){$(this)...})
* 选择器找不到时为`[]`；支持:has和:contains(相当于has-text) :input/text/password/submit/checked/selected表单元素
* 进一步选择，关键是用这些方法后对象仍是jQuery对象：find(选择器/jQuery对象) filter(选择器/回调函数) map() next(支持选择器) nextAll() prev() eq(第几个) slice(替代:eq和:lt) first() last() even() siblings() children() contents()也会选中文本节点 parent()直接父元素 parents()一直往上的每一个父元素集合，支持选择器来过滤 closest() 从自己开始往上找满足选择器的第一个元素
* end()：链式调用时，假设前面调用了find()和css()，调用此函数后可返回最开始的对象上
* text() html()：前者支持回调
* css()：作用于inline style
* is()：类似于$()，不过返回bool
* 操作类：hasClass() addClass() removeClass() toggleClass()
* 操作属性：attr() removeAttr()。prop用于没有值的属性：prop('checked')->true，attr('checked')->'checked'
...
* $.get getJSON getScript加载后就执行：第二个参数为object的查询字符串
* $.post：第二个参数是表单body
...
* ghost：类似于Wordpress
* 数据库：Sequelize、TypeORM
* 服务器
  * PostgREST 把PG变成RESTAPI
...
---
https://wangdoc.com/javascript/dom/element.html#elementscrollleftelementscrolltop
...
TODO: https://developer.mozilla.org/zh-CN/docs/Web/HTTP/CORS https://zhuanlan.zhihu.com/p/24198444
