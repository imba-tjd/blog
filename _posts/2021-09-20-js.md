...
* 展开操作符`...`：支持一般数组和iterable的展开，[...arr]相当于浅拷贝数组或把伪数组转换为真数组。{...o1, ...o2}相当于合并object，{...o1, k:v}相当于修改或新增属性
...
* typeof返回字符串：typeof 123 -> 'number'; typeof f -> 'function'; typeof []/{}/null(历史原因其实是错的) -> 'object'
...
* 原理实际上是对象，[]里的会自动变成字符串
* foreach遍历：ES6 for of，自动跳过undefined。for in仅返回索引。iterable类型具有forEach方法用回调
...
* splice(pos, delcnt, 变参新值)：从指定索引(可为负)删除几个元素作为返回值，再从该位置添加元素。原地修改数组，返回被删除的。传单个参数会删除后面的全部，等于将原数组在指定位置拆分成两个数组
...
* ES6语法糖：已有变量p，`{p}`相当于`{p:p}`。key加上中括号后可在里面动态计算，且可用变量
...
class Person {
    ...
    hello() { // 没有function关键字；其实ES6普通对象也能这样
        console.log('hello')
    }
...
* Date对象
  * 不加new得到的是字符串 "Sun Dec 15 2024 16:44:55 GMT+0800 (中国标准时间)"，相当于d.toString()。d.toLocaleString() -> "2024/12/15 16:45:04"
  * 解析：new Date('datestr') 支持ISO格式，默认当前时区，如果没有时间默认为0，支持用空格代替T。也支持上面的字符串
  * 格式化成ISO：d.toISOString() -> "2024-12-15T08:50:06.636Z"
...
* setTimeout() setInterval()
  ...
  * setTimeout不传时间或传0并不是真的0秒后就执行，而是等本轮事件循环完成后再执行，可循环中避免大量设置DOM造成阻塞，而且浏览器可根据是否是当前tab进一步延迟以省电。另一种方式：requestIdleCallback
...
* userAgent：弃用了，改用userAgentData（纯字符串->对象）
...
* submit()手动提交，会触发onsubmit事件，返回false就不会提交；用户在最后一个输入框按回车也会提交。如果不想表单提交后跳转，处理函数第一句用e.preventDefault()或onsubmit返回false
...
* obj.addEventListener(eventname, e=>...)：添加事件处理程序，第三个参数可传{once:true, capture:true}
  * 移动端滑动性能优化：监听touchxxx事件时，如果不需要调用preventDefault()，则应设置passive:true
* e.stopPropagation()：事件会由内而外冒泡，此方法就是停止冒泡
* e.target是最初触发事件的元素，当在父元素上加事件处理程序但目的是处理多个子元素，冒泡到上层时此变量就有用了；而那时this就只是上层元素，永远等于e.currentTarget
...
* HTML的onxxx属性为JS语句（函数体）而不是函数名，会自动包装成一个具有event参数的函数，且还会再包一层with(document)和with(当前元素)，表单里还会再加with(form)，因此不用加document.、form里可以直接用submit()
...
* 不能访问DOM，不能跨域加载JS
* 顶层对象是self
* 不同worker之间内存是隔离的，不能直接访问彼此的对象。SharedArrayBuffer除外
* 库：Workerize
...
* 原理：客户端GET text/event-stream。服务端按特定格式不断返回数据，一般是`data: 数据\n\n`，发以冒号开头的注释保活
* 封装：const e = new EventSource('url', opt) 选项包括是否发送cookie、要发送的headers、超时。处理e.onopen onerror onmessage事件。断连会自动重连
...
* 安装
  * https://github.com/Schniz/fnm Rust写的跨平台Node版本管理和安装器，本身用脚本安装。fnm install 18/--latest; fnm use 18。临时使用：fnm env --use-on-cd | Out-String | Invoke-Expression
  * npx n latest 不支持win
  * nvm 不支持win。另有nvm-windows
  * curl -fsSL https://deb.nodesource.com/setup_19.x | bash - && apt install nodejs
  * https://registry.npmmirror.com/binary.html?path=node 其中latest文件夹只有各大版本的前9个小版本，10排序在1后而不是9后
* CLI
...
* 后缀为mjs，或package.json指定type:module。都没有时node会看是否用了ESM语法来检测
...
import { a, c as b, say } from './m.js' // 按需导入。与解构不同，是“符号绑定”，可理解为引用
...
import 'vue' // 在node环境下，会导入node_modules里的东西
```
...
* bin:{"myapp":"xxx"} 使用者安装后会在./node_modules/.bin下创建二进制入口，scripts里的命令和npx能调用到而无需添加那个目录进PATH
...
* https://github.com/biomejs/biome：目标是替代Babel, ESLint, webpack, Prettier, Jest。类似的：https://github.com/oxc-project/oxc 前端oxc好
* 不学的
...
## [tsconfig.json](https://aka.ms/tsconfig)

* tsc --init
* 查看所有选项的默认值：tsc --all
* module:ESNext、NodeNext(Node16后根据package的type解析为CJS或ES2020)
* 根下的include:["src"]，等效于`src/**/*`。不是compilerOptions的属性，而是与它同级。不是绝对的文件边界，它匹配到的文件import的内容不需要include也会处理
* outDir:dist
* sourceMap:true 调试需要
* noEmitOnError 默认false，tsc报错时仍生成js文件，因为有的js确实能运行
...
* 继承：extends
* rootDir 用于清理输出目录结构。例如源代码在src里，如果本项设为src，则输出后没有src；如果设为.则保留。ts6后默认为tsconfig所在文件夹，之前会自动推断
* types：指定要处理的@types/xxx包。会默认添加代码中import的（如果有）；之前会处理所有node_modules里安装的，现在不会了。一般仅当node开发时填node，vite模版里会自动添加
* paths:{"jquery":"node_modules/jquery/dist/jquery.min.js"} 映射导入库的路径
* lint：noUnusedLocals、noUnusedParameters、erasableSyntaxOnly（不使用enum）、noFallthroughCasesInSwitch
* composite模式：用于monorepo。项目结构变为根tsconfig仅包含子项目，子项目里启用本项和其他选项。在根用tsc -b编译所有项目。会自动处理依赖顺序、增量编译
* lib：不写时，会根据target自动注入；如果写了则无默认值。ESNext默认esnext、dom、dom.iterable、dom.asynciterable、webworker.importscripts、scripthost（Windows Script Host历史包袱）
* moduleDetection: force 将每个文件当做模块而非全局脚本。对于vite，已经是模块化了，此选项让tsc对齐。另外d.ts不受影响
* tsc编译
  * -p 指定tsconfig。一般不用指定，自动从CWD依次往上找。可用来指定jsconfig
  * -d 也生成d.ts
  * -w watch
  * --noEmit 单独用表示只进行错误检查。若用打包器则写入配置文件中
...
* 在js中启用ts检查
  * `// @ts-check`在单个js中启用
  * 全局启用：VSC的设置中js/ts.implicitProjectConfig.checkJs。好像并不会被tsconfig的值覆盖
  * tsconfig的checkJs，隐式allowJs。使用时还必须指定“不在项目中”的outDir，纯ts不用是因为本来就不会检查js
* 设置
...
* jsconfig.json
  * 不是必要的，一般用途：排除不需要intellisense的文件夹、一个VSC工作区中包含多个项目（各创建一个本文件）、用tsc编译js
  * “继承”于tsconfig，本质一样，只是默认启用了allowJs
  * 先ctrl+space创建基本结构，添加module:ESNext和exclude:["node_modules","dist"]或用include
    * 如果目录结构为 工作区/src/本文件和其他JS项目文件 工作区/dist和node_modules，则无需排除
  * 默认会添加package的dependencies的类型包。手动添加在jsconfig中用"typeAcquisition":{"include":["jquery"]}
* 扩展
  * ~~StandardJS 强制性的风格~~ 优先ESLint
  * Prettier
  * CSS Peek 在HTML中Peek class来源
  * quokka.js 即时显示表达式的值
  * npm Intellisense
  * SonarQube for IDE 也支持其他语言
* `No inputs were found in config file ...`：没有ts文件可供编译
* VSC的tsc LS独立于npm安装的，右下可更改
...
* chart.js
* dotenv
* lodash lodash-es toss/es-toolkit https://github.com/you-dont-need/You-Dont-Need-Lodash-Underscore：函数式编程和工具类
* ramda 函数式编程，与ts兼容性差
* Socket：ws、socket.io
...
* 临时HTTP服务器：http-server（最后更新2022年6月）、serve（vercel出的）
  * json-server --watch db.json 支持HTTP查询和修改
* immerjs 不可变状态管理
...
* htmx 在html里写属性发出ajax请求和控制dom，14KB
* preact 重新实现的react，4KB
* solidjs 类react，无VDOM
* qwik TS，全栈，类react，首屏加载速度快，体积小。没有1.0
* lit.dev 快速创建WebComponents。在线IDE：https://webcomponents.dev/
* MithrilJS 现代化的SAP，9KB
* 各框架语法对比：https://component-party.dev/
...
* SpiderMonkey：FF的js引擎。https://archive.mozilla.org/pub/firefox/nightly/latest-mozilla-central/jsshell-win64.zip
* WASM引擎：cloudflare/workerd https://github.com/appcypher/awesome-wasm-runtimes
...
* form：https://react-hook-form.com https://formik.org/
* Next.js
* shadcn：UI库，使用了tailwind。特点是那些组件的源代码直接拷贝到项目里，而不是用封装的黑盒
...
* Electron是完整开发桌面软件的方式，只要会前端技术就行。tauri是给传统软件加入WebView作为GUI。nwjs：能在HTML里直接使用node的api，而Electron是分离的
...
https://zhuanlan.zhihu.com/p/24150808 ES6
