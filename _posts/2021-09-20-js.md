...
* e.target是触发事件的元素，当在父元素上加事件处理程序但目的是处理多个子元素，冒泡到上层时此变量就有用了；而那时this就只是上层元素，永远等于e.currentTarget
...
* 根下的include:["src"]，等效于`src/**/*`。不是compilerOptions的属性，而是与它同级。要不就用`exclude:["dist","**/node_modules/*"]`
* outDir:dist
* sourceMap:true 调试需要
