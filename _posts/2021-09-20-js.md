...
* 解构赋值
  ...
  * `...`也可用于一般数组和iterable的展开，[...arr]相当于浅拷贝数组，但不能用于伪数组 TODO: 经过测试，[...args]可以把伪数组转成真正的数组
  * var { toString: s } = 123
...
* substring()：两个参数都是索引，但end却是右开区间，负数当作0，两个参数中的较小者作为起始点，行为诡异一般不用。substr()第一个参数可为负会倒数，第二个参数是length，不在核心规范中但实际上都实现了，又但是应该避免使用。slice同数组的，一般用它
...
* 默认操作的都是当前包的运行时依赖，install时会下下来。-D/--save-dev表示开发依赖，install --production时不会下。-g略。-d为debug
* npm config set registry https://registry.npm.taobao.org
* i/install pkg@latest，up/update按package的语义更新lock，un/uninstall
* ls：列出安装了的包，ll也显示包的描述；--all显示包内部的依赖。outdated显示过时的包
* package.json
  * 创建：npm init -y
  * name、version、license、author、keywords、description
  * main:"index.js"  npm start运行的文件
  * bin
  * dependencies:{"pkg":"ver"}、devDependencies
  * scripts:{"自定义命令": "bash"} 用 npm run 命令 运行
  * type:"module"
  * exports（12.7，使用后封禁子路径）
* npm cache clear
...
* 语义化版本
  ...
  * 所以存在lock且不编辑package时install，package就跟没有一样
* npm view pkg：查看包的信息
...
* fs
  ...
  * promises提供了一组async方法而不是回调，优先使用。const fs=require("fs").promises
...
* htmx 在html里写属性发出ajax请求和控制dom，兼容IE11
* lit 快速创建WebComponents
* preact 重新实现的react，压缩后3KB
* solidjs 类react，无VDOM
* qwik TS，服务端渲染，类react，首屏加载速度快，体积小。没有1.0
* immerjs 难度大
