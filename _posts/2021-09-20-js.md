...
* substring()：两个参数都是索引，但end却是右开区间，负数当作0，两个参数中的较小者作为起始点，行为诡异一般不用。substr()第一个参数可为负会倒数，第二个参数是length，不在核心规范中但实际上都实现了，又但是应该避免使用。slice()同数组的，一般用它
* split()：一般传正则。无参调用不会做切分，''变为字符数组，' '无法处理连续空格和换行符。字符数组转字符串：arr.join('')
...
* JSON.stringify(o, null, '  ')：第二个参数可为['要序列化的属性',]，也可为(key, value)=>{}对每个对象都用一遍的convert函数。第三个参数为缩进。若o定义了toJSON函数则会自动使用
...
* window.onload：所有资源加载完成后触发，在这里取元素不用担心还没加载完导致找不到；但若网页已经加载完则不会触发，要配合document.readyState。注意不是document.onload。body和img等几个也支持。document上的DOMContentLoaded事件：初始的HTML加载完就会触发
...
* 默认不带cookie，option加`credentials:'same-origin'`，或用`credentials: 'include', mode: 'cors'`
* 404或500等并不会引发错误，只会在网络错误（如IP无法解析、CORS不允许）的情况下才视为错误
* 不自带超时、取消、监视进度，而XHR有
* window.Headers、Response、Request
* URLSearchParams：构建查询字符串
* 第三方网络库：axios sindresorhus/ky
...
req.open('GET', '/bar/foo.txt', false);  // false表示是同步阻塞的
req.send(null);
if (req.status === 200) {
...
* 安装
  * https://github.com/Schniz/fnm Rust写的跨平台Node版本管理和安装器，本身用脚本安装
  * npx n latest 不支持win
...
* 导出本模块的对象：`exports.xxx=xxx`，其中exports是预定义的object
...
* 路径
  * ESM导入本地的文件必须加后缀，CJS会自动猜测后缀和允许导入含有index.js的文件夹。Node可以自定义Loader模仿CJS行为，但浏览器要求加，因此不考虑不加
  * 使用CJS路径的Node库：import fs from 'node:fs/promises'，否则要写import { promises as fs } from 'fs'
...
* bin:{"myapp":"xxx"} 安装后会创建二进制入口
* dependencies:{"pkg":"ver"}、devDependencies
* scripts:{"自定义命令名": "bash内容"} 使用用 npm run 命令名。存在server.js时npm start会使用它，不支持mjs
* type:"module"
* engines：最低使用的Node版本
* 不允许有trailing。不能直接写注释，但允许在根元素下写一个"//"的属性
...
* i/install pkg@version，up/update按package的语义更新lock，un/uninstall
  ...
  * -g：安装到%AppData%\npm\node_modules
* ls：列出安装了的包，ll也显示包的描述；--all显示包内部的依赖。ou/outdated显示过时的包
* cache clear --force
...
* https://github.com/huozhi/bunchee 零配置
...
* ts-node-esm：必须要有tsconfig里设置module才能工作
* https://github.com/esbuild-kit/tsx 感觉可以完全替代ts-node。esno：在tsx上再加了一点点功能，不使用
* https://github.com/digital-loukoum/esrun
* 其它各编译器的比较：https://github.com/privatenumber/ts-runtime-comparison

## 在浏览器里直接执行TS脚本

* `<script src="https://unpkg.com/@babel/standalone/babel.min.js" type="module"></script> <script type="text/babel" data-type="module" data-presets="typescript" src="./test.ts"></script>`
...
* 导入本地模块用from './xxx'或'./xxx.js'，不能是.ts
* 命名空间namespace：不使用
...
* @types/xxx维护了大量常见库的声明，其中node应作为-D，其余的应为直接依赖
...
* rootDir 默认为tsconfig所在目录，应包含所有待编译源文件，感觉不需要更改。如果需要指定需编译的文件夹范围，可用根下的include属性设为["src/**/*"]，（非compilerOptions，而是与它同级），此项也会更改输出的目录结构
* outDir:dist
* noEmitOnError 默认为false，tsc报错时仍生成js文件，因为有的js确实能运行
* noImplicitAny true
* sourceMap 可选true
* resolveJsonModule 开启后能import from json文件，仅限Node
* esModuleInterop 模板默认启用，允许从CJS中导入库
* tsc编译
  * 无参tsc：读取cwd下的tsconfig，编译所有include的ts文件
  * -d 也生成d.ts
  * -w watch
...
* jsconfig.json：“继承”于tsconfig，默认启用了allowJs
...
https://bun.sh/ 不支持Win
https://github.com/tauri-apps/tauri Electron的同类产品
quokka.js VSC扩展，即时显示表达式的值
Electron：https://github.com/electron-userland/electron-forge
