...
* 导出本模块的对象：`exports.xxx=xxx`，其中exports是预定义的object。默认导出：module.exports=xxx或export=xxx
...
* 映射
  ...
  * `Readonly<T>`：表示把T的属性变为只读的对应的类型。实现上是{readonly [P in keyof T]: T[P]}，此处P in会依次把P绑定到T的属性上
  * Partial：把属性变为可选的
  * `Pick<O, 'a'|'b'>`：抽取属性子集
