...
* Precision精度 is not accuracy准度：如现在时间其实是1:00:00，如果回答1:30:00则精度高，回答1:05则准度高。GetTickCount函数有1ms的精度，但准确度取决于计时器tick频率；QueryPerformanceCounter有更高准度，但使用前要先用QueryPerformanceFrequency获得精度，且用起来更慢
* -DWIN32_LEAN_AND_MEAN：当不使用COM时定义，能加速
* `_WIN32_WINNT`和WINVER：MinGW目前默认为0xa00(WIN10)

### 错误码

* 0xc0000135：缺失DLL
* 0xc0000139：定位符号失败，如将MSVC的DLL与MinGW的exe链接到一起
* https://learn.microsoft.com/zh-cn/windows/win32/debug/system-error-codes 好像和HRESULT无关。FORMAT_MESSAGE_FROM_SYSTEM：https://learn.microsoft.com/zh-cn/windows/win32/api/winbase/nf-winbase-formatmessage
