...
* -sfx7z.sfx：图形化自解压
* LZMA2
  * 默认压缩等级为5，用-mx=9极限压缩，=1可完全代替zip
  * 默认启用多线程，360压缩默认单线程，机械硬盘考虑用`-mmt=-`关闭多线程
  * -ms=e -mqs 分文件类型固实压缩，能秒开txt
  * 是LZMA的修改版本，对于不可压缩数据能更好地处理，但是一定要开固实压缩
* ZIP：支持检测后缀，-mcu使用U8储存文件名，-mcp=xxx解压时使用指定代码页。不支持Deflate算法
...
* 自己创建支持被rundll32调用的库：实现`void CALLBACK EntryPoint(HWND hwnd, HINSTANCE hinst, LPSTR lpszCmdLine, int nCmdShow);`，其中不加A或W的后缀默认是MBCS编码，若用W后缀则第三个参数为LPWSTR。但实测失败了
