...
* fsutil repair set c: 1：启用NTFS自愈功能，默认是基础修复并警告，state查询是否有损坏
...
* -DWIN32_LEAN_AND_MEAN：当不使用COM时定义，能加速
...
* SMB：使用445端口不可更改，被运营商封了。WinServer 2025和Win11 25992才支持配置端口。over QUIC可用443但Azure不支持，且也要一些配置。还支持压缩。可考虑强制用SMB 3.1.1。Set-SmbServerConfiguration -EncryptData True
* WebDAV：基于HTTP
...
新建DWORD BingSearchEnabled值0。实测regedit会报错，进安全模式后可以创建。
```

## 共享

* 开启网络发现：第一次点 此电脑-网络，选“否”，即可，会启用所有专用网络的网络发现。之后输入账户密码应就可访问 C:\Users\该用户 的所有内容
* 更改公共网络和专用网络（网络位置）：Win11设置-网络和Internet-右上角属性。Win10新版应该也在设置里，老版在“本地安全策略”里。切换不同Wifi应该会不同
* 无密码共享：控制面板-网络和共享中心（或此电脑-网络-右键属性）-更改高级共享设置-所有网络-最后一项关闭。第一项公共文件夹共享是C:\Users\公用，开启后所有人无需密码任意权限。这里也可以用来关闭网络发现
* 共享文件夹：在要共享的文件夹上右键属性-共享-高级共享，权限添加Everyone，再到安全里再添加Everyone。非“高级共享”的共享会暴露完整路径，且会在父文件中依次添加“特殊权限”，导致即使没有共享内容也能被看到路径。只有C:\Users下的文件夹的属性里才存在共享Tab
...
* 禁用TLS1.0：reg add "HKLM\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\TLS 1.0\Client" /v Enabled /t REG_DWORD /d 0 再加DisabledByDefault=1 另外还要改WinHttp的配置且要改32位和64位两处
...
* 更换微软账户关联的系统许可证版本：疑难解答 - 搜索许可证
* Win+X不使用PS：reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced\" /v DontUsePowerShellOnWinX /t REG_DWORD /d 1 /f
