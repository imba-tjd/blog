...
* 图形界面可以右键运行，当时就能测试一下
* 隐藏运行：选用另一个用户的身份执行，且要选不管那个用户是否登录；SYSTEM两者都满足。已知当前用户选Hidden没用
* 关闭断开电源时默认会杀掉程序
...
* 查看：msinfo32
* 关闭：bcdedit /set hypervisorlaunchtype off，会关掉hyper-v和wsl2，但其他vm应该还是可用的。当然bios里关闭vt也可以，都关

## WD

必须到WD面板里关闭“篡改防护”。导入后实测24H2能关掉所有实时防护，但那个杀毒进程仍在运行，占100MB。
...
* 禁用TLS1.0：reg add "HKLM\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\TLS 1.0\Client" /v Enabled /t REG_DWORD /d 0 再加DisabledByDefault=1 另外还要改WinHttp的配置且要改32位和64位两处 TODO:看Win11是什么样的
