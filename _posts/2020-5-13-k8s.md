...
* Control Plane：API Server（暴露k8s API，是其它组件与k8s交互的入口）、etcd（状态和配置数据）、Scheduler（根据调度策略为新Pod选择一个Node）、Controller Manager（有多个，分别管理一种资源，如Node、Deployment）
* Node：Kubelet（与API Server通信）、Kube-proxy（维护网络规则）、容器运行时（containerd）
...
* Pod：一个pod可以含有多个紧密配合的container，一定在同一个Node上，生命周期相同；共享网络（如相互通信用localhost）、支持IPC，但不直接共享存储。一个容器仍只运行一个进程，sidecar容器为主容器提供服务，如日志监视、HTTP反代
...
* StatefulSet
  * 很多有状态的程序都需要集群式的部署，意味着节点需要形成群组关系，每个节点需要一个稳定唯一的ID(例如Kafka BrokerId, Zookeeper myid)来作为集群内部每个成员的标识，集群内节点之间进行内部通信时需要用到这些标识
  * Headless Service：不分配ClusterIP(spec.clusterIP:None)
  * Pod可以重建，但每个Pod有稳定的DNS名字和（自动创建专属独立的）持久卷（用volumeClaimTemplates）。重启时能按顺序进行
  * 如果只有单副本，可以用Deployment+PVC，卷也是持久的。默认一个PVC只能被一个节点挂载，手动可指定共享（一般用于只读静态资源）。不能自动创建
...
spec:
  ...
  strategy:
    rollingUpdate:
      ...
      # 这两者都是默认值。效果是更新时会先拉起新Pod，等真正可用了才销毁老的，防止启动时间过长；还要配startupProbe
    type: RollingUpdate
    ...
    spec:
      ...
      - name: nginx
        ...
        resources: # 如果只限制而不请求，则请求的和限制的相同；如果只请求，则限制的值会是集群的默认值（kind: LimitRange）
          ...
          limits: # 超过就炸
            ...
        livenessProbe: # 健康检查，它失败了则会重启容器，解决“进程活着但卡死”的情形。readinessProbe失败了只会暂时摘除流量，检查程度应比live更深。startupProbe判断应用是否启动成功，一般留较长的超时时间
          httpGet:
            path: /v1/health
            port: 443
            scheme: HTTPS
...
* 网络
  ...
  * Ingress：相当于反代，HTTP可以根据前缀分配到不同的后端服务，可以是HTTPS终结点。华为云上后端服务必须为NodePort类型的
...
* HashiCorp Vault：用来储存秘密的，是consul的公司出的。它还出了Nomad，定位与k8s类似但也可用于非容器，复杂程度低
* Temporal：“耐用性工作流引擎”。编排逻辑，确保那些需要跑几小时甚至几天的业务流程不会因为 Pod 重启而中断
