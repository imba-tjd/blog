...
## [k3s](https://rancher.com/docs/k3s/latest/en/installation/install-options/server-config/)
...
* 有的命令加不加s都一样，logs除外
* `alias k="kubectl"`；`touch ~/.kube/config; chmod 600 ~/.kube/config`；`source <(kubectl completion bash)`；`alias docker=crictl`
...
```bash
k get xxx -o wide可以得到更多的信息，-o json可以以json格式输出，同时指定多个资源用逗号且无空格，-v=7可以显示详细信息
k create/get/delete node/deploy/ns/issuer/pod/service/componentstatus/service/certificates <资源的name>
k apply -f xx.yml/<-R -f .>/<-f -> [-n <指定空间名>]；get那些也可用
k config current-context/get-contexts/use-context/view name [nsname]
k version/cluster-info
k logs [-f] podname或-l lablename
k describe 资源类型/资源名 # 显示已部署的资源的信息，其中资源名可省
k explain <资源类型> # 类似于文档，会显示API版本
k exec podname或deployment/depname -- cmd arg
k run ngx --image nginx; k expose pod ngx --port=8080 --nodeport=30000 --type=NodePort; # 直接运行nginx pod
...
k proxy --port=xxx # 用于调试ClusterIP类型的Service时暂时暴露出来
```
...
spec:
  ...
  template: # 下面的内容相当于Pod.yml（apiVersion: v1），还需name
    metadata:
      labels: # 必须要有，且必须在template里
        app: nginx # key也可以自定义
    spec:
      containers:
      - name: nginx
        image: nginxinc/nginx-unprivileged # bclswl0827/v2ray-openshift; v2ray/official; xiaokaixuan/ss-v2ray-plugin bawodu/v2ray-kubesail
