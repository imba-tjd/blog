...
9. 这个方案的问题在于，private key既用于证书身份识别(authentication），又用于premaster key的解密。一旦private key泄露，那么所有使用这个SSL证书的密文premaster key都能被解密。但根据CF的图，第四步发送的其实是一个premaster key，之后再根据两个随机数生成密钥，~~我看不出哪里有问题~~客户端发送的随机数是明文的，服务端发送的是可以被所有人解密的。
