...
9. 这个方案的问题在于，服务器私钥既用于证书身份识别(authentication)导致必须固定，又用于premaster key的解密。一旦服务器私钥泄露，所有使用这个SSL证书的密文premaster key都能被解密，随机数又都是公开的，就能生成会话密钥，故不具有前向安全性。而DH每次随机生成私钥。
