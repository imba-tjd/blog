...
4. 客户端用CA公钥解密证书，成功则随机生成一个会话密钥（单纯由客户端生成），用服务端的公钥加密发出（只有服务端能解密）。
5. 服务端用私钥解密，返回一个确认的包。
...
9. 这个方案的问题在于，服务器私钥既用于证书身份识别(authentication），又用于premaster key的解密。一旦服务器私钥泄露，那么所有使用这个SSL证书的密文premaster key都能被解密。但根据CF的图，第四步发送的其实是一个premaster key，之后再根据两个随机数生成密钥，~~我看不出哪里有问题~~客户端发送的随机数是明文的，服务端发送的是可以被所有人解密的。
