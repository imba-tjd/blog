...
HTTP1.1及以下，没有字段用来区分一个response是归属于哪一个request的，只能在发送一个request之后，等待response的到来。但HTTP2有。

优化：1.缓存，不发送请求。2.合并多个请求，减少次数。3.压缩，图片用有损的。

### HTTP/2

* 基于stream的多路复用：同个域名只用一个TCP连接，浏览器不再有6个连接的限制。单个连接可以承载任意数量双向数据流，数据流以消息的形式发送，消息又由一个或多个帧组成。可以并行发送，不会有消息的队头阻塞
* 二进制分帧：头放到一个帧里文本传输，数据帧以二进制传输，多个帧乱序
* 头部压缩：客户端和服务器端使用“首部表”来跟踪和存储之前发送的键值对，包括静态表和动态表，如将GET方法预先约定编码为2。每次请求和响应不再发送相同的数据，只发送差异数据
* 服务端推送Server Push，又叫Comet。其它手段：轮询、长轮询、WebSocket
...
* 必须用TLS1.3
* 适用于高丢包网络环境和移动网络
* 浏览器测试：https://quic.nginx.org/
  * 浏览器强制使用：--origin-to-force-quic-on=host:port
* 网站测试：https://http3check.net/
...
客户端测试：https://www.howsmyssl.com/ https://tls13.akamai.io/ https://tls13.1d.pw/ 后者仅仅支持1.3
...
4. 核心步骤：客户端用CA公钥解密证书，成功则随机生成一个Pre-Master Secret/Key, PMS（单纯由客户端生成），用服务端的公钥加密发出，只有服务端能解密。
5. 服务端用私钥解密，返回一个确认的包。双方根据两个随机数和PMS生成会话密钥；这样做是为了防止客户端的PMS不是随机的。
6. 之后一直使用会话密钥对称加密通信。算上TCP握手一共3-RTT，再算DNS解析就是4RTT。
7. 4也可以携带数据，称作TLS False Start，相当于握手只消耗了一个RTT；但不支持RSA。
8. 会话复用：缓存对称密钥，则总共2RTT。分为Session ID/caching和Session Ticket；前者在服务端内存中缓存密钥；后者服务端将密钥加密后让客户端带上，服务端无状态。具体见：https://www.zhihu.com/question/52320264
9. 这个方案的问题在于，服务器私钥既用于证书身份识别(authentication)导致必须固定，又用于PMS的解密。一旦服务器私钥泄露，所有使用这个SSL证书的密文PMS都能被解密，随机数又都是公开的，就能生成会话密钥，故不具有前向安全性。而DH每次随机生成私钥。

### TLS 1.3

1. Client Hello时还携带用于生成密钥的key_share和客户端DH参数（即DH公钥；DH私钥自己留着）
2. Server Hello使用唯一支持的加密方式AEAD生成会话密钥，发送服务端DH参数。RSA不支持前向安全被废弃；CBC模式密码易受BEAST和Lucky 13攻击
3. Client接收并进行证书验证，生成相同的密钥，不用发包确认。接下来就是互相发数据。算上TCP握手一共2-RTT

* 新增0-RTT模式
  * 在上一次连接中，握手完成之后，服务端会发送一条ServerConfiguration消息，在随后的客户端发起第一个TLS记录 ClientHello过程中，直接附加加密的应用程序数据(early data)。即发送数据是0RTT，但算上TCP握手总共还是1RTT（除非开启TFO）
  * 没有前向安全性，因为它是使用PSK中的密钥加密的。在多个连接之间可能受到重放攻击，普通的连接靠的是server的随机数防御的
* 会话恢复：弃用了Session ID和Ticket。使用pre_shared_key(PSK)，类似于Ticket但不用额外发送。都会受到重放攻击
* 密钥导出算法：使用新设计的HKDF算法。1.2是PRF算法
* 支持的加密套件：TLS13-AES128-GCM-SHA256、TLS13-CHACHA20-POLY1305-SHA256。查询CPU是否支持加速：grep aesni /proc/crypto
...
* 4xx客户端错误
  ...
  * 406 Not Acceptable 服务端不支持客户端发送的Accept: XXX，或客户端没有发送但应发送
...
* Connection：后面跟逐跳首部，比如Upgrade和Keep-Alive；close表示关闭Keep-Alive；HTTP1.1默认Keep-Alive了，其实1.1规定除非显式close否则都是Keep-Alive的。HTTP2及3不支持此头和Keep-Alive头（因为有另外的复用机制）。另外根据标准，在使用代理时根本不应使用keep-alive机制
...
* Alt-Svc(HTTP Alteative Service)：浏览器收到此头后会转而使用它指定的地址和端口，上层无感知，地址栏还是原来的。要求必须为HTTPS且新地址的证书支持原域名。用于服务端主动要求客户端切换连接，传统方法是HTML内容中使用多个域名
...
* Content-Disposition：attachment; filename=xxx.pdf 使得浏览器访问时触发下载而不是进行预览
...
* Cache-Control
  * 强缓存，若生效，浏览器不会发出请求，显示的响应代码是200而不是304
  * public 服务器发送，显式表示可以被所有缓存区缓存，如代理服务器，且允许POST缓存；private表示只能被用户浏览器缓存
  * no-cache 客户端用ctrl+f5时会发送，此时完全不会用本地缓存
  * no-store 服务器发送，表示连放在临时文件夹里暂存也不可以，用于机密信息
  * max-age=xx(秒) 服务器发送表示缓存多久。f5时客户端会发送=0，但仍会验证Etag/Last-Modified
  * immutable 表示f5也不重发请求，但只有FF支持
...
* Expires：强缓存，指定绝对时间，废弃了
* 弱缓存/协商缓存 Last-Modified和ETag：在请求时分别用If-Modified-Since和If-None-Match带上，如果没更新就不传输。同时存在时后者优先
* Last-Modified的问题：1.只能精确到秒，1秒内多次变化反映不出来。2.服务器的系统时间也许不可靠。3.文件系统的最后修改时间也许不可靠
* Pragma: no-cache 废弃了
...
* Strict-Transport-Security(HSTS)：max-age=31536000; includeSubDomains 时间设为0可使得网站立即允许HTTP访问。此处时间为1年。还有一个preload
* X-Requested-With: 传统AJAX会设置它为XMLHttpRequest，导致必为非简单请求

## Session、Cookie

* 技术和实现不一样。HTTP是无状态的，为了保持住状态，比如用户是否登录（点开新网页也是登录的），需要一种方法，就是Session技术。其实就是前端保存一个id，后端根据这个id来识别会话，存取数据
...
## 压力测试(stress test)/负载测试(load test)/带宽测试

* 类型
  * 负载测试：在预期并发下的表现。确定目标并发数：预期的峰值用户数/TPS/QPS、历史峰值数据
  * 压力测试：找到系统的极限或失效点。分阶段增加并发数，每阶段要持续一些时间，直到系统性能急剧下降或崩溃（拐点），之后再恢复低水平测一会儿
  * 尖峰测试：如秒杀场景，短时间内大量并发
  * 浸泡测试：稳定性测试，长时间测试以检查内存泄漏
  * 阈值：如非200小于1%，p99小于200ms
* QPS：每秒请求数，TPS每秒事务数。如一次“下单”是一个事务，可能涉及多个请求
* 并发量和吞吐量：前者是客户端发起的请求数，后者是服务端的实验数据。吞吐量随并发量增加而先上升后下降
* Coordinated Omission协同遗漏：简单来说，客户端不应该等待服务端返回后才开始下一次请求。假设服务端中间阻塞了10秒，具有此问题的测试工具只会看到有N个(=指定的线程数)请求超时，其余请求不超时。而实际上应该在10秒内继续发，这才符合现实世界中的请求情况，大量用户每个发少量请求（尖峰测试），而不是少量用户每个发大量请求；测试工具必须手动指定每秒发多少个，只指定线程数和每个线程要发的包数不行
* https://github.com/locustio/locust Py，需要写py脚本，有WebUI，测试和监控调用各个HTTP API的耗时等
* https://github.com/grafana/k6 Go，配合用户写的js脚本测试。也有云平台运行测试
* https://github.com/apache/jmeter JAVA，有GUI
* https://www.artillery.io/
* autocannon Node，wrk C不更新了。wrk2 -d10 -t2 -c100 -R200 -L https://www.liuquanhao.com/ | tee result.txt
* slowhttptest 模拟CC攻击
* iperf3
  * 服务端：-s [-u]
  * 客户端：-c <serverip> -P 5多线程 -b 100M -t 60
  * 不官方支持Win，有msys包 https://files.budman.pw/ https://github.com/userdocs/iperf3-static
* https://github.com/librespeed/speedtest PHP，服务端
* https://github.com/axboe/fio 测IO
* microsoft/ntttcp ntttcp-for-linux ctsTraffic(仅Win) 都是测带宽的。lagscope测延迟（TCPing）
* https://github.com/udhos/goben 不活跃
* 在线：https://loader.io/pricing https://www.blazemeter.com/pricing https://www.dotcom-tools.com/website-speed-test
* 强度测试：如网不好的环境

### tsenart/vegeta

* `echo "GET http://xxx/yyy" | vegeta attack -duration=5s -output=result.bin`
  * -rate 默认50/1s，除号前后都可调。-duration 默认30s，用0表示无限。二者相乘就是总发包数。-connections最大连接数默认10000大于rate用不到
  * -keepalive 默认true，好像有bug关不了
  * -timeout 默认30s，当在duration中发包完成后，会至多等待此值
* vegeta report result.bin、vegeta plot result.bin > result.html。也可以attack时不加output，直接|vegeta report
* 同时测多个目标、指定请求体、Header：看format，写指定格式的配置文件，用-targets指定。后两者也可用-body -header
* 对于客户端，可能需要调整ulimit -n和-u。相对于cpu来说，更可能出现内存瓶颈
* 自动增加负载测试、绘图脚本
  * 脚本，从每秒1次测到每秒100k：https://github.com/tsenart/vegeta/tree/master/scripts/load-ramping
    * 可以单独查看某一个报告，但不能合在一起plot
  * 安装绘图工具，仅Win：https://sourceforge.net/projects/gnuplot/files/gnuplot/
  * 实测：速率为16000/s时能全部200，P99延迟也正常。但设定为20000/s时各项指标均下降，显示发包速率为8921/s，P99为28s

### hatoo/oha

* 有tui、汇总报告
* 默认-c=50个线程（< ulimit -n），一共发-n=200个包。局域网内不到1秒就测完了
* 指定测试时间：-z 5s -w。与-n不能同用
* 贴近实际情况，避免CO问题：-z 5s -q 每秒总发包数 -w -t 30s --latency-correction --disable-keepalive

## 其它

* 多语言构建实时聊天应用的SDK：https://github.com/GetStream
