...
* 服务端需添加http头：`alt-svc 'quic=":443"; ma=2592000; v="39"'`
...
1. TCP三次握手。
2. 客户端发起请求Client Hello，与TCP第三次握手共用一个包。包含使用的TLS版本、客户端当前的时间和一个client random随机密码串、Session ID（没有则为0）、支持的加密组合方式、Server Name。
3. 服务端Server Hello：时间、随机数、Session ID、server random、服务器选中的加密方式。证书里面有用CA私钥加密的自己的RSA公钥。
4. 客户端用CA公钥解密证书，成功则随机生成一个AES密钥（会话密钥），用服务端的公钥加密发出。
5. 服务端用私钥解密，返回一个确认的包。
6. 之后一直使用客户端生成的那个密钥加密通信。算上TCP握手一共3-RTT。
...
9. 这个方案的问题在于，private key既用于证书身份识别(authentication），又用于premaster secret的解密。一旦private key泄露，那么所有使用这个SSL证书的密文premaster secret都能被解密。但根据CF的图，第四步发送的其实是一个premaster secret，之后再根据两个随机数生成密钥，我看不出哪里有问题。

### TLS 1.3

* Client Hello时还携带用于生成密钥的key_share和客户端DH参数
...
* 新增了0-RTT模式，即在上一次连接中，握手完成之后，服务端会发送一条 ServerConfiguration 消息，在随后的客户端发起第一个TLS记录 ClientHello 过程中，直接附加加密的应用程序数据；即发送数据是0RTT，但算上TCP握手总共还是1RTT
* 相比过去的的版本，会话恢复在服务端是无状态的，使用了新的pre_shared_key交换；即弃用了Session ID方式的会话复用，采用PSK机制的会话复用。TLS会话复用应使用tickets而不是caching，如果后者服务器被入侵，那所有客户端都会受影响
...
* Last-Modified 有两个问题：1.只能精确到秒，1秒内的多次变化反映不出来；2.在轮询的负载均衡算法中，如果各机器读到的文件修改时间不一致，有缓存无故失效和缓存不更新的风险
