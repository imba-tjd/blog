...
* Cache-Control: 服务器发送public显式表示可以被所有缓存区缓存，如代理服务器，且允许POST缓存，private只能被用户浏览器缓存。no-cache客户端发送表示不管本地缓存，重新进行请求，ctrl+f5时发送。no-store服务器发送，表示连放在临时文件夹里暂存也不可以，用于机密信息。max-age=xx(秒)，服务端发送表示缓存的时间，在此期间客户端发送请求不会发出去，直接用缓存；普通刷新时客户端会发送0表示忽略该项缓存，但仍会验证Etag/Last-Modified（如果有），在地址栏按回车不会触发。immutable表示即使普通刷新也不重发请求，但只有FF支持。若此指令成功读到缓存，会显示200而不是304。还有一些其它的看不懂的值，略。作为代理，出现private no-cache no-store时不能缓存，客户端也可能要求不读取缓存，以及min-fresh指定刷新时间，max-stale must-revalidate proxy-revalidate没必要考虑因为过期时重新请求一下即可
