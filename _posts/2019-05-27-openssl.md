...
* Win
  ...
  * https://wiki.overbyte.eu/wiki/index.php/ICS_Download#Download_OpenSSL_Binaries 基本只有crypto ssl exe三个，非常小
...
* shell命令
  ...
  * 手动输入：`echo -n`不自动换行；计算完后不止结果，还留了一点别的信息，去掉用`|awk '{print $1}'`
...
* .pfx/.p12(PKCS12)：二进制格式，一般包含一个私钥和与其相关的多个证书链，用于IIS，一般有密码保护（设计上必须有密码，如果输入空也是从空派生的）
* .jks：Java的，可以转成p12

## 创建自签证书(PKCS#10)

* https://certificatetools.com/ 可填写csr的字段，也能生成证书
* ssl-cert(debian apt包)：OpenSSL证书请求工具的简单封装
* https://github.com/FiloSottile/mkcert 零配置生成用于本地开发的证书，有效期3年
  * mkcert -install：自动生成$(mkcert -CAROOT)/rootCA-key.pem和rootCA.pem并安装到系统、Chrome（不支持Win下FF）、JAVA的证书储存中
    * 迁移：复制文件夹到另一台计算机上，设置$CAROOT，再-install
  * mkcert -ecdsa example.com "*.example.com" 根据系统中的ca签发证书。-client还生成客户端验证的
  * 证书中的信息含有计算机的用户名
* https://github.com/cloudflare/cfssl
* https://github.com/zmap/zlint 检查是否符合标准
...
使用已存在的密钥创建证书，跳过生成csr，仍需指定subj。时长如果太长浏览器会不信任：
...
```bash
openssl s_client [option] example.com:443 < /dev/null
```

* 必须指定端口，否则默认4433。曾经用-connect
* 不使用域名，指定ip和sni：-tls1_3 -servername "host.com" -connect 1.1.1.1:443
* 如果要指定加密套件，TLS1.2用-cipher，1.3用-ciphersuites
* `</dev/null`使建立完成后就结束，不加则会进入交互式界面等待输入而阻塞
* -status -tlsextdebug 显示OCSP Stapling的情况
* 重定向输出 `2>&1`
* -showcerts 显示证书链。-brief只显示一些可读的
* proxy host:port 发HTTP CONNECT
...
* ECDH是椭圆曲线的DH，ECDSA是椭圆曲线的DSA，ECIES是椭圆曲线的加密
...
* https://developers.google.com/tink 谷歌的密码学库，基于boringssl，以目标为导向，有多种语言的实现
...
* 浏览器对于ed25519证书的支持：https://security.stackexchange.com/questions/269725 测试：https://ed25519-test.germancoding.com
