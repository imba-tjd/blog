...
* log：单纯使用即开启访问日志。默认不记录敏感信息头、有一些限流
...
* log：默认有一个default的，会把运行时日志（但默认不包括访问日志）记录到console。级别INFO(默)/WARN/ERROR、输出到文件(默认rotated)
* strict_sni_host on
...
* 环境变量：{$ENV_VAR:DEFAULT_VAL}会在启用时一次性解析完毕。运行时：{env.XXX}，但外部插件需自行支持
* 下面的变量：{xxx}
...
* 对于host不匹配的http，会返回200和Server:Caddy，内容永远为空。host用0.0.0.0会导致始终返回空
