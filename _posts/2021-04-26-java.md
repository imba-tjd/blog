...
* 虚拟线程：Thread.startVirtualThread(runnable)创建并执行。Thread.ofVirtual().unstarted(runnable)创建但不执行。Thread.ofVirtual().factory(); fa.newThread(runnable)
...
* 常见命令
  ...
  * mvn clean compile 先清理再运行到compile，生成class文件到target目录。关键是不clean可能出错了也不报
  * mvn test 包含了compile
