...
map.get() / containsKey()
map.put(...) / putIfAbsent() / computeIfAbsent(用于计算V代价高如不必要的new) / merge(k, k不存在时的v, (k存在时的v,第二个参数)->v ) 均会添加进map且返回前一个V（如果之前不存在则返回null）
...
ConcurrentHashMap、ConcurrentSkipListMap 一般常用putIfAbsent，且推荐先get()检测
...
try(PreparedStatement stmt = con.prepareStatement("SELECT ... WHERE id=?")) { // 原生不支持命名参数
    ...
    if (i != null)
        stmt.setInt(2, i); // 如果2是可空的，这是设置它的唯一方法
    else
        stmt.setNull(2, Types.INTEGER);

    ResultSet rs = stmt.executeQuery() // 可以不try，stmt关闭时会自动关闭
    while(rs.next()) {
        int id = rs.getInt(1); // 索引不是从0开始。如果列值为null，会变为0；Boolean对应false
