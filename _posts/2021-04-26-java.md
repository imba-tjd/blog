...
* JAVA_HOME：当存在多个JDK时，设置此环境变量可用想要的版本。已知Java8的java.exe不会检测它
...
lst = List.of(1, 2, 3); // 只读，set运行时抛异常，想写可再传给ArrayList的构造函数；也能传Integer[]对象转List，不是包装，仍只读，但不能是int[]；copyOf接受其它容器；不要用Arrays.asList，是包装且非只读且不是普通的ArrayList；lst.subList()返回的是一个内部类的视图，且父列表增加删除后不能再用子视图
...
* 运行外部命令：Runtime.getRuntime().exec()
...
* 版本：[1.0,2.0)、[1,)，但这样指定会把所有大于1的版本都下下来；用mvn versions:resolve-ranges会替换为具体版本。LATEST和RELEASE仍能使用但宣布废弃了。versions:use-latest-releases和display-dependency-updates无法用于springboot因为会检查它的依赖。versions:update-parent可以更新springboot-starter-parent
...
<parent>
  ...
  <version>1.0</version>
  <relativePath>../parent/pom.xml</relativePath> 默认为../pom.xml。spring-starter-parent为<relativePath/>表示不找项目里上一层的，直接去找仓库里的
</parent>
...
* org.junit.jupiter:junit-jupiter:5.9，scope=test。或在dependencyManagement添加指定版本的org.junit:junit-bom，之后再用其它依赖就不用指定版本了
