...
* IDEA
  ...
  * 设置-编辑器-通用-自动导入-自动优化import
  * 设置-编辑器-通用-文件编码-Properties文件：改为U8，否则存在中文时会乱码，好像只有Java8及之前要改，好像每个项目都要改
  * 设置-编辑器-通用-Move code fragments with drag-and-drop 关掉
  * 帮助-更改内存选项：调整最大内存，默认2GB有点小
...
* 运行class：java -cp a.jar:pkg/*:bin com.Main，即指定main()所在的完整类名，不是指定.class文件
  ...
  * 运行选项必须在运行目标前，可用JAVA_TOOL_OPTIONS环境变量设置，对运行jar也有效
    * -ea启用assert语句。-XX:+ShowCodeDetailsInExceptionMessages空指针异常时显示更详细的信息
...
* 对象初始化器：在类定义里裸写一个{}，会在每次普通构造函数前调用，能访问实例成员，当多个构造函数具有相同逻辑又怕忘了手动调用复用时可用
  * 双大括号：其实就是匿名类加对象初始化器。但由于能访问new的obj的类里的对象，如果将obj作为返回值传出去，有可能导致资源无法被释放
...
* enum枚举：可用==比较两个对象，可定义方法，可在类中用`实例名(参数),实例2;`创建枚举常量对象
...
* 可变性
  ...
  * `<?>`其变量既不能读也不能写，只能做一些null判断。是所有`<T>`的超类，可理解为保留了外层类型的Object，能被其它`类型名<具体类型>`赋值。一般用泛型方法代替。`Class<?>`中用到
...
java.util.concurrent：CopyOnWriteArrayList、ConcurrentHashMap、CopyOnWriteArraySet、ArrayBlockingQueue、LinkedBlockingDeque
...
Files.readString(p.resolve("data.txt"))/readAllLines()/lines()/readAllBytes()，writeString()/write(p,lines/bytes)/newInputStream()
Files.list()/newDirectoryStream()/walk()
...
* java.util.stream.Stream
  ...
  * collect(Collectors.toList()/toSet()/toMap(kf,vf)/joining(","))
  * toArray(String[]::new)。直接toList()是不可变的
  ...
  * Arrays.stream(arr)返回IntStream，它toArray()能返回int[]；普通的装箱stream转它用mapToInt(Integer::valueOf)，它转装箱的用boxed()
  * takeWhile() dropWhile()
* `Optional<T>`
  * 原意是替代null，但不可序列化
  * 创建者返回空时用Optional.empty()；一定有值时返回Optional.of(xxx)，如果xxx为null会抛异常
  * 调用方用if(op.isPresent())判断是否有结果，get()获得结果。或orElseGet(fun)/orElseThrow()
...
* 第三方库
  ...
  * feign：在多种http客户端上的封装，受到了retrofit的启发，是Spring Cloud组件之一。但是现在不更新只维护了
...
* DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")
  ...
  * 一般情况下使用ofLocalizedDateTime(FormatStyle.MEDIUM) -> `2023年6月12日 19:33:30`
...
* java.util旧API
  * Date：getYear()要+1900，getMonth()要+1。内部实际上储存的是时间戳，但对外（包括序列化反序列化）默认以本地时区表示
    * java.sql.Timestamp：Date的子类，添加毫秒功能
  * SimpleDateFormat：线程不安全。new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"); sdf.format(date)。能在此处setTimeZone()
  * Calendar添加了一些对Date简单运算的方法，只能用Calendar.getInstance()创建，最后.getTime()
  * 旧转新：Date和Calendar用.toInstant()再变成ZDT，TimeZone用.toZoneId()
  * 新转旧：Timestamp.of(ldt)
...
* 并发工具
  ...
  * CyclicBarrier：达到指定数量的线程后才开始同一时间执行
  * Exchanger：类似于Go的阻塞Channel，不过是两者互相发送数据
* 虚拟线程
...
* 运行外部命令
  ...
  * new ProcessBuilder("cmd", "arg").start()
    * 获得STDIN用getOutputStream()，因为使用者要往里write
...
* -XX:+UseLargePages：大内存如16G时启用，还需设置/sys/kernel/mm/hugepages/hugepages-2048kB/nr_hugepages，值至少为16G/2M=8192
  * Linux下考虑改用-XX:+UseTransparentHugePages，还要设置echo "madvise" > /sys/kernel/mm/transparent_hugepage/enabled，相比之下某些时候能减少内存占用
* -XX:+AlwaysPreTouch：启动jvm时申请较多内存，能减少延迟卡顿
* 在GC时合并重复的字符串：-XX:+UseStringDeduplication JDK17及之前仅限G1，之后都支持
* -XX:+UseShenandoahGC：也是低延迟，适用于Web服务端，内存占用比G1还小。RedHat提供JDK8的
...
* 初始化：mvn archetype:generate -B -DgroupId=com.company.project -DartifactId=project
  ...
  * properties
    * 必须设置maven.compiler.source和target，用release无效，否则默认为1.5。一般先设置java.version再引用它
...
* dependency.scope：默认是compile表示编译时需要用到该jar包，还可以是test如junit、runtime如JDBC驱动因为自带接口只需要运行时用jar编译时不用、provided如servlet-api。optional：设为true表示依赖不传递，如A依赖Lombok，B依赖A，则B不会依赖Lombok
...
parent：一个单纯的parent/pom.xml，与其它模块平级
<groupId>略
...
org.slf4j:slf4j-api
ch.qos.logback:logback-core和logback-classic
...
public class FactorialTest {
    ...
    @Mock // 在setUp中设置MockitoAnnotations.openMocks(this); when(clock.getZone).thenReturn(...)，clock相当于动态对象
    Clock clock;
}
```
...
* 协议为带有FOSS例外的GPL，即项目可以使用被认可的开源协议，否则要么就购买商业许可
...
* JDBC URL
  ...
  * timezone=auto或UTC+8 与mysql的设为LOCAL且加另一个参数效果一样
