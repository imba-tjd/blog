...
* 第三方库
  ...
  * feign：在多种http客户端上的封装，受到了retrofit的启发，是Spring Cloud组件之一
...
* java.util旧API
  * Date：getYear()要+1900，getMonth()要+1。内部实际上储存的是时间戳
    * java.sql.Timestamp：Date的子类，添加毫秒功能
  * SimpleDateFormat：线程不安全。new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"); sdf.format(date)。默认使用本地时区，只能在此处setTimeZone(TimeZone.getTimeZone("Asia/Shanghai"))
...
* JDBC URL
  ...
  * timezone=auto 数据库和后端不在同一时区时推荐使用
