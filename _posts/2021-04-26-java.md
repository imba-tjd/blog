...
* 分析工具
  ...
  * jhsdb jmap --pid xxx --heap显示GC和堆的概况，--histo显示实例数量和大小，--binaryheap导出为hprof格式。此命令还有jstack jinfo指令，因为它可以处理coredump；clhsdb说是交互式debugger，进入后又有一些指令
  * jconsole：GUI工具
  * JMC：openjdk出的JFR工具
  * async-profiler：生成火焰图
  * VisualVM：Oracle出的，可能安装OracleJDK会自带
  * JMH：benchmark
