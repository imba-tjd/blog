...
* jpackage --input . --name 项目名 --app-version 1.0 --win-dir-chooser --win-console --win-shortcut --main-jar out.jar或--module-path . --module 模块名/完整类名：创建带有图形界面的安装程序，Win下还要装Wix
...
new File(虚拟路径); f.exists() f.isDirectory() f.mkdirs()
```
