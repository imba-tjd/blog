...
* 混合py和java：idea安装python插件（对应pro版，装了它不用装Pycharm Community插件）。项目设置 - SDK添加Python的。模块新增
...
* ThreadLocal
  ...
  * 使用前先在类中声明`static final ThreadLocal<String> ctx = new ThreadLocal<>()`，创建者try{ctx.set(o); step1(); step2()}finally{ctx.remove()}，使用者ctx.get()
    * 只有把TL在局部变量里new才会产生内存泄漏。上面这种使用方法即使不remove，要么线程结束会回收，要么进线程池下一次使用set时会覆盖。说泄漏的原理是kv实际上由线程维护，且k是ctx对象且为弱引用，当函数结束后ctx被回收，k变为null，就无法再取v了；而上面这种用法，ctx不会被回收，生命周期与Thread一样，实际上是强引用
