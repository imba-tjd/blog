...
map.put(...)/get()/containsKey()/putIfAbsent()/computeIfAbsent(用于计算V代价高如不必要的new) 其中put均会返回V
...
* 时间戳：System.currentTimeMillis()，类型为long的毫秒。另外long字面量要加L后缀。另有System.nanoTime更适用于计算间隔
