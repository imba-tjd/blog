...
* VSC
  ...
  * 自动删除未使用的import："editor.codeActionsOnSave": { "source.fixAll": true, "source.organizeImports": true }
...
* 可变性
  * 使用者声明在函数参数或变量的类型后，可直接用于类；不是放在类和接口的定义上。编译器根据此变量的方法中T所在位置限制允许使用的方法
...
* -XX:+UseZGC：延迟低；最好设置mx，一般最少16G，CPU负荷最好小于70%。借鉴了Azul的C4算法
