...
CopyOnWriteArrayList 无锁并发安全，但原理是每次插入时整个复制超级低效，适用于“事件订阅列表”
...
* volatile：可见性、避免指令重排
  ...
  * 指令重排：如f(){v=1; ok=true}可能先执行ok，对于g(){if(ok)使用v}来说，单线程下才没问题。给ok加volatile即可避免重排
