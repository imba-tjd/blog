...
* JDK
  ...
  * Liberica：https://bell-sw.com/pages/downloads/ 提供Lite版（通过jlink创建的）和带FX的Full版，32位的在下面但只有LTS有
...
int opt = switch (fruit) { // 表达式，有返回值
    case "apple" -> 1;
    case "pear", "mango" -> 2;
    case String s && !s.isEmpty() -> ... // 类型匹配，且null不会匹配进去。好像新版写法用when
...
* record Point(int x, int y) {}：定位是命名元组，不是用于封装数据的。自动生成构造函数、不可变成员、toString、equals、hashCode；如果要检查范围可写省略参数和括号的构造函数且不用自己给字段赋值。在模式匹配中可用类似于构造函数的语法解构成员并使用，且可用var
* 枚举：可用==比较两个对象，可定义方法
...
* 类型擦除
  ...
  * 不能new泛型类型数组，只能new非泛型版的，再在前面加 @SuppressWarnings("unchecked")，但`List<List<String>>`却又没事，不懂
* 可变性
...
* 数据库：jOOQ（性能超差，不如用mybatis-dynamic-sql）、JDBI（JDBC封装）、HikariCP、https://github.com/mybatis-mapper/mapper
* http://www.monkey1024.com/javaseroute
* 虚拟线程：创建和阻塞代价很低，不用池化，想限制数量时用信号量
* Semaphore s = new Semaphore(10); s.acquire(); try{run()}finally{s.release()}
* Lock l = new ReentrantLock(); l.lock(); try{run()}finally{l.unlock()}
