...
* 内部类
  ...
  * 可用于Lazy单例模式：静态内部类创建final static instance=C()，外面定义getInstance()返回inner.instance。非Lazy的单例一般用静态字段就好了
...
* enum枚举：可用==比较两个对象，可定义方法，可在类中用`实例名(参数),实例2;`创建枚举常量对象。E.values()获得所有实例，valueOf("实例名")或Enum.valueOf(clazz, "实例名")从字符串转换，e.name()与未重写时的toString()相同，ordinal()略
  * 可用于实现单例模式：就把class改成enum，成员加一个INSTANCE;就好了。而且自带序列化机制。但不能继承
...
@Override public boolean equals(Object o) {
    return o instanceof Person p &&
        Objects.equals(this.name, p.name) && this.age == p.age;
}
...
* 泛型约束：泛型定义中`<T extends R>`
  ...
  * 模拟self类型：抽象父类`B<T extends B<T>`，且具有`protected abstract T self()`。子类`D extends B<B>`且实现self()返回this。这样子类能继承父类中带有T的函数，且此时T就是子类这个具体类型。其中父类的泛型T的约束可理解为任何继承了父类的类，即子类
* 可变性
...
自定义类实现迭代器：implements Iterable<E>，实现Iterator<E> iterator(){return new Iterator<>(){ 匿名类实现hasNext()和next()，存放状态，访问外面类的元素 }}
