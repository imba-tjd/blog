...
lst = List.of(1, 2, 3); // 只读，set运行时抛异常，想写可再传给ArrayList的构造函数；也能传Integer[]对象转List，不是包装，仍只读，但不能是int[]；copyOf接受其它容器；不要用Arrays.asList，是包装且非只读且不是普通的ArrayList，除非是Java8；lst.subList()返回的是一个内部类的视图，且父列表增加删除后不能再用子视图
...
* java.util.stream.Stream
  ...
  * Stream.concat()连接两个stream。Guava的Streams支持变参
...
* 运行外部命令：Runtime.getRuntime().exec()、ProcessBuilder
