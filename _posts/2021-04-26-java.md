...
* 原子操作：java.util.concurrent.atomic：AtomicInteger
  * incrementAndGet()
...
* 常见命令
  ...
  * mvn clean compile 先清理再运行到compile，生成class文件到target目录。单纯compile只会编译未编译的文件
  * mvn test 包含了compile TODO: 好像这个会编译所有文件？
  * mvn package 打包生成jar到target目录 TODO: 是不是不会clean？
  * mvn install 把当前项目打包到本地仓库作为依赖供其它项目使用
* POM是可以继承的，默认就有一个SuperPOM。手动创建暂时不学。查看解析后的：mvn help:effective-pom
...
* 版本：[1.0,2.0)、[1,)。但这样指定会把所有大于1的版本都下下来。以-SNAPSHOT结尾的每次都会下载
* java里import的东西会在用到时才去找jar，而不会在编译期报错。可在Main里用Class.forName(xxx)缓解

## Gson

* com.google.code.gson:gson:2.10
* TODO：Jackson。@JsonIgnore、@JsonProperty(access = Access.WRITE_ONLY)、JsonIgnoreProperties(ignoreUnknown = true)
...
* 暂时不学的：jOOQ 性能超差，不如用mybatis-dynamic-sql
...
* connectionTimeout默认30秒，是等待从池里取出连接的时间。maximumPoolSize默认10
