...
class C implements Serializable {
    ...
    transient int size; // 标记为“瞬时”的不会被自动序列化，一般是根据其他成员计算出来的
    int i;
...
* 理论
  ...
  * 有的类对于static字段没做处理，这叫therad-hostile线程不利。因为内部没处理，别的自己无法控制的使用者也不会做处理。而普通对象自己可以处理
...
* volatile：可见性、避免指令重排
  ...
  * 指令重排：如f(){v=1; ok=true}可能先执行ok，对于g(){if(ok)使用v}来说，单线程下才没问题。给ok加volatile即可避免重排；若数据存在依赖关系不会重排
