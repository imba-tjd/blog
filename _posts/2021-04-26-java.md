...
* -XX:+UseEpsilonGC：不进行GC，要开-XX:+UnlockExperimentalVMOptions，一般还要加-Xlog:disable
* -XX:SoftMaxHeapSize=1G：尽量不要超过此值规定的最大堆大小，仅限ZGC
* -XX:+UseStringDeduplication 在GC时合并重复的字符串
* java -XX:+PrintFlagsFinal -version：查看所有的选项
