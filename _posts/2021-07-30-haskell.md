...
* data Season = Spring | Summer | Fall | Winter deriving (Eq, Show)，其中Spring称作0元构造函数，deriving (Eq, Show)自动生成允许==比较和Show转成自身对应的字符串
* 在ADT上的函数：isHot::Season -> Bool; isHot Winter = False; isHot _ = True
* 带参数的构造函数：data Shape = Circle Float | Rectangle Float Float，则Circle 1 :: Shape。此类型上的函数，用模式匹配取出参数：area::Shape->Float; area (Circle r) = pi * r^2; area (Rectangle a b) = a*b
* 某一构造函数的类型是data定义的类型本身的时候叫递归代数类型
* 手动定义Show的实例：instance Show Season where show Fall = "Autumn"。Eq：where Spring == Spring = True
* record类型
  * 构造函数的参数是有意义的名字：data Person = MkPerson { personName :: String, personAge :: Int } deriving (Show); p1 = MkPerson { personName = "Bob", personAge = 8 }
...
* 与接口和抽象类相似
* class MyClass a where f :: a -> String 可以有多态参数，之后定义多个instance MyClass Int/Bool where ...进行实现
* f2 :: (MyClass a, Show a) => a -> String; f2 = (show a) ++ (f a); f2 123
