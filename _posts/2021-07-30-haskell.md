...
* ghcup
  * 安装Haskell环境的工具
  * export BOOTSTRAP_HASKELL_NONINTERACTIVE=1; export BOOTSTRAP_HASKELL_MINIMAL=1; curl -sSf https://get-ghcup.haskell.org | sh; . ~/.ghcup/env; ghcup install ghc --set; ghcup install cabal
...
* Cabal：老牌的安装包的工具，现在的命令都应该用v2-xxx。不能用来安装ghc。在apt库中的名词是cabal-install
...
* seq a b：永远返回b，但保证先对a求值，类似于if a==0 then b else b；但如果结果未用到，不会对a求值；$!运算符类似于$但对右边立即求值
...
* https://www.reddit.com/r/haskell/ https://www.reddit.com/r/haskellquestions/
* 如何调试
