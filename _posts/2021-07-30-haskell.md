...
* GHC
  * https://downloads.haskell.org/ghc/latest/ Win版(mingw32)解压后3G，且没有重复文件，仅当本机已有llvm工具链可用软链接替换mingw32文件夹
    * 镜像：https://mirrors.cloud.tencent.com/stackage/ghc/
  * https://downloads.haskell.org/debian/ apt的安装指导，目前Debian内置源最新也才8
* Haskell Language Server(HLS)：https://downloads.haskell.org/~hls/ Win版大概360MB，下好后解压对应版本的以及wrapper到path里
* GHCup
  * 安装Haskell环境的工具
  * VSC的扩展需要它才能自动安装HLS，要不就手动下HLS
  * Linux无交互安装ghc和hls：curl -sSLf https://get-ghcup.haskell.org | BOOTSTRAP_HASKELL_NONINTERACTIVE=1 BOOTSTRAP_HASKELL_GHC_VERSION=latest BOOTSTRAP_HASKELL_INSTALL_NO_STACK=1 BOOTSTRAP_HASKELL_INSTALL_HLS=1 BOOTSTRAP_HASKELL_ADJUST_BASHRC=1 sh
    * 只安装GHCup本体：BOOTSTRAP_HASKELL_MINIMAL=1
    * 交大镜像：`mkdir -p ~/.ghcup && echo url-source: {OwnSource: https://mirror.sjtu.edu.cn/ghcup/yaml/ghcup/data/ghcup-0.0.8.yaml} > ~/.ghcup/config.yaml`，再将官方网址替换为 https://mirror.sjtu.edu.cn/ghcup/script/install.sh 。帮助文档：https://mirror.sjtu.edu.cn/docs/ghcup
    * 中科大镜像：https://mirrors.ustc.edu.cn/help/ghcup.html
  * 安装ghc并设置可执行命令：ghcup install ghc latest --set。但HLS不一定支持latest；不加latest则安装推荐版本
  * ghcup install cabal/stack/hls
  * 默认装在 ~/.ghcup 中，需要手动在bashrc中加入`. ~/.ghcup/env`，或安装时BOOTSTRAP_HASKELL_ADJUST_BASHRC；Win下是C:\ghcup\bin
...
* data Season = Spring | Summer | Fall | Winter，其中Spring称作零元(Nullary)构造函数；加上deriving (Eq, Show)自动生成允许==比较和Show转成自身对应的字符串的功能
...
* 函子(Functor)的实例支持fmap函数，能接受a->b，对“盒子”内部进行运算。任何实现了此功能的构造函数就是函子
  * []的fmap就是map
...
* 简单来理解，data kind就是polymorphism，Functor就是mappable，Monad就是automation
...
* 类型构造函数的“类型”称为“种(kind)”。Haskell里函数类型和kind都用箭头表示
* 不带参数类型的kind为`*`，如Int和[Int]和`Int -> Bool`，称为type
* 带一个参数的kind为`* -> *`，称为一元类型构造函数。如Maybe、IO、`(,) Int`、`(->) Int`、`[]`，它们接受一个类型参数，产生另一个类型。`(,)`的kind是`* -> * -> *`
  * 如果构建一个ADT，则一元类型构造函数自动成为函子。但对于函数类型，涉及逆变时，则不是函子
