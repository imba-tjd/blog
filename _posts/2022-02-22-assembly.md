...
* 栈的表示：可以想象一个杯子，地址从上往下增大，下面高地址是栈底，上面低地址是栈顶，增长方向从下往上，压栈后栈指针rsp会变小。另一种画法：倒过来的杯子
...
} // 返回：pop rbp; ret 因为没有更改rsp，此时栈顶的就是之前push的rbp，能直接pop回去。ret见上面，修改ip
...
* 另一款变速工具：https://github.com/game1024/OpenSpeedy/blob/master/docs/README_cn.md
...
* F7单步进入，F8下一行，F9一直运行（一般启动后就用一次），Ctrl+F9运行到返回（此时ax寄存器就是函数的返回值），Alt+F9运行到用户代码。双击十六进制列可加断点，双击内存地址列变为显示相对偏移。Ctrl+G转到内存地址或标签或符号
...
* 源代码调试和符号解析支持pdb文件，但不支持GCC的dwarf。有程序能转换但需VS环境。系统模块符号可右键下载
...
* 实操
  * 寻找MinGW下的main函数：先F9到用户代码的入口函数，进入它call的函数，按g，在下面的部分找一个传了三个参数的call的函数，就是main，给它添加标签(:)，里面的第一个函数调用是glibc的初始化
...
* 能反汇编，能看到局部变量、栈、寄存器。但不适合用作普通的反汇编工具，无法修改汇编，无法图形化查找字符串。一般用法是附加到出问题的进程上观察内存使用状况，以及处理异常
...
* 命令总结：https://bbs.pediy.com/thread-271490.htm
  ...
  * 寄存器r 进程信息| 反汇编u 反汇编整段函数uf 显示内存d 修改内存e 搜索内存s（-sa搜ASCII，-su搜Unicode） 内存信息!address
  ...
  * 列出句柄：!HANDLE
  * 添加自己程序的pdb：.sympath+c:/xxx/Debug .reload。不需要设置srcpath因为pdb里包含源代码路径信息（但在不同电脑上不行）
* 扩展插件
...
* https://github.com/uxmal/reko 有静态反编译能力。需.net6
...
* https://binary.ninja/demo/ 有网页版免费最大2M，桌面版有非商用免费的。静态反编译
