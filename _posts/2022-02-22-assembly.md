...
* 第一次运行是进行配置，都选否。之后选x64dbg。设置里调成中文，重启。再在设置里把TLS回调函数暂停关掉
* 主窗口是反汇编。右边是寄存器，RIP是下一条指令，双击可快速转到。右下角是栈，上面的是低地址
* F7单步进入，F8下一行，F9一直运行（一般启动后就用一次），Ctrl+F9运行到返回，Alt+F9运行到用户代码
* 右键-搜索-当前模块或所有模块-字符串，右键-在所有命令上切换断点（即给搜出来的都加断点）
* 右键-流程图(G)可查看当前一小块函数的代码，分析(A)可查看是由哪里跳转到本函数的
...
* https://github.com/uxmal/reko 有一定的静态反编译能力
* https://github.com/radareorg/radare2 只有CUI https://github.com/radareorg/iaito 是前者的GUI https://github.com/rizinorg/rizin 是radare2的Fork https://github.com/rizinorg/cutter 是rizin的GUI
