...
* 指令寄存器：rip/eip
* AT&T风格需在前面加%
* Linux下x64传递参数依次用：?di ?si ?dx ?cx r8? r9?，Win下用cx dx r8 r9。其中赋值要明确位数，但取值可能直接按rxx取
...
* Intel风格不在指令后加类型后缀
* 以#开头的是注释，以.开头的是伪指令
* 当读写内存时，几乎所有指令的操作数都应理解为地址，操作的是那个内存地址指向的值。
...
* lea S, D：`D <- &S`
* 也可理解为将S视为立即数而非取它的值。感觉等价于MOV D, [S]（Intel风格）
...
* 主窗口是反汇编，左边的地址是虚拟地址。右边是寄存器，RIP是下一条指令，双击可快速转到。中间是函数参数寄存器。右下角是栈，上面的是低地址
* F7单步进入，F8下一行，F9一直运行（一般启动后就用一次），Ctrl+F9运行到返回（此时ax寄存器就是函数的返回值），Alt+F9运行到用户代码。双击十六进制列可加断点，双击内存地址列变为显示相对地址
* 右键-搜索-当前模块或所有模块-字符串，右键-在所有命令上切换断点（即给搜出来的都加断点）
* 右键-分析模块(Ctrl+A)可标出函数，在工具栏上的函数按钮也可以用了。流程图(G)可查看当前函数的代码包括分支，可点进函数，右键转到上一个(-)从函数中返回。在函数入口点查找引用(Ctrl+R)可查看有哪些地方调用了本函数。显示指令提示可给出指令的功能
* 寻找MinGW下的main函数：先到用户代码的入口函数，进入它call的函数，按g，在下面的部分找一个传了三个参数的call的函数，就是main。给它添加标签(:)
* “符号”含有模块导入了的API，双击如MessageBox，下断点，停住后此时是系统代码无意义，用运行到用户代码
* 按空格修改当前行的汇编或右键-二进制-用nop填充。修改完后右键-补丁-修改文件，生成修改后的程序
* 主窗口(CPU)表示反汇编程序指令(.text)，下面的内存窗口用于数据部分，右键菜单下面的部分用于解读数据，如那一块是指针就选“地址”
...
* 反汇编资料：https://github.com/x64dbg/x64dbg/wiki/Frequently-Asked-Questions
...
* https://github.com/radareorg/radare2 只有CUI https://github.com/radareorg/iaito 是前者的GUI，运行需要ucrtbased.dll不自带 https://github.com/rizinorg/rizin 是radare2的Fork https://github.com/rizinorg/cutter 是rizin的GUI。实测cutter比较友好，有静态反编译能力、全局调用图
