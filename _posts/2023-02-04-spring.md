...
* 纯随机token + User微服务：一种做法是前端直接访问各个业务微服务，业务内部再根据用户token访问User微服务；缺点：Chatty，每个请求都要访问User微服务。另一种做法是Backend For Frontend(BFF)，门面模式，前端访问一个网关，网关从User微服务获得数据后再依次访问各个业务微服务
* JWT
  * 官方说读作jot
  * 将用户ID和权限信息放在客户端
  * 有一些固定字段，key都是三个字母
  * 必须验证所需的签名算法(alg)，标准允许用none
