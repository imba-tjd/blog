...
* 解决Cache Aside缓存和数据库不一致性
  ...
  * 高一致性方式：更新二者前加锁
* 常见问题
  * 缓存穿透：用户大量访问某一不存在的数据，缓存中没有就会去查DB。解决：也缓存空值+短过期时间，或将所有数据加载进布隆过滤器
...
* 默认是按类型注入的，不过不同包名但相同类名的Component默认生成一样的Bean名。可在ComponentScan时指定nameGenerator=实现了BeanNameGenerator的类，里面的函数的返回值就是bean的名称，一般检查definition.getBeanClassName().endsWith(重名类)，返回BeanClassName
* 按名称注入：@Qualifier("名称")。另一种方式：变量名设为Bean的名字
* 创建时加@Primary，则不指定名称时使用这个
