...
* Cache Aside
  * 写时：先更新数据库，再删除缓存。读时：如果读取的数据没有命中缓存，则读数据库、写缓存、返回给用户
...
* 解决缓存和数据库不一致性
  ...
  * 高一致性方式：更新二者前加锁
* 常见问题
  * 缓存穿透：如用户大量访问某一不存在的数据，缓存中没有就会去查DB。解决：也缓存空值+短过期时间，或将所有数据加载进布隆过滤器
  * 缓存击穿：某热点数据过期。解决：更新过期时间、预热
  * 缓存雪崩：大量数据集中过期。解决：加随机时间
