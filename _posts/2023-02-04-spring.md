...
* 消息传递语义
  * at-most-once：无需副本、生产者无需重试(或acks=0)、消费者先commitSync再处理消息（但感觉Async更好）
  * at-least-once：消息不会丢失，但可能被重复发送
    * 生产者：只在retries>0时才有意义（默认为0），当它>0且acks=all时会自动开启幂等性enable.idempotence，可以不产生重复记录，且重发的消息的的最终顺序保持了原本的顺序
    * 消费者和自动提交
...
log.retention.hours 默认168即7天后删除。但只有“关闭”(log.segment.bytes和log.roll.hours)的片段才会开始倒计时
...
for i in range(10):
  ...
  p.produce(topic, value, key, callback) # 支持命名参数。异步方法，会立即返回。如果队列缓存满了，会立即报BufferError，但默认大小足够，如果本地堆积了考虑程序有问题
p.flush()

with closing(c):
  c.subscribe([topics])
  while True:
    msg = consumer.poll(timeout=1.0) # 不能无参调用无限等待，因为poll时会发送心跳，如果poll的间隔超过max.poll.interval.ms就会被踢出组
