...
* 消息传递语义
  ...
  * at-least-once：消息不会丢失，但可能被重复发送
    * 生产者：默认retries>0且acks=all时会自动开启幂等性enable.idempotence，可以不产生重复记录，且重发的消息的的最终顺序保持了原本的顺序
    * 消费者和自动提交
