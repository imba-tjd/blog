...
* 读取配置：@Value("${key:默认值}") String s、#{beanName.prop SPEL表达式}
...
* @ConditionalOnBean（依赖创建Bean的顺序，只应在自动配置类上使用，一般可用OnClass代替）、MissingBean(name=bean名，value=bean.class)、SingleCandidate、MissingClass、Resource
