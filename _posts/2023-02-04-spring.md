...
* 使用：`RedisTemplate<T,R>`和便捷的StringRedisTemplate
  * 专门化某一类操作(operations views)：.opsForValue()，也可以直接注入ValueOperations
* @EnableRedisRepositories：实体类O加@RedisHash，成员有一个T id，则创建一个接口继承`CrudRepository<O, T>`，就可以使用了
...
Boot3.2 RestClient：Fluent API，同步，无需webflux。需手动创建，用create(baseurl)或builder()或自动配置了的RestClient.Builder
rc.get()
...
* 可见Eureka专注于服务发现，且优先AP。Zookeeper用于分布式协调与同步、元数据管理，与etcd是同类的，优先CP。Consul就是主要这两个功能，但也提供kv存储，优先CP
...
bin/kafka-topics.sh --create --topic quickstart-events --bootstrap-server localhost:9092 只要填任意一个broker即可 --config topic级别的k=v
...
listeners 一般意义上的监听。控制节点要写CONTROLLER
advertised.listeners 只有broker要设置，controller不用。是客户端访问的地址，客户端连接本节点后，会又被要求去访问本项配置中的地址，与客户端和节点之间的网络环境有关
只在内网访问：listeners设为内网ip，advertised.listeners不设置，会自动与listener一样
只在外网访问：listeners不改，advertised.listeners设为公网ip。如果访问在容器中运行的实例，则设为publish的ip和端口；k8s从集群内访问则设为服务名
...
生产者配置：
实时性和小包数量的trade-off：batch.size默认16KB，当消息满足此大小后会立即发出，否则再等linger.ms（默认0ms）后立即发出
...
Stream：@EnableKafkaStreams 自动配置StreamsBuilder
```
