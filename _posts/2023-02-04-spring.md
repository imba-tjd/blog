...
* 运行：mvn spring-boot:run。还有start非阻塞运行（但必须先构建），stop停止
  * win下classpath太长：-Dspring.spring-boot.run.fork=false
...
* 读取环境变量：${XXX:默认值}
* Profile：在本yml中加---，第一个配置是default环境，后面的每个都要指定spring.config.activate.on-profile.profiles: xxx；或创建application-dev.yml，会继承普通的配置。运行时-Dspring.profiles.active=xxx
...
* 设置找不到类型为T的Bean时不报错，而是用预设的默认值：@Autowired(required=false) T t = xxx

### @Bean

* 加在AppConfig里的方法上，方法名不重要，返回类型重要，表示用方法创建那种类型的Component
* 默认只会调用一次，是单例
...
* 可以注入的所有类型：https://docs.spring.io/spring-framework/reference/web/webmvc/mvc-controller/ann-methods/arguments.html
* matching-strategy现在默认为path-pattern-parser，支持{*p}表示0或多个，以及**只允许在结尾出现
...
* GCLIB中的final方法无法被代理。访问直接初始化的实例属性要通过getter否则是null，因为动态产生的子类没有执行父类的构造函数，而实例属性的初始化实际上就是在构造函数里执行的
* 有一个spring-boot-starter-aop，不清楚是否需要
* JoinPoint：程序运行中的某个点，在spring中仅为方法的执行

```java
@Aspect @Component
public class LoggingAspect {
    @Before("execution(public * com.company.service.UserService.*(..))") // 执行UserService的每个方法前执行，里面的东西称为Pointcut表达式，(..)表示任意参数。另一种使用方式是@Pointcut(表达式)放在本类的一个空函数f上，再写@Before("f()")需要的逻辑的函数
...
* @ControllerAdvice：一般配合@ExceptionHandler(value = Exception.class)全局处理异常，当Controller中发生指定异常时就会转而执行本注解指定的类中的方法
