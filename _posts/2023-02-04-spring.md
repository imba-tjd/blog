...
* Handler参数支持的所有类型：https://docs.spring.io/spring-framework/reference/web/webmvc/mvc-controller/ann-methods/arguments.html
  * Spring对于jakarta.servlet的封装：ServletServerHttpRequest
  * 读写body流可以用java.io.InputStream/OutputStream和Reader/Writer
* 返回406 Not Allowed：序列化或反序列化对象失败，包括没有合适的转换器、存在private成员
* 忽略XML类型：在createWebMvcConfigurer中重写configureContentNegotiation，configurer.ignoreAcceptHeader(true).defaultContentType(MediaType.APPLICATION_JSON);
...
* 加在类上，可指定要影响的包或类。实际上也是一种Controller，仅用于处理@ExceptionHandler @InitBinder @ModelAttribute
* 全局处理异常：@ExceptionHandler(value = Exception.class) void Handler(Exception e) 当普通Controller中发生指定异常时就会转而执行本方法，参数除了可以接收异常对象外，其它的应该和普通Controller一样
...
restTemplate.exchange(使用HttpEntity)
```
