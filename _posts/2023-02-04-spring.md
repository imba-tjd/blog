...
* 避免版本号：build finalName ${artifactId}
...
* 不使用starter-parent
  ...
  * spring-boot-maven-plugin添加execution goal=repackage。或手动运行spring-boot:repackage
...
* Handler参数支持的所有类型：https://docs.spring.io/spring-framework/reference/web/webmvc/mvc-controller/ann-methods/arguments.html
  ...
  * MultipartFile file.stransferTo(Path / 自己new的File对象) 对应前端input type="file" name="file"
...
* spring.session.store-type=若只有一个spring-session则无需配置; spring.session.redis.xxx
