...
* 消息队列：解耦（发送者和接收者不必了解对方）、异步（发送者无需等待消息被处理）。流量削峰、(生产者)消息重发、(消费者)重试消费、死信队列
...
* partition分区：分布式的关键。一个topic划分为多个分区分布在多个broker上，不同的分区（对于一个消费者组来说）可以并行消费
  * 生产者能决定将消息放到哪个分区上，一般如果有key就根据hash(key)%分区数，如果没有key则轮流放
  * 一个分区里的消息是有序的。当分区数不变时，key相同，则默认一定放到相同的分区里
...
* at-least-once（默认）：消息不会丢失，但可能被重复发送（只在retries>0时才有意义），默认已开启幂等性enable.idempotence（还要求必须acks=all）可以不产生重复记录。但对于消费者是poll()时默认每5秒提交的，如果poll后commit了，但消息没处理就挂了，就丢了（其实是at-most-once，但如果是同步处理且poll提交的是前一次的offset就没问题，不知道是不是）；如果没到5秒且处理了一部分，则挂了恢复后会重复处理前一部分。一种解决办法是手动处理完后commitAsync且在关闭前commitSync；或用spring，无需调整autocommit。如果消费者会输出到外部系统，要自己实现幂等性，一般如果消息里有主键，覆盖是没问题的，又称effectively-once
...
* win版可以用bin/windows下的bat。会在程序所在磁盘的根目录下创建/tmp文件夹

```bash
bin/kafka-storage.sh format -t $(bin/kafka-storage.sh random-uuid) -c config/kraft/server.properties

bin/kafka-server-start.sh 可选--daemon config/kraft/server.properties --overwrite 配置k=v

bin/kafka-topics.sh --create --topic quickstart-events --bootstrap-server localhost:9092 只要填任意一个broker即可
查看：--describe --topic quickstart-events、--list、删除

服务器配置：
通过环境变量配置：如KAFKA_ADVERTISED_LISTENERS
node.id 每个实例都必须不一样
...
compression.type=lz4或zstd  服务器也有同名配置，且默认值为producer字面量，表示保留生产者的配置。一般应有生产者压缩，否则无法节省网络流量。应选择适量的buffer否则压缩效果不好。不适合实时数据、加密后的高熵数据，适合JSON XML
acks=0/1/all 默认all表示等follower同步完再返回，1表示leader完成即可
retries 默认是0

消费者：
推荐设置client.id
offset根据group.id保存
auto.offset.reset 没有offset时从哪读，默认latest
```
...
消费者：@Component 中 @KafkaListener(topics = "someTopic") void processMessage(String content, ConsumerRecordMetadata meta) {}
