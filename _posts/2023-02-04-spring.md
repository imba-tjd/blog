...
* 购买后增加积分的场景：先创建唯一id和待提交的积分记录，开启本地事务，做购买业务，修改积分记录，提交事务。（另一线程开定时器）检查积分记录是否成功，如果成功则投递到MQ。若在事务中直接投递MQ，一是若事务回滚则积分还是会增加，或者如果MQ投递失败会使得主业务回滚；二是访问MQ有网络开销，影响主业务延迟

### 微服务用户架构

* 单体程序：一般用session。存在副本时：前一层反代。缺点：若某一用户信息只存在于某一实例内存中，则有session pinning问题；若让所有实例都存所有用户信息(session replication)，则有浪费内存、同步延迟问题
* 纯随机token + User微服务：一种做法是前端直接访问各个业务微服务，业务内部再根据用户token访问User微服务；缺点：Chatty，每个请求都要访问User微服务。另一种做法是门面模式，前端访问一个网关，网关从User微服务获得数据后再依次访问各个业务微服务
* JWT
  * 将用户ID和权限信息放在客户端
  * 有一些固定字段，key都是三个字母
  * 必须验证所需的签名算法，标准允许用none
...
* 隐含了@Component。区别：可以被bootstrap，会处理内部的@Bean
...
rowMapper：(resultSet, rowNum)->{处理一行结果返回对象} 或 BeanPropertyRowMapper.newInstance(Pojo类.class)
...
* 好像不支持join
* 参数：如果只有一个参数，则可以任意引用。如果是Map，则模板里可用#{key}获得val。如果是Pojo，可用#{属性名}。如果有多个参数，则一般需要@Param，因为默认编译后局部变量名变为了arg0
...
颜色：spring.output.ansi.enabled=默认detect 估计是在Linux上启用Win上不启用。可改为always。日志输出到文件时不能开
```
