...
* IoC：原本表示一种设计原则(Principle)，具有多种实现方式：DI、事件驱动、模板方法。由IoC容器负责创建组件、根据依赖关系组装组件、按依赖顺序正确销毁（生命周期）。程序只需使用，也容易Mock。另一种理论：包括DI（Push模式）和依赖查找（Pull模式），Java生态圈早期用后者，基本上就是一个`Map<服务名, 对象>`，使用者 getService("name")；后来完败于DI。SpringFramework是IoC的一种实现，其他还有JNDI、EJB等
...
* Controller只做HTTP、cookie相关的事，包括参数接收、校验、返回VO和状态码；只调用一个Application Service，后者再负责调用多个Service组装业务逻辑
...
* 打包成Docker映像：mvn -Pnative spring-boot:build-image。单纯用GraalVM编译，不支持交叉编译：native:compile。https://b23.tv/BV1YSDGYREVU
...
* 右边添加依赖选Spring Web(spring-boot-starter-web)，提供MVC和REST支持。boot4改名webmvc
* 其它创建方式：Spring Boot CLI、curl
* IDEA插件：Spring Debugger（JB官方出的，需Ultimate）
...
### [application.yml](https://docs.spring.io/spring-boot/appendix/application-properties/)
...
* Profile
  ...
  * 任意路径的配置文件：spring.config.import=optional:file:./myconfig.properties
...
server:
  ...
  shutdown: graceful # 【默】Ctrl+C或kill -2会等待已连接的结束。超时指定spring.lifecycle.timeout-per-shutdown-phase
  compression:
...
spring:
  application.name: myapp # cloud一般会用到，如果不设置将显示UNKNOWN
  main:
    lazy-initialization: true # 垃圾框架一般不会考虑而不兼容
    web-application-type: none # 设置后不启动web服务，用于测试ApplicationRunner
  jackson:
  ...
  threads.virtual.enabled: true # 需Java21，Boot3.2。对于非web的持续运行项目，还要spring.main.keep-alive=true。boot4对于jdk21默认启用
```
...
* 如配置文件、命令行等，每一个kv来源称为PropertySource，使用时会一次性加载到各自的内部Map里缓存
* 多个PS按顺序组成了Environment。在运行时添加（addFirst）、修改（replace）PS，就更新了Environment
* Env到@ConfigurationProperties和@Value还有一次映射
  * @Value需spring-cloud-context，在使用它的类上加@RefreshScope；ConfigurationProperties默认就有此功能
  * 修改Env后，引入actuator调用refresh端点。手动实现它的逻辑：注入ContextRefresher调用refresh()。另一种做法：注入RefreshScope调用refreshAll()
  * 对于CP，也可用eventPublisher.publishEvent(new EnvironmentChangeEvent(this, changedKeys))
* 一般而言动态配置需要一个ConfigServer，会自动轮询或推送修改Env，否则不如重启进程
* 如果确实想监视文件改变，一般另用一个文件，用FileWatcher监视改变或定时任务检查，解析进PS
...
* 默认是单例的，加@Scope("prototype")则每次获取的都是新的。当一般意义上new的对象要用到spring的bean时则会声明为“原型”作用域，或用于解决并发问题
  * 单例Bean中使用非单例Bean
    * 方法一：写一个返回非单例Bean类型的返回null的方法，上面加@Lookup，会自动重写内容，参数自动传递给构造函数
    * 方法二：注入ApplicationContext，getBean
    * 如果直接用@Autowired非单例成员，则因为单例只存在一个，导致非单例也只有一个；但这也是一种合理的用法，当想要的效果是多个不同的单例bean各获得其自己的非单例bean时就这么用
* WebApp才有的作用域：request（一次HTTP请求。另有WebSocket）、session、application（一般不用，只当存在多个DispatcherServlet时才与普通单例有区别）
  * 在单例Bean中使用：@Scope的proxyMode=ScopedProxyMode.TARGET_CLASS
* 初始化周围做动作
  * 实现BeanPostProcessor，重写Before和After，一般配合instanceof转为具体类型，设置其属性
  * PostConstruct是在每个Component里都有自己的；这个只有全局一个，所有的Bean初始化时都会调用它
  * Before的返回值是bean对象，如果返回null，则后续的BeanPostProcessor就不会被调用了
* 实例化周围做动作
  * 实现InstantiationAwareBeanPostProcessor（继承了BeanPostProcessor）
  * Before的参数有一个Class对象，因为此时Bean还没创建，一般返回null表示让Spring来创建。若返回非null，则后续生命周期会少多个步骤，比如不会注入Bean中的属性、自动调用init()和BeanPostProcessor的两个方法
  * After若返回false，则不会注入属性
* 操纵BeanDefinition（代表创建Bean的需求）
  * 实现BeanDefinitionRegistryPostProcessor（继承了BeanFactoryPostProcessor），生命周期在BeanPostProcessor之前
  * Registry可以getBeanDefinition(bean名)，设置是否是primary等；可以添加registerBeanDefinition(自定义bean名, def)，可以remove
  * 本来bean名确定后就不能改了，可以先get，再remove，在register解决

### @Component

* 不必是public的
* 初始化和销毁时做动作：@PostConstruct init()，@PreDestroy destroy()，函数名不重要，前者会在本类Autowired完成后执行
* 默认名称为类首字母小写，可用@Qualifer()修改
* 接口不必加，只在实现类上加，之后注入接口能自动找到

### @Autowired

* 在所需要的另一个Service或接口的字段上加@Autowired。或推荐在构造函数里注入，后备字段用final；若只有一个构造函数则什么都不用加，有多个则选一个加。或普通的加了加@Autowired的方法，一般叫setXxx
* 在整个程序启动时就会执行。无需public
* 设置找不到类型为T的Bean时不报错，而是用预设的默认值：@Autowired(required=false) T t = xxx。或用@Nullable或Optional
* 默认按类型注入。若一个类型存在多个Bean，依次：
  * @Qualifier("想要的Bean的名称")
  * @Primary 一个Bean创建的时候加了
  * 变量名等于Bean的名称
  * 报错
* 一个接口可以有多种实现，@Autowired可以加在List<接口>或Map<String代表beanName,接口>字段上，会添加所有实现
* 解决不同包名但相同类名的Component默认生成一样的Bean名：在ComponentScan时指定nameGenerator=实现了BeanNameGenerator的类，里面的函数的返回值就是bean的名称，一般检查definition.getBeanClassName().endsWith(重名类)，返回BeanClassName
* @Nullable：boot4使用jspecify，是与google和jb联合制定的

### @Bean

* 加在Configuration里的方法上，方法名就是Bean的名称，返回类型表示用本方法创建那种类型的Bean，参数相当于注入
* 当需要手动创建Bean且不需要代理里面的方法时使用
* destroyMethod="close"
* 保留自动配置的，定义同类型的：defaultCandidate=false。需boot3.4
* FactoryBean：比@Bean更抽象的用来创建指定Bean类型的接口。xxxFactoryBean实现`FactoryBean<T>`，重写getObject()。注入对应类型时会自动先实例化工厂再调用getObject()
...
* @Import(另一个Bean.class)：定义在Config类上，作用与@ComponentScan一样。一般用于加载非自己的包又不想调ComponentScan或想精确控制顺序

### ApplicationContext

* 就是IoC容器，注入（不必用ApplicationContextAware）后可用getBean(MyService.class)或getBean("name")获得实例，registerBean创建Bean
* 创建Ctx：new AnnotationConfigApplicationContext(Configuration类.class)
* BeanFactory：是ApplicationContext的父接口，一般不用。是简单工厂模式

### @SpringBootApplication

* 相当于加了@Configuration @ComponentScan，会自动扫描装配它所在的包及子包的Component
...
* Boot4的BeanRegistrar：用普通编程方式而不是注解创建bean，当条件注解太复杂而无法表达时使用

#### 自制starter

* https://docs.spring.io/spring-boot/reference/features/developing-auto-configuration.html
* my-starter：引入spring-boot-starter、my-autoconfigure
* my-autoconfigure：引入optional的spring-boot-starter、spring-boot-autoconfigure-processor（TODO:与spring-boot-autoconfigure的区别），可选configuration-processor。引入其他的my2-autoconfigure
  * src里写my.autoconfigure.XxxAutoConfiguration.java 用条件装配和配置映射
  * META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports 文件里一行一个自己的Configure类的全限定名
  * 老版：META-INF/spring.factories里写org.springframework.boot.autoconfigure.EnableAutoConfigurtion=\换行 自己的Configure类

## Spring Web

* @RequestParam
  * 用于URL的查询参数。可选参数：name="xxx"用于路由参数名与Handler参数名不一致、defaultValue="xxx"或required=true（默认就可选且无默认值）
  * 也可用于表单数据：Mapping加consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE，handler参数用`MultiValueMap<String, String>`
...
* 静态资源
  * 自动支持resources/static等目录里的静态文件（不需要写Controller，写了优先），一般还要设置前缀路由spring.mvc.static-path-pattern=/static/**
...
* 模板引擎：Thymeleaf。文件放在resources/templates
...
public class UserController {
    @GetMapping("/register")
    public String register(Model m) { m.addAttribute(k,v); return "redirect:/signin"; } // 返回的String是指定视图名而不是真的字符串，Model是个引用类型
    @PostMapping("/signin")
    public ModelAndView signin() { var mv = new ModelAndView("signin.html"); mv.addObject(k,v); return mv; }
}

// 返回普通对象要加此注解。否则String会被解析为ViewName传给模板引擎处理
...
* Initializr依赖里添加Validation
* import jakarta.validation.constraints.*;
* Controller类（还支持Handler的参数、返回值）上加@Validated，是spring提供的。另一种方式：@Valid，是jakarta提供的。@Validated支持验证组、可自动级联
* Handler参数或对象的字段上加
...
* Handler参数的BindingResult result .hasErrors() .getFieldErrors() 存放出错的结果。不写时会抛异常，之后全局处理
* 异常：方法级别 ConstraintViolationException，Controller层 MethodArgumentNotValidException

### Session

* HttpSession
  * 可以在Handler参数里注入，会自动创建。setAttribte(k,v)
  * 手动控制：HttpServletRequest req.getSession() 无参时与(true)相同，当不存在时会创建；改用false则当不存在会话时返回null。如果想当存在时才取值，可用WebUtil.getSessionAttribute()
  * req.changeSessionId()、s.invalidate()
* 服务端配置
  * 默认是Tomcat的
  * server.servlet.session.persistent默认false，重启服务端后会话就消失了。设为true后会在TMP/tomcat.端口.xxx/下放东西
  * server.servlet.session.timeout=秒 默认30m且默认只精确到分。无需设定spring.session.timeout。boot2不认
* Cookie
  * 配置项：server.servlet.session.cookie
...
* 引入spring-boot-starter-aop，无需@EnableAspectJAutoProxy，默认全使用CGLIB代理(proxyTargetClass=true)
* 功能类似于装饰器，但可以写规则自动应用到所有的方法上
* spring-boot-starter-aop引入了aspectjweaver，但只是复用了它的注解、用它来解析切入点表达式。实际上仍只是SpringAOP，JoinPoint仅支持方法；而完整的aspectj支持字段、getter等，需要编译或加载时增加agent
...
* Advice通知的类型：Before、After（相当于Finally）、AfterReturning目标代码正常返回时才执行拦截器、AfterThrowing、Around目标代码执行前后都执行。Advice概念本身指在某一JoinPoint上执行的代码逻辑
...
Object doLogging(ProceedingJoinPoint pjp) throws Throwable {
    ...
    Object retVal = pjp.proceed(); // 返回值可强转成实际类型。参数可以传自己的Object[]，如果为空则自动传递原来的
...
public void logException(Throwable ex) { // 可缩小具体类
    System.out.println(ex);  // 不能catch
}

切入点表达式：
execution(public * com.demo.MySvc.*(..))
  public可选
  第一个*是任意返回类型，必填
  包名部分可选，用c..表示c包及其子包
  最后一个*匹配方法名，必填；按顺序前一个部分若有则必匹配类名
  (..)表示任意参数，(*)表示一个参数
within(类名)：匹配类中所有方法。结尾加+也匹配子类。不会处理运行时
  相似的还有target(类名)，如当子类没有重写父类的某方法时，若要匹配此方法，within(父类)，或target(子类)。AspectJ中within还能匹配更多切入点。总的来说感觉不用target
@within、@annotation(自定义注解完全限定名)：分别匹配应用了此注解的类(里的所有方法)、方法。需要Bean主动加注解，防止误伤
  binding form：表达式用@annotation(a)，Handler参数用A a，会自动匹配A类型和绑定到a变量里
  传统方法：(MethodSignature)jp.getSignature(); sig.getMethod(); m.getAnnotation(A.class)
...
```java
@Bean DefaultAdvisorAutoProxyCreator advisorAutoProxyCreator() {
    var creator = new DefaultAdvisorAutoProxyCreator();
    creator.setProxyTargetClass(true);
    return creator;
}

class LogInterceptor implements MethodInterceptor { // 其他接口：MethodBeforeAdvice、AfterReturningAdvice、ThrowsAdvice
    @Override public Object invoke(MethodInvocation invocation) throws Throwable {
        System.out.println("【前置】调用方法: " + invocation.getMethod().getName());
        try {
            Object result = invocation.proceed();
            System.out.println("【后置】方法执行成功");
            return result;
        } catch (Exception e) {
...
@Bean Advisor saveUserAdvisor() { // Advisor = Pointcut + Advice
    var pointcut = new NameMatchMethodPointcut(); // 还有JdkRegexpMethodPointcut、AnnotationMatchingPointcut、ControlFlowPointcut（当指定类在调用栈中且符合命名规则时匹配，速度是普通的10倍慢）
    pointcut.addMethodName("saveUser"); // 可多次调用；还支持几种简单的*通配符：前缀、后缀、中间、两边
    return new DefaultPointcutAdvisor(pointcut, new LogInterceptor());
    // 预设了特定类型的PointCut的Advisor方便使用：NameMatchMethodPointcutAdvisor、RegexpMethodPointcutAdvisor。new出来后提供了内置PointCut相同的方法
}

预定义的拦截器：
SimpleTraceInterceptor：会输出方法名、参数、用时
PerformanceMonitorInterceptor：能设定一个阈值，超时会提升等级为WARN。还能输出耗时统计
DebugInterceptor：会输出返回值
CountingTraceInterceptor：统计方法被调用了多少次
```

## Async

* AppConfig上加@EnableAsync，要异步的方法上加@Async("可选执行器名")。返回值可以是void或CompletableFuture
...
@ConfigurationProperties("cus_setting.datasource")
...
* 可以加在具体Service类或public方法上。也可以加在接口上但一般不推荐
...
* 事务传播
  ...
  * 嵌套事务：REQUIRES_NEW，如果里层出错时只想回滚里层时使用
* 失效情形
  * AOP失效：如非public方法、final/static方法、类方法内部调用
  * 事务信息与线程绑定（ThreadLocal），如果新开一个线程执行事务则不会加入
...
* 创建接口继承 CrudRepository / ListCrudRepository(Boot3)、PagingAndSortingRepository。若需要，可同时继承
* 自带save(T) findById() findAll() count() delete(T) existsById()
* 也可以不用Repository，用JdbcAggregateTemplate
...
public class DataJdbcConfiguration extends AbstractJdbcConfiguration {
    @Bean public BeforeSaveCallback<AbstractEntity> absEntityBeforeSet() {
...
* org.mybatis.spring.boot:mybatis-spring-boot-starter 3对应Boot3
  ...
  * 自动扫描所有@Mapper并注册，不必用@MapperScan；除非Mapper不在@SpringbootApplication包下。或者说用了@MapperScan就相当于对那个指定包下的所有接口用了@Mapper
...
* 对于boot3.4，不再管理OkHttp的版本
* 默认跟随跳转。禁用：spring.http.client.redirects=dont-follow
...
var req = RequestEntity.post("{id}", 123).accept(...).header(k,v).body(o).build();
...
* structured logging (boot3.4)：输出为机器易读的json格式，有三种可选
...
* ObjectUtils：addObjectToArray给数组添加元素。isEmpty对多种类型判空（StringUtils的设计有问题废弃了）
* FileCopyUtils：处理并自动关闭IO流，包括复制、写入文件、读取为字符串
* ReflectionTestUtils：方便地根据名字使用private的字段和方法
* ClassUtils：getAllInterfaces能获得接口的父类在内的所有接口
* 缓存 Spring cache
...
* LockRegistry：分布式锁
* Spring Boot DevTools(spring-boot-devtools, scope=runtime, optional=true)：通过重载ClassLoader加载用户代码，而保持库代码不变。若有语法错误可能不会报错或编译失败。在java -jar运行时自动禁用
...
* 暴露更多功能：management.endpoints.web.exposure.include=xxx,xxx或`*`、management.endpoint.shutdown.enabled=true、management.endpoint.env.show-values=ALWAYS
...
* 架构知识：https://learn.microsoft.com/zh-cn/azure/architecture/ https://learn.microsoft.com/zh-cn/azure/architecture/guide/architecture-styles/
