...
* 读取环境变量：${XXX:默认值}
* Profile
  * 在本yml中加---，第一个配置是default环境，后面的每个都要指定spring.config.activate.on-profile.profiles: xxx
  * 或创建application-dev.yml，会继承普通的配置
  * 运行时-Dspring.profiles.active=xxx
...
* 实际上是“重型”@Bean
* 初始化和销毁时做动作：@PostConstruct init()，@PreDestroy shutdown()，函数名不重要
...
* 相当于加了@Configuration @ComponentScan，会自动扫描和装配它所在的包及子包的Bean
...
* 支持返回Record，无需也不能加@Component，但默认不改变字段大小写
...
* @Order(1) @Component xxxInterceptor实现HandlerInterceptor接口
...
* 创建后还要添加才能启动：createWebMvcConfigurer(@Autowired HandlerInterceptor[] interceptors)中void addInterceptors(InterceptorRegistry registry){registry.addInterceptor(interceptor)}
* @ControllerAdvice：一般配合@ExceptionHandler(value = Exception.class)全局处理异常，当Controller中发生指定异常时就会转而执行本注解指定的类中的方法
* 与Aspect相比，拦截器的功能可理解为是固定的，如preHandle的参数是固定的HttpServletRequest。Aspect是对特定方法的增强
...
resources/mapper/UserMapper.xml。或放在resources/com.demo.mapper下，则不需要配置mapper-locations
