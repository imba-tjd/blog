...
* 单体程序：一般用session。存在副本时：前一层反代。缺点：若某一用户信息只存在于某一实例内存中，则有session pinning问题；若让所有实例都存所有用户信息，则有浪费内存、同步延迟问题
...
* 运行：mvn spring-boot:run。还有start非阻塞运行（但必须先构建），stop停止
  ...
  * 多模块项目在根下运行：-pl app。必须先compile，用-am无效。只给需要创建可执行jar包的模块加spring-boot-maven-plugin。根pom可以继承自springparent，子pom继承根；也可以根只用module指令，子pom继承自springparent
...
* BeanFactory：是ApplicationContext的父接口，一般不用
