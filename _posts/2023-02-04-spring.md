...
* 专门化某一类操作(operations views)：.opsForValue()，也可以直接注入ValueOperations，但HashOperations就不能直接注入了
* @EnableRedisRepositories：实体类O加@RedisHash，成员有一个T id，则创建一个接口继承`CrudRepository<O, T>`加@Repository
* HashOperation：基本没用。如果用`<String,byte[],byte[]`，那还不如用普通的Value版，如果用`<String,String,String>`，可以手动放Map，但不能直接放对象，因为即使用Jackson序列化出来也是`<String,Object>`而不是`<String,String>`，除非再去改RT的hash的序列化方法
* Stream
  * 写入的时候仍然是Map，不过因为不再追求kv是纯文本，这次可以序列化成二进制了。用StreamRecords.objectBacked(o).withStreamKey(key)从对象创建记录发送
  * 手动读取时第一个参数用clazz。创建选项用StreamReadOptions.empty()。Consumer.from()表示消费者组，处理后要用rt.acknowledge(key,msg)。offset在用了消费者组后可用StreamOffset.create(key, ReadOffset.lastConsumed())，会第一次用最后一条记录，之后自动维护；否则要手动维持，第一次用StreamOffset.latest(key)之后用from(record)
  * Listener模式，适合表示事件：StreamMessageListenerContainer.create(connectionFactory, StreamMessageListenerContainerOptions.builder().targetType(clazz).build()); container.receive(offset, msg->{处理}); container.start() 对于消费者组用receiveAutoAck
...
* CommandLineRunner：会在项目启动后执行，不会阻塞ServletDispatcher。是接口，在Configuration里写一个@Bean的函数return new args -> ...。也可用@Componet里的方法上加@EventListener(ApplicationReadyEvent.class)
...
* 消息队列：解耦（发送者和接收者不必了解对方）、异步（发送者无需等待消息被处理）。流量削峰、(生产者)消息重发、(消费者)重试消费、死信队列、优先级
...
* partition分区：分布式的关键。一个topic划分为多个分区分布在多个broker上，不同的分区（对于一个消费者组来说）可以并行消费，只有一个分区则此topic不能并行消费
  * 生产者能决定将消息放到哪个分区上，一般如果有key就根据hash(key)%分区数，如果没有key则轮流放
...
* replication副本：消息只会在leader中读写，多个following只复制。当leader挂了后会从follower中重新选举
* 消息传递语义
  * at-most-once：无需副本、生产者无需重试(或acks=0)、消费者先commit offset(enable.auto.commit=false且commitSync)再处理消息
  * at-least-once：消息不会丢失，但可能被重复发送。只在retries>0时才有意义（默认为0），默认在>0且acks=all时会自动开启幂等性enable.idempotence，可以不产生重复记录，且重发的消息的的最终顺序保持了原本的顺序
    * 但对于消费者，默认自动提交是poll()时每5秒提交offset，如果poll后commit了，但消息没处理就挂了，就丢了，此时其实是at-most-once。一种解决办法是关闭自动提交，处理完一批记录后commitAsync且在关闭前commitSync。或用spring，无需调整autocommit
    * 如果没到5秒且处理了一部分，则挂了恢复后会重复处理前一部分。如果消费者会输出到外部系统，要自己实现幂等性，一般如果消息里有主键，覆盖是没问题的，又称effectively-once
...
服务器配置：
Docker镜像confluentinc/cp-kafka通过环境变量配置：如KAFKA_ADVERTISED_LISTENERS
...
内外都访问：listeners监听两个端口且改自定义名字如INTERNEL和EXTERNEL，内部的用内网IP，advertised.listeners同理（如果不为控制结点则可以不填与listeners一致），listener.security.protocal.map将自定义名字设置验证方法如PLAINTEXT，inter.broker.listener.name设为内部的

log.retention.hours 默认168即7天后删除。但只有“关闭”了的片段才会开始倒计时：log.segment.bytes和log.roll.hours
log.dirs 一个主机上开多个实例时要改
...
最大消息大小：broker的message.max.bytes，topic的max.message.bytes，生产者的max.request.size，默认1MB；消费者的fetch.max.bytes，默认50MB
...
acks=0/1/all 默认all表示等min.insync.replicas数量的follower同步完再返回，1表示leader完成即可
retries 默认是0

消费者配置：
推荐设置client.id
auto.offset.reset 没有offset时从哪读，默认latest。设为none表示如果当前消费者没有offset记录则报错
max.poll.records 默认500，影响poll()，底层fetch时会自动缓存更多
group.id：Connector默认为connect-cluster，spring的stream默认为${spring.application.name}，不要和它们重名

认证和加密：
PLAINTEXT表示无认证无加密，SSL表示TLS双向证书验证和加密（也可像HTTPS只有单方证书，只加密无认证）。
SASL_PLAINTEXT表示只认证，传输无加密。SASL_SSL表示用SASL认证，SSL传输加密。
SASL有多种方式，其中用用户名密码的有PLAIN和SCRAM。PLAIN验证时会发送原始密码，应配合SASL_SSL。SCRAM不会泄漏原始密码，要预先在broker启动前做一些配置。
储存在硬盘上的静态数据，没有内置加密方法。可以用生产者端到端加密。
```
...
消费者：c.subscribe(List.of(topic)); while (true) { for(var record : c.poll(Duration.ofMillis(100))) {record.key()/value()} }

// Spring: org.springframework.kafka:spring-kafka
spring.kafka.bootstrap-servers=localhost:9092
spring.kafka.consumer.group-id=myGroup

@Autowired KafkaTemplate<String, String> kt;
生产者：kt.send(topic, val) 或 (topic, key, val) 返回 Future 可以.whenComplete((result, ex)->{if(ex == null)等待结果else处理异常})
消费者：@Component 中 @KafkaListener(topics = "topic1,t2") void processMessage(String content, ConsumerRecordMetadata meta) {}
Stream：@EnableKafkaStreams 自动配置了StreamsBuilder，用它创建KStream
自动创建topic（若已存在则不会重复创建）：@Bean NewTopic t() {return TopicBuilder.name("t1").partitions().replicas().config(k,v).build();}。还可用KafkaAdmin.NewTopics一次创建多个
```
...
* UI：https://www.kafkamagic.com/ https://www.conduktor.io/
