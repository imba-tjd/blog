...
* Stream
  * key又可看作group名或stream名
  * 写入：add(Map)，不过因为不再追求HKV是纯文本，可以序列化成二进制了。从对象创建记录：StreamRecords.objectBacked(o).withStreamKey(key)或newRecord().in(key).ofObject(o)；o是单独的String也行
  * 读取：read()
...
* CommandLineRunner、ApplicationRunner：会在项目启动后执行，不会阻塞ServletDispatcher。是接口，在Configuration里写一个@Bean的函数return new args -> ...。也可用@Componet里的方法上加@EventListener(ApplicationReadyEvent.class)或ContextRefreshedEvent.class，前者会启动玩tomcat，后者在bean创建后tomcat启动前
