...
spring:
  ...
  jackson:
    ...
    date-format: yyyy-MM-dd HH:mm:ss # 默认ISO格式，影响序列化和反序列化
    deserialization:
...
* ConditionalOnExpression("${conf} == true（可省） && ...")
...
* HttpSession默认是Tomcat的
  ...
  * Cookie默认加了Httponly，没加Secure
  * server.servlet.session.timeout=秒 默认30m
...
* spring.session.store-type=若只有一个spring-session则无需配置; spring.session.redis.xxx; spring.session.timeout未设置时会自动用servlet的
...
* CommandLineRunner：会在项目启动后执行。是接口，在Configuration里写一个@Bean的函数return new args -> ...
...
// SpringBoot3.2：RestClient，Fluent API但是同步的，不需要starter-webflux
restClient.get()
