...
* nonce：只使用一次的随机数，无需保密。用于确保消息的唯一性，防止重放攻击
* scramble：一种数据混淆或变换操作，如sha(pw+salt)
* SCRAM：一种算法，能在无加密信道上完成验证而不泄露密码
* cipher：根据上下文，可以指ciphertext密文，或加密算法（如凯撒cipher）
...
hash算法可以验证数据的完整性，但无法保证数据防篡改（真实性）和来源（身份认证）。如果简单地把消息和其哈希值发出去，中间人可以修改消息再重新计算哈希值。\
MAC是带有密钥的哈希。CMAC是用对称加密算法计算的MAC，现在一般不用，只用HMAC。\
MAC与DSA的区别：MAC相当于对称加密，DSA相当于非对称加密。Poly1305是比HMAC更快的MAC。\
数字签名提供非否认性，MAC不行。它们都不提供机密性。

* Encrypt-and-MAC(E&M)：把明文分别独立加密和MAC。接收者解密，再计算MAC，与提供的比较。最差
* MAC-then-Encrypt(MtE)：从明文计算MAC，再把两者加密
* Encrypt-then-MAC(EtM)：将明文加密，再从密文计算MAC。推荐
* hash(key||message)存在长度扩展攻击，但hash(message||key)没问题。https://crypto.stackexchange.com/questions/8728
* AEAD(带有关联数据的认证加密)：同时提供机密性、完整性和真实性，且对于“关联数据”支持后两者，方便处理元数据
...
* XSS跨站脚本
  ...
  * 缓解办法：CSP
* CSRF跨站请求伪造：用户先正常访问某一存在漏洞的网站A，登录了，具有cookie。用户访问恶意网站，里面存在一个img标签，src是A的GET接口，则触发漏洞。若接口是POST的，可构造form，自动submit
  * 解决办法：加验证码或隐藏的随机表单字段。ajax一般取meta里放的csrf-token；另一种方法：服务端放到Cookie里，客户端取出来放到请求头里，服务端验证请求头。验证Referer
* 区别：XSS是用户过分信任网站，直接执行网站的脚本，如把本页面的cookie发送到某个地址。CSRF是网站过分信任用户，某个合法用户（有合法cookie）调用了API，但是表单提交不会验证跨域
* 通过XSS可以绕过基于Token的CSRF防御措施（本质上就是执行任意js）
...
* mysql_native_password：mysql.user中储存SHA(SHA(pw))无盐。验证时服务端发随机数，客户端sha两次后与随机数异或，发送给服务端验证，不需要加密连接
* caching_sha2_password：创建时，把密码加盐 sha2 5000次；虽然正常情况下消耗了服务端算力，但目的是防止拖库后离线攻击。首次登录时，先尝试快速登录，失败。创建TLS（如果未使用则用服务器的RSA公钥），发送明文密码，sha2 5000次完整验证。成功后，服务端缓存sha2(pw)。快速登录时，服务端发送nonce，客户端与明文密码进行一些运算后发送，服务端进行相同操作
...
* 路径穿越与覆盖：如 ../，可以读取或覆盖系统文件
* Zip Slip：构造特殊的压缩包，路径中有 ../
* 软链接：上传包含软链接的压缩包
* 横向越权：前端指定其他人的路径里的文件
...
* https://github.com/fkie-cad/cwe_checker 检查二进制文件中常见的漏洞
* https://lucia-auth.com/ 使用js和ts实现验证逻辑的资源
* https://github.com/Z4nzu/hackingtool
* 防爬虫 https://github.com/TecharoHQ/anubis
