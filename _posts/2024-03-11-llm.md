...
* 英文：llama mistral gemma。小模型：phi。超小：MBZUAI/LaMini-Flan-T5-77M（微调自谷歌）、apple/OpenELM-270M-Instruct
...
* evaluate
* 加速
  * optimum.BetterTransformer：model.to_bettertransformer()或pipeline中accelerator="bettertransformer" free-lunch加速，支持CPU和GPU。内部使用pytorch的SDPA，内部使用FlashAttention算法；启用也可用attn_implementation="sdpa"，但BetterTransformer功能更多；某些特性会逐渐合并到普通的transformer里
  * Flash Attention 2(flash-attn)：attn_implementation="flash_attention_2"。仅支持GPU，宣传说如果支持则没有理由不用；不清楚是否仅加速推理
  * ExLlamaV2：仅支持GPU
...
* quanto：HF出的易于使用的量化工具包，兼容torch.compile()，可在线量化，支持CPU；不支持PEFT，不支持保存（可能支持但还没集成到transformers）；感觉对于推理完胜bnb。也是Optimum的后端
