...
* 英文：llama mistral gemma。小模型：phi、llama3.2
  * 超小：facebook/opt-125m、MBZUAI/LaMini-Flan-T5-77M 微调自谷歌但不是CausalLM而是Text2Text、apple/OpenELM-270M-Instruct（tokenizer用的llama2的，用起来麻烦）、MobiLlama-05B-Chat、facebook/esm2_t6_8M_UR50D Fill-Mask、https://huggingface.co/Felladrin
...
* 结构化输出：Provide content in JSON format with the following keys: aaa, bbb。Foramt your response as a list of items separated by commas。Make your response as short as possible.
...
* 加速
  ...
  * https://github.com/microsoft/onnxruntime https://github.com/microsoft/deepspeed
...
* optimum-intel
  ...
  * Neural Compressor（仅CPU）：optimum-cli inc quantize。基于块结构化稀疏性，支持主流DL框架
...
* backend（和pytorch是同一地位的）：openvino、onnx
...
* 从训练到生产全流程的平台，包括监控，最后生成API：https://www.comet.com https://neptune.ai/ 开源替代：https://mlflow.org/ https://wandb.ai/ (又叫Weights & Biases) TensorBoard。辅助开发RAG和Agent应用的平台：https://mindsdb.com/
* 要解决的问题
  * 准确性：模型提供的答案是否正确
...
* Realse版本区别：CPU推理用avx2；不知道是否可用openblas，可能依赖系统中的。vulkan和kompute好像是同类的，都支持某些NV和AMD。cudart含有CUDA运行时当系统里没装时才用，否则用cuda版。IntelGPU用sycl，内部用了oneAPI。AMD用hipBLAS。linux只有唯一选择用的CPU
* 简单模式
