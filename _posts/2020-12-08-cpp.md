...
std::ios_base::sync_with_stdio(false); // 关闭C风格IO同步提升速度，就不要用scanf和printf了，等价于cin和cout.tie(nullptr);
...
regex_search(str, m, pattern); // 返回bool
cout << m.str(); // 可用m[n]取分组
regex_search() // 完全匹配（即隐式$，与py不同）
...
cout << (it == st.end() ? "true":"false") // 括号不可少
...
public:
    Dog() {};
    Dog(string n): name(n),... {} // 这种语法称作成员初始化列表
    ...
    virtual void print() const; // 末尾的const表示不修改对象的状态，实现中要写，只有这样才能被const引用对象调用
}; // 注意
...
ifstream infile(filename); // 读到文件末尾时评估infile就是false
...
* shared_ptr：多个指针引用同一个对象时使用；不要循环引用；尽量使用make_shared初始化；涉及将this指针提升为shared_ptr时要用shared_from_this()
