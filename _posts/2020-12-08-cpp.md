...
* 可以让non-const函数调用const函数重载，不能反过来，因为const函数不改变状态而non-const不保证。同名函数有无const是两个重载
* 权限
  ...
  * 友元类和函数：在public下写friend R，则R类可访问本类对象的private的。写friend void f(T t)，则任意一个地方的void f(T t)，t是本类对象，可访问本类private的。不会被继承
  * 多个父类要分别写权限
* 继承、OOP
