...
* C++11引入大括号初始化，此时可不写=，int i {1.2}会报错而非隐式截断
* C++17支持在if中创建变量：if(int a=1;condition)
* C++17规定简单赋值和复合赋值表达式右边的早于左边的，因此 i=i++ + 1 不是未定义行为；还规定了位移运算符左边早于右边，因此cout<< i<< i++不是未定义行为。另外C++11规定前缀自增副作用早于其值，因此 i=++i+1 不是未定义行为
...
cin.getline(chararr, len); // 读取到C的字符数组。不能用string
...
* 支持 + +=(append) ==
* C++17提供string_view，基本相当于只读指针+长度，能统一处理string和C风格字符串。但对于末尾\0不做保证，不能与C API交互
* C++20提供ranges，有split view

```cpp
#include <string> // 虽然被iostream间接包含了，但那是实现的问题
string s = "123";
auto s2 = "Hello world"s; // C++17，注意最后的s
string { size, fill_ch不可省 }、string { char_p, size }
s.size(); // 3；与length()无区别，不计算末尾\0
s.substr(from [, cnt])
s.find("23"); // 返回索引；还有rfind。不支持指定s的搜索范围，它的count参数缩小的是pattern范围。find_first_of是查找pattern中任意一个字符
s.insert(at, "xxx");
s.replace(from, n, "repl"); // 删除从from开始的n个字符，再插入repl
s.reserve(100); s.capacity(); s.shrink_to_fit();
s.push_back('1'); s.pop_back(); // 像vector一样使用，也支持algorithm如reverse()

// 与数字之间互转
stoi(str); to_string(num);
from_chars(str, str+len, num); // 不抛异常，还支持sv。C++17
...
#include <regex> // 第三方库：谷歌的re2、intel的hyperscan
string str = "asdf1234";
...
// std::format 需C++20 GCC13 Clang14。std::print，需C++23 GCC14(-lstdc++exp) Clang18
...
* 其它容器
  * list双向链表，forward_list单链表
  * deque(读作deck，内部是连续块的链表，具有O(1)的访问速度)，是queue和的stack底层。后两者称为容器适配器
  * priority_queue底层用vector
  * set和map称为关联容器
* Lambda
...
```cpp
#include <vector>
vector<int> vc {1, 2, 3}; // 不要用vector<bool>，用bitset。C++17也可以不写<int>
vector { cnt, fill_v不写则默认初始化 }。数组转它：vector { arr, arr+n }，也支持it.begin() end()
...
std::erase_if(vc, predicate) // C++20，代替std::remove + erase，因为remove只是把元素移动到末尾。还支持迭代器范围
vc.data() // 指向内部缓冲区的指针
...
for (auto it = vc.cbegin(); it != vc.cend(); it++) // 手动遍历迭代器输出
    cout << *it;
危险：T* t = &v[v0]; 如果之后vector自动扩缩容，会移动元素，t和迭代器失效。一般外部存ndx；如果要对象地址稳定，用vector<unique_ptr<T>>或list
...
// 报错默认构造函数被删除：用了不可哈希的类型，要特化std::hash<T>
...
* 只在声明中写，实现时不用写的：virtual、override、static
  * static对象必须在类外初始化，且要再写一遍类型。或用C++17 inline static变量可在类内初始化
* 范围限定
  * 匿名命名空间：用途类似于static函数，调用时不用加命名空间名，也只在本文件中可见
  * 用::访问类级别的成员
  * using后范围内如果有重名的`xxx`对象，用无前缀的`::xxx`调用本范围的，直接`xxx`会报错
  * 子类重写了父类函数或定义了自己的构造函数时会隐藏其它函数，可引入用using Base::Base/foo
* 函数
  * 函数支持重载。支持默认参数，必须只能设置一次，一般放在声明处
  * 可以让non-const函数调用const函数重载，不能反过来。同名函数有无const是两个重载，如果返回引用一般都要写，分别返回可修改和不可修改的引用
  * mutable 字段使得const函数也能改变该字段，设计上是不把它视作对象的状态。一般用于：缓存、mutex、统计信息（计数器）
* 权限
  * struct与class没有本质区别，只不过成员和继承默认public
  * protected继承会把public的变成protected的，private继承（class默认）会把public和protected都变成private的
  * 友元类和友元函数：在public下写friend R，则R类可访问本类对象的private的。写friend void f(T t)，则任意一个地方的void f(T t)，t是本类对象，可访问本类private的。不会被继承
  * 多个父类要分别写权限
* 重载运算符有两种方式：一种是成员函数，隐含this。另一种是有lhs和rhs两个参数的friend函数，若写在头文件中要加inline

```cpp
class Dog {
    string name;
public:
    Dog() {}; // 无参构造函数，大括号表明这是实现，也可用=default。不会初始化基元类型成员，类类型可以
...
class MyDog: public Dog {
    void print() const override; // 注意位置。override同java，可选
};

初始化：
Dog d{};  调用无参构造函数。不能写Dog d();因为这被认为是声明函数。不过{}会优先匹配initializer_list
Dog d;  如果Dog用默认构造函数，则d的基元字段不会初始化（垃圾值）。如果自定义了无参构造函数，则与{}一样。new Dog和new Dog{}同理
Dog d1(MyDog());  直接初始化，执行拷贝构造函数。TODO: AI说这样也是声明函数，用(MyDog())可解决
Dog d2 = MyDog();  复制初始化，若存在explicit Dog(const MyDog& rhs)时执行它，否则与直接初始化一样
Dog d3 = Dog();  C++17后保证与Dog{}无区别，不会调用拷贝构造函数
初始化与赋值意义不同，在引用上很明显：`int a; int& b; b = a;`不行，而`int& b {a};`可以，`int& b=a;`也可以且=不表示赋值

聚合类型(Aggregate Type)：允许列表初始化（不用声明构造函数，直接{按字段声明顺序初始化}）。不能有自定义构造函数、虚函数、非public字段；可以有父类但父类要满足这些要求。字段可以是类对象
POD (Plain Old Data)：是聚合类型的子集（限制更多），字段也必须是POD的（不能有string），字段只能出现在整个继承树中的一个类（影响不大，一般无继承）。目的：与C的二进制兼容，能memcpy
```

### 继承、OOP、特殊构造函数

* 阻止类继承、阻止函数重写：final关键字
* 菱形继承：继承了同一个父类的两个类在继承时标记virtual，继承了那两个父类的子类在初始化时必须也初始化公共父类，最终公共父类只有一份。有运行时开销，成员尽量少
* vtable：每个存在（或继承了）虚函数的类的对象会包含一个隐藏指针vptr，指向其类的vtable。vtable是一个函数指针数组，每个条目对应一个虚函数。子类具有自己的vtable，内存布局上类似于类的继承，先放父类虚函数，再放自己的，这样子类指针转为父类后能正常用；如果其中某个函数没有override，它会用父类的函数地址
* 特殊构造函数（适用于管理资源的类）
  * 三之法则：同时定义拷贝构造函数、拷贝赋值运算符（销毁dst的原资源再拷贝）和析构函数
  * 五之法则：如果要实现移动语义，还要同时定义移动构造函数、移动赋值运算符
  * 零之法则：如果没有管理资源，就不要定义特殊函数。使用RAII容器能自动处理
  * 编译器默认会生成三法则的特殊函数+无参构造函数
    * 当成员存在引用（无法重绑定）和const时无法自动生成拷贝赋值函数
    * 声明定义任意构造函数则不会生成无参构造函数
    * 声明拷贝或移动的其一会阻止另一方的默认生成
  * 如果一个类存在删除了拷贝构造的字段，则它的拷贝构造函数会被自动删除
* 子类会自动调用父类的public无参构造函数
* https://github.com/microsoft/proxy 重新实现的运行时多态

```cpp
class Dog {
    // 拷贝构造函数，初始化时用到，里面一般浅拷贝某些资源，如char[] 用 new + std::copy
    Dog(const Dog& d);

    // 移动构造函数，把d里的有生命周期的资源赋给自己，再把d里的那些设为null，防止d错误析构
    Dog(Dog&& d) noexcept;

    // 拷贝赋值函数，在非初始化时用=赋值时用到
    Dog& operator=(const Dog& d) {...; return *this};

    // 用于类型转换到T的函数，就是没有返回类型的
    operator T();
};
...
public:
    ...
    // 防止切片：多态必须通过指针或引用才能生效，否则若将子类实例赋值给父类对象会发生切片
    Interface(const Interface& _) = delete;

    // 如果有子类或存在任何virtual函数，则必须定义（非纯虚声明）virtual的析构函数。因为默认生成的析构函数非virtual（但有noexcept）
    // 会隐式调用成员的析构函数，自己完成后再自动调用父类的。子类如果没有管理自己的资源可不定义
    virtual ~Interface() = default;
    // 如果需要实现逻辑
    virtual ~Interface() noexcept {}

    // 纯虚函数：该类看作接口，无法单独实例化（但如果需要管理资源完全可以用构造函数），派生类必须实现此函数
    virtual foo() = 0;
}
```
...
* 声明函数时加=delete可用于编译期禁止它，如f(int)能用f('a')调用，声明char重载=delete就会优先匹配禁掉
* static_cast：一般的强转，如数字转换、子类指针转换为父类指针。编译期类型检测
* const_cast：用于去掉const修饰符
* dynamic_cast：把父类指针转换成子类指针或反过来，不符合时返回nullptr；也能转换引用，不符合时抛bad_cast。一般认为不应使用，如果是运行时多态闭集可用variant，编译时用Concept。依赖RTTI。另有typeid关键字更直接
...
template<typename T> // 实现方法，写在.h中，且无需inline
Box<T>::insert(...){...}

// 特化，当用到Box<int>时自动使用下面的实现。如不实现在头文件中，要用extern template声明
template<>
class Box<int> {...}
...
// 类型别名模板
template<typename C>
using Element_type = typename C::value_type; // 取C类型的value_type成员类型。typename告诉编译器是类型而不是静态变量，当在模板中引用一个 依赖于模板参数 的名字（包括type traits），且该名字表示一个类型时要用
template<typename Container> void algo(Container& c) {
    vector<Element_type<Container>> vec; // 表示任意一个具有value_type成员的类型，取它的元素类型作为vec的泛型参数
}
// 类似于偏特化
...
* 避免元编程大量报错：-Wfatal-errors
* constexpr用在函数签名开头：对函数有一些限制，可以使用循环和局部变量。赋值给constexpr变量时在编译期求值，赋给const变量时可能在运行期求值。参数如果是等效常量就在编译期求值，否则也能在运行期计算，不用重复定义一遍。一般放在头文件里，隐式内部链接。if constexpr()在编译期消掉分支
...
使用：template <fable T> 含有f的实现。<typenmae T> 一般的实现

// 递归变参模板，需要出口，另一种方式：void print(){}
...
* `#include <utility>`，不过会被许多常见的头文件间接包含。此头文件还有 swap()、exchange()、to_xxx。swap+x.clear()->move
* 左右值
  * ++a是左值，与C不同
...
* 被move后的对象处于“valid but unspecified”的状态，要求调用析构函数不能出错。“移动”并不代表“消失”，移动的源和目标是两个对象
* 如果对象的地址被观察（address observable），就不应该允许 move
...
class C {
    vector<int> _v;
public:
    C(vector<int> v): _v{std::move(v)}
}
```

## 其他类型

* 强枚举类型：`enum class Color {Red, Blue}; Color c = Color::Red;`
  * 不会隐式转换为整数。主动转换：`std::to_underlying`（C++23）
  * 从整数转换成它：初始化器
  * 无法和整数数值比较，除非加`:unsigned int`
  * Neargye/magic_enum：与字符串互转，header_only，静态反射
...
* variant单个类型表示多种可能情况，闭合类型集合。替代union，或表示正确结果或错误。或当类型确定时的运行时多态：`variant<int, double> v; if(holds_alternative<int>(v)) cout<<get<int>(v); get_if<int>(&v)`
  * struct Visitor { void operator()(int val) const {...}、另一个const string&重载 }; std::visit(Visitor{}, v1)
...
* span：就是`T* + size`，不拷贝内容。能直接接收vector、array、栈上数组。如果传指针，还需传大小
...
```cpp
#include <fstream>
ofstream outfile(filename, ios::默认trunc追加用app); if(!outfile) 文件打开失败。是RAII的
ifstream infile(filename); while(infile) ...; for(int i; infile>>i;)。infile.eof()达到文件尾，infile.fail()上一次读取失败如类型不匹配
string content(istreambuf_iterator<char>(infile), istreambuf_iterator<char>()); // 读取全部内容到string里
二进制：ios::binary，f.write(reinterpret_cast<char*>(&x), sizeof(x))
还有一个ate，打开时在末尾，可以seek回去。而app只能写末尾
...
* 裸指针表达无所有权语义：当它还活着的时候，程序员必须保证它指向的资源必须是活着的；当裸指针结束生命周期的时候，不需要做任何操作。一般用在函数参数
...
* shared_ptr
  * 一般用在异步、事件驱动、缓存，一个对象被多个异步操作持有，不确定最后谁完成。如果生命周期在编译期可以确定，大概率不要用它
  * 创建用make_shared，之后按值拷贝（否则引用计数不增加）。可以属于不同线程，引用计数是原子的，如果持有者都只读，推荐加const，不用锁了
  * 不要循环引用。涉及将this指针提升为shared_ptr时要继承enable_shared_from_this
* weak_ptr：不控制目标对象的释放，循环引用时将其中一个改成weak_ptr。临时使用if(auto sp = wp.lock())，expired()判断是否过期
...
unique_ptr<Dog> dogp { Dog的构造函数参数 }; // 代替new，在堆上构造。也可以传裸指针
dogp->bark();

auto dogp2 = make_shared<Dog>(构造函数参数);

shared_ptr<Dog> dogp3 = std::move(dogp); // u转s
auto dogp4 = make_unique<Dog>(std::move(dog)); // 栈上对象转堆
dogp.reset(dogp_bare); // 给默认初始化过的智能指针赋值
auto arr = make_unique<int[]>(10); // 特化了数组

指定裸指针的清理函数。此处无额外开销，如果用函数指针则有：
struct TDeleter {
...
C++26：
#include <stacktrace>
catch([[with_stacktrace]] std::exception& e) { cerr << std::stacktrace::from_current_exception(); }
```

## 多线程

* std::thread 必须join()或detach()。jthread(C++20)具有RAII特性自动join
* mutex：一般不直接使用，声明mutable字段，加锁时用lock_guard(m_mutex)，是RAII，在一小块区域加锁{里面写guard}
  * 读写锁：shared_mutex字段，unique_lock写锁，shared_lock读锁。当多线程并发读同一个共享块时用，否则效率不如普通的
* 常见把thread作为自定义Worker类的字段
  * 当没有“关联”任务时，不会占用操作系统线程
  * 创建线程时一般会捕获this，因为要用Worker对象的函数。写法：thread(&MyClass::Work, this)、jthread([this](stop_token st) { Work(st); })。前者写法的this是成员函数指针调用规则，如果用jthread也会自动尝试第一个传st
  * 但如果Worker在vector中发生自动扩缩容，this会失效，解决办法见上方vector笔记。或用enable_shared_from_this，捕获self（当前对象的shared_ptr）
* condition_variable
* 原子变量
  * 可指定内存屏障。一般用法：flag.store(true, std::memory_order_release)、flag.load(std::memory_order_acquire)、counter.fetch_add(1, std::memory_order_relaxed)仅保证原子性不保证有序性用于计数器。默认seq_cst所有原子操作单一全局顺序
  * Store Buffering问题：如线程A x=1;r1=y，线程B y=1;r2=x，直觉上至少有一个r会被赋值为1，但实际可能都为0，因为赋值时存到缓存而不是主存里了。如果此处用release/acquire，仍然无法解决，因为C++的R保证之前的读写完成（StoreStore）、A保证之后的读写不超过a（LoadLoad）、LoadStore不重排序由二者共同保证。但却允许StoreLoad重排序，也即可以先读，导致仍然都为0。R允许下方的操作往上跑，不限制后面的代码提前执行（只要不违反数据依赖）；A允许上方的操作往下跑，不限制前面的代码延迟执行。解决办法就是用seq_cst
...
* struct Empty{}; sizeof(EmptyS) -> 1，因为要保证不同对象有不同地址。C是0
* struct S: Empty { int x; }; sizeof(S) -> 4。Empty不占空间，即EBO
...
static thread_local std::mt19937 rng(std::random_device{}()); // 具有内部状态
static thread_local std::uniform_int_distribution<int> dist(1, 5); // 没保存状态，但仍不能多线程共用
int sleep_seconds = dist(rng);
std::this_thread::sleep_for(std::chrono::seconds(sleep_seconds)); 不可打断
```
...
* https://abseil.io/ Google的C++基础库。https://github.com/facebook/folly
* https://github.com/cameron314/readerwriterqueue 无锁队列，header-only
* https://github.com/gabime/spdlog header-only，-DSPDLOG_USE_STD_FORMAT
* https://github.com/uxlfoundation/oneTBB 并发库
* https://github.com/catchorg/Catch2 单测
...
std::byte、std::as_bytes

RUST: https://github.com/google/comprehensive-rust https://github.com/microsoft/RustTraining
...
https://zhuanlan.zhihu.com/p/2052882507285128213
