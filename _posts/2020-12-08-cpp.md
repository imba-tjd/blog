...
```cpp
#include<vector>
vector<int> vc {1, 2, 3}; // 可以为空不初始化，普通数组就不行；不要用vector<bool>，用bitset；大括号省略了=
...
auto [a,b] = pair<int,int>{1,2}; // C++17拆包的方法，会声明a和b两个变量
...
Dog d3 = Dog(); // c++17后与优化无关保证与d无区别，不会调用拷贝构造函数，且这样还能用在类成员变量中
```
