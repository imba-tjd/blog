...
```cpp
class Dog {
    // 拷贝构造函数，初始化时用到，里面一般浅拷贝某些东西，如char[]
    Dog(const Dog& d);

    // 移动构造函数，把d里的有生命周期的资源赋给自己，再把d里的那些设为null，防止d错误析构
    Dog(Dog&& d) noexcept;

    // 如果有子类或存在任何virtual函数，则必须定义（非纯虚声明）virtual的析构函数
    // 会隐式调用成员的析构函数，自己完成后再自动调用父类的。子类如果没有管理自己的资源可不定义
    virtual ~Dog() = default;
    // 如果需要实现逻辑
    virtual ~Dog() noexcept {}

    Dog& operator=(const Dog& d) {...; return *this}; // 拷贝赋值函数，在非初始化时用=赋值时用到
