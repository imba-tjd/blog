...
* Lambda
  ...
  * capture写&会捕获所有局部变量的引用，写&x只捕获x，写=是拷贝所有，不用也要写[]。常量表达式无需写捕获
  * 参数必须声明类型
  * ->return可不写
  * 实际是函数对象
...
* 可以让non-const函数调用const函数重载，不能反过来因为const函数不改变状态而non-const不保证：用`static_cast<const C&>(*this)`把自己转换成const引用，调用完后用`const_cast`去掉const修饰符。另外`reinterpret_cast<const byte*>(&in)`将对象视为字节序列，转换成其它类型是未定义的；dynamic_cast把父类指针转换成子类指针，不符合时返回nullptr，也能转换引用，不符合时抛bad_cast。C风格的强转就是自动选择C++的四个强转之一。C++20提供bit_cast
* 纯虚函数：声明virtual函数时在结尾加=0，则该类看作接口，无构造函数无法单独实例化，派生类必须实现此函数。如果自己不是抽象类却又没实现虚函数，Lint检测不到错误
* 若A的构造函数接受B对象，B的构造函数接受int，可直接A(1)、A a = 1；实现参数为initializer_list的构造函数就能支持初始化列表。在构造函数参数前加explicit禁止此行为
...
* X64下默认为一种标准调用约定（叫做x64 __fastcall），指定cdecl和stdcall会自动忽略；另有GCC不支持的vectorcall
