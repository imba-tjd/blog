...
* 可以让non-const函数调用const函数重载，不能反过来因为const函数不改变状态而non-const不保证：用`static_cast<const C&>(*this)`把自己转换成const引用，调用完后用`const_cast`去掉const修饰符。C风格的强转就是自动选择C++的四个强转之一
...
public:
    ...
    Dog(string n): name(n),... {} // 这种语法称作成员初始化列表，且初始化顺序为对象在类中的声明顺序，与此句顺序无关。也可用name{n}
...
* 返回值默认开启了Return Value Optimization：普通函数返回栈上实例(包括同类型右值和同一个左值)会在调用者处直接分配和调用一次构造函数，不会返回右值、拷贝赋值、析构右值；也可能调用移动构造函数。不要也无法返回局部变量的引用，但若想允许调用者访问函数所在对象的成员则可以考虑返回引用。对于传进来的引用参数可以考虑返回move，这时候不可能自动优化
...
* 替代union：`std::variant<int, double> v; if(holds_alternative<int>(v)) cout<<get<int>(v);`
