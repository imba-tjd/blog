...
* 函数
  ...
  * 可以让non-const函数调用const函数重载，不能反过来。同名函数有无const是两个重载
* 权限
...
* vtable
  * 每个存在虚函数的类的对象会包含一个隐藏指针vptr，指向其类的vtable；vtable是一个函数指针数组，每个条目对应一个虚函数。子类具有自己的vtable，内存，如果其中某个函数没有override，它会指向父类的函数地址
* 特殊构造函数（适用于管理资源的类）
  * 三之法则：同时定义拷贝构造函数、拷贝赋值运算符（要先销毁原资源）和析构函数
