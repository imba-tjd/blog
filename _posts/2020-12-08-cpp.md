...
* 权限
  ...
  * 友元类和函数：在public下写friend R，则R类可访问本类对象的private的。friend void f(T t)，则任意一个地方的void f(T t)，t是本类对象，可访问本类private的
* 函数
  * 支持重载。支持默认参数，必须只能设置一次，一般放在声明处
...
* 构造函数
  * 单参数构造函数隐式转换：若A的构造函数接受B对象，B的构造函数接受int，可直接A(1)、A a = 1、一个接收S的函数f，S又具有接收int的构造函数，则可以直接f(123)调用。在构造函数参数前加explicit禁止此行为
    * 实现参数为initializer_list的构造函数就能支持初始化列表
...
public:
    ...
    Dog(Dog&& d); // 移动构造函数，把d里的有生命周期的资源赋给自己，再把d里的那些设为null，防止d错误析构
