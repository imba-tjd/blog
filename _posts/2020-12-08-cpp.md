...
sort(vc.begin(), vc.end()); // 默认升序。第三个参数传greater()是降序，或自定义写返回bool的函数，a>b是降序。多字段升序：if(a.x!=b.x)return a.x<b.x; ...; return false。还有stable_sort
accumulate(b, e, 0) // 求和。对于double序列，初始值必须写0.0，否则会先转换成整数
transform(vc.begin(), vc.end(), vc2.back_inserter(), [](int x){return x*x;});
...
public:
    ...
    virtual ~Dog() noexcept {} // 必须要是virtual的否则有问题，一般如果存在任何virtual函数就应该声明析构函数；会隐式调用成员的析构函数，自己完成后再调用父类的
...
* optional：使用上就可为nullptr的对应类型的指针没啥区别、any相当于动态类型
...
* std::thread 必须join()或detach()
* mutex和shared_mutex：一般不用。用lock_guard和unique_lock，是RAII
