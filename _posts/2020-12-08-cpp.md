...
std::erase_if(vc, predicate) // C++20，代替std::remove + erase，因为remove只是把元素移动到末尾
vc.data() // 指向内部缓冲区的指针
...
* 权限
  ...
  * 多个父类要分别写权限
* 对于POD：new MyStruct() 会清零初始化，new MyStruct 不会初始化
...
* vtable
  * 每个存在（或继承了）虚函数的类的对象会包含一个隐藏指针vptr，指向其类的vtable。vtable是一个函数指针数组，每个条目对应一个虚函数。子类具有自己的vtable，内存布局上类似于类的继承，先放父类虚函数，再放自己的，这样子类指针转为父类后能正常用；如果其中某个函数没有override，它会用父类的函数地址
* 特殊构造函数（适用于管理资源的类）
  * 三之法则：同时定义拷贝构造函数、拷贝赋值运算符（销毁dst的原资源再拷贝）和析构函数
  * 五之法则：如果要实现移动语义，还要同时定义移动构造函数、移动赋值运算符
  * 零之法则：如果没有管理资源，就不要定义特殊函数。使用RAII容器能自动处理
  * 编译器默认会生成三法则的特殊函数+无参构造函数。当成员存在引用和const时无法自动生成拷贝赋值函数。声明定义任意构造函数则不会生成无参构造函数。对于C++11，声明拷贝或移动的其一会阻止另一方的默认生成
...
* dynamic_cast：把父类指针转换成子类指针或反过来，不符合时返回nullptr；也能转换引用，不符合时抛bad_cast。一般认为不应使用，如果是运行时多态闭集可用variant。依赖RTTI。另有typeid关键字更直接
...
* `#include<utility>`，不过会被许多常见的头文件间接包含。此头文件还有`swap()`和`exchange()`。swap+x.clear()->move
* 左右值
  * ++a是左值，与C不同
...
* 被move后的对象处于“valid but unspecified”的状态，要求调用析构函数不能出错
...
* 强枚举类型：`enum class Color {Red, Blue}; Color c = Color::Red;`。不会隐式转换为整数，用初始化器可从整数转换成它；无法和整数数值比较，除非加`:unsigned int`。手动转换为对应的整数：`std::to_underlying`
...
* variant单个类型表示多种可能情况，替代union，或表示正确结果或错误，或当类型确定时的运行时多态：`variant<int, double> v; if(holds_alternative<int>(v)) cout<<get<int>(v); get_if<int>(&v)`
...
* 用裸指针表达无所有权语义：当它还活着的时候，程序员必须保证它指向的资源必须是活着的，并且当裸指针结束生命周期的时候，不需要做任何操作。一般用在函数参数
...
* shared_ptr：多个指针引用同一个对象时使用；不要循环引用；尽量使用make_shared初始化；涉及将this指针提升为shared_ptr时要继承enable_shared_from_this。一般用在异步、事件驱动、缓存，一个对象被多个异步操作持有，任何一个都可能是最后完成的；如果生命周期在编译期可以确定，大概率不要用它
