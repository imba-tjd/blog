...
// std::format 需GCC13 Clang14
#define FMT_HEADER_ONLY
#include "fmt/format.h" // 正常使用（-lfmt）可用core更精简
cout << fmt::format("Hello {}!\n", "world"); // 或用fmt::print。C++23支持std::print，需GCC14 Clang18
...
* 还有list是双向链表，forward_list单链表。deque(读作deck)双向链表是queue和的stack底层，list是priority_queue的底层，称为容器适配器。set和map称为关联容器
