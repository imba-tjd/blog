...
Close_Wait：服务端收到客户端的FIN，看自己是不是还有数据要发送。
出现大量的Close_Wait：服务端（被动关闭）程序没有调用close()，如数据库阻塞，没有回复第二个FIN。
客户端的状态是FIN_WAIT_2，如果1分钟没等到响应会直接关闭。

Time_Wait：主动发起关闭的那一方（一般是客户端）的最后一个状态，需等1分钟才会释放socket。如果不复用socket可能会耗尽。
