...
Close_Wait：服务端收到客户端的FIN，看自己是不是还有数据要发送。出现大量的Close_Wait是什么情况：https://www.cnblogs.com/yorkyang/p/7657683.html，反正是服务端的问题，比如数据库阻塞，没有回复第二个FIN。客户端的状态是FIN_WAIT_2，如果1分钟没等到响应会直接关闭。

Time_Wait：主动发起关闭的那一方（一般是客户端）的最后一个状态，需等两分钟才会释放socket，如果不复用socket可能会耗尽。服务端如果出现，可能的原因：HTTP1.0由服务端关闭、长连接客户端持续无响应超时（一般是网络问题）或数量太多（主动关闭最早连接的）、与内部之间没有用连接池
