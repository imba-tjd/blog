...
* 迪米特法则 Law of Demeter（降低访问耦合）：每一个软件单元对其它单元只有最少的知识，且局限于与本单元密切相关的。如出现o.f1().f2()则违反了此原则，但若f(o)=>g(o.f1()); g(x)=>x.f2()则没违反；“不和陌生人说话”对应了此原则
...
* 与策略模式相比，Client不需要知道当前状态，就能改变行为；而策略模式，Client必须明白不同算法的区别
