...
* 迪米特法则 Law of Demeter（降低访问耦合）：每一个软件单元对其它单元只有最少的知识，且局限于与本单元密切相关的。考试中如出现o.f1().f2()则违反了此原则，但若f(o)=>g(o.f1()); g(x)=>x.f2()则没违反；“不和陌生人说话”则对应了此原则
...
* 在观察目标(Subject)里定义观察者集合，可以添加(register/attach)和删除(remove)，定义notify()在需要通知时循环调用抽象观察者类的update(data)
...
* 状态之间的转换：可以由环境类负责，此时环境类充当状态管理器角色。也可以由具体状态类负责，此时状态类和环境类之间会存在依赖关系，即Client调用环境类上想要的方法，环境类转发调用State对象的方法实现，且参数传递自己，State方法里面修改环境类参数的state
