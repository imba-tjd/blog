...
* 顺序图：二维第四象限，横轴是对象，第一个一般是角色，纵轴是时间，从上往下进行。描述多个对象之间的交互，时间轴之间用横的箭头表示函数调用
  * 也可能称作交互图。但实际上交互图包括顺序图和其他几个图
  * 通信图：就是去掉了时间轴，改用123、2.1 2.2
* 活动图（流程图）：就是有开始和结束、菱形表示分支选择的那个图
