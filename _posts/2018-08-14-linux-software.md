...
* 支持交互式命令mega-cmd以及一大堆脚本式命令mega-*，都会自动启动mega-cmd-server
* login email，会话信息保存在home中。默认不开启HTTPS，用https on开启
* get下载且可不登录直接从分享链接下载，put上传，quit退出交互式shell
...
* 自动多线程
* -c断点续传
* -O指定要保存的文件名，不加时默认用URL最后的部分。输出到stdout：`-q -O-`
