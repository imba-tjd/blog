...
* 数据来源，一般没有足够的数据，解决：找数据集（Kaggle）、合成数据
* 四类角色：领域专家，知道什么数据是重要的、如何获取。数据科学家，数据挖掘，模型训练和部署。ML专家：定制化模型。SDE：开发维护数据流水线、serving
...
* 均值 EX、μ、mean
  * μ是总体真实均值，无法得知。因此我们会抽取样本，计算样本均值x̄来估计μ
  * EX和μ是同一个值，表示进行无数次试验后，随机变量所有可能取值与其对应概率的加权平均。从概率分布的视角，无论我们是否进行实际观测，只要随机变量的概率分布是确定的，它的期望值就是确定的
* 方差 variance：一种测量分散程度(variation)的方式，低方差为集中
  * 总体（真实）方差 σ² = E[(X-μ)^2]。此处的E展开后为 X取具体值x的概率 乘以 (x-μ)^2 求和；如果概率相同，则算式为后者除以n。如果是连续型随机变量、已知概率密度函数则为求积分
    * 另一种记号：DX = E(X²) - (EX)²
  * 样本方差 s² 是总体方差的估计量 = SS(mean) / (n-1) 即 每个样本值与全体样本值的平均数之差的平方值的平均数。也叫 抽样方差、无偏估计、无偏方差。n-1称为贝塞尔校正
  * 协方差：conv(X,Y) = E[(X-EX)(Y-EY)] = E(XY)-EXEY
  * 标准差 Standard Deviation σ = 方差的算术平方根。又称均方差、均方根值（RMS）。样本标准差用s
* SS(mean) - sum of squares around the mean 均值周围平方和：计算X的平均值μ，再将每个x计算(x-μ)^2，求和
  * SSR - sum of the squared residuals 也称为L2损失。(预测值 - 真实值)^2，求和
* Var(mean) - variance around the mean 方差 = SS(mean) / n
* MSE, Mean Squared Error 均方误差, Var(residual)：是常见的损失函数。也叫总体方差、有偏估计。再开方称为均方根误差RMSE
...
* CARTs：分类和回归树
* 向量的L1范数：每个分量取绝对值，相加。L2范数：每个分量分别平方，相加，开根号。L∞范数：所有分量的绝对值中最大的那一个数
* TODO: 除以n和除以n-1的区别
...
* 有的模型如LR不接受空值：Input contains NaN, infinity or a value too large。有的模型内置支持但很少。“NaN陷阱”：当模型中的某个值超出浮点精度限制时，系统会将该值设置为NaN，而不是数字。当模型中的一个数字变成NaN时，模型中的其他数字最终也会变成NaN
...
线性模型假设特征之间存在线性关系，要将分类变量转换为数值。

* Nominal无序分类变量用独热编码OneHot Encoding。Ordinal有序用标签编码Label Encoding，如教育程度：小学<中学<大学。
  * 如果使用标签编码，如性别编码为0 1，模型可能错误地认为“1比0大”。如邮编虽然全为数字，但不视为数值数据
* 类别数量极大时，独热编码会导致维度爆炸。此时需选择其他方法：目标编码Target Encoding（用类别目标均值替代、将频次少的全归为“其它”类）、Embedding、频率编码
* OneHot独热编码的多重共线性问题：如3个值会创建3个feature分别是[1,0,0],[0,1,0],[0,0,1]，但其实用2列就能表达，分别是[0,0],[1,0],[0,1]。解决办法就是删掉一列，pd用drop_first=true，sklearn用drop='first'
* 树模型不需要转换，或模型支持指定参数自动转换，如categorical_feature、enable_categorical=True
* OneHot向量的稀疏表示法：一个数字代表那个下标为1，其余为0
...
将连续变量分箱(binning/bucketing)。

按分位数：每个桶的样本数大致相同，对于偏差数据可以尝试。

可以用于将feature的值聚类：如年龄对于购买力的影响，25和26不应有显著区别。可分为0-18 18-40 40-60 60-100四类再OneHot编码。但边界处会突变，可改为0-18 15-40 35-60 55-100
...
* 向量的L2归一化：v / sqrt(sum(vi**2))，即对每个值除以v的L2范数，得到的向量长度为1。消除向量长度对模型的影响，进行相似度计算时有用
...
* 特征冗余的影响：某些因素在各维度重复出现，导致影响被放大。或权重分散到多个维度上，权重稀释；权重值低不一定是它不重要
* PCA主成分分析
  * 无监督学习
  * 将数据集绘制成点，用类似于线性回归的方式找到一个方向，称为PC，保留了最大数据方差。二维降一维时就去掉与之正交的，多维下就寻找正交且继续具有最大方差的
  * 会“旋转坐标轴”，产生新坐标
  * 使用前应对数据放缩
  * 只能线性降维，需要数据基本线性可分
  * sklearn.decomposition.PCA 用户指定处理后的维数(n_components)或百分比或mle算法自动猜测
  * 算法步骤：标准化数据。构造协方差矩阵。计算特征值和特征向量。降序排序特征值，从而排序特征向量。选择最大的n个特征值对应特征向量，构造投影矩阵。方差解释比：各主成分特征值的占所有特征值之和的比例。载荷：原始特征对主成分的贡献程度
* 因子分析：是前者的扩展。还能分类
...
* 用最小二乘法(least squares method, LSM)将数据拟合到直线。数学上可以求导得到正规方程直接得到参数值，但工程上一般用迭代的方式
  * 当数据符合正态分布时，使用MSE优化LR，等价于最大似然估计
...
* Huber Loss胡贝尔损失：结合了MAE和MSE的优点
  * MSE对离群点敏感，因为误差是平方的，一个大的误差会被放大得更多，导致模型为了拟合少数离群点而牺牲了整体的拟合效果
  * MAE梯度是常数，在误差很小时可能收敛速度较慢
  * HuberLoss 当误差较小时类似MSE，较大时类似MAE

### 岭回归 Ridge Regression / L2正则化 Regularization

* 当训练数据量很少时，即使训练集拟合得好，测试集也不好，称作低bias高variance。岭回归增加bias提升泛化能力，还能用于处理多重共线性参数不稳定的问题
...
* 虽然叫回归，但其实仅用于分类
* sigmoid函数：英文表示形状像S的函数、S形曲线。logistic函数：一种典型的sigmoid函数，f(z) = 1 / ( 1 + e ^ -z )
...
* 又称为 多项式逻辑回归。用于多元分类最后一层，有几个输入就有几个输出（类别），将输入转化为“概率”，不改变相对大小关系。当K=2时退化为二元逻辑回归
* f(i) = e^i / sum(e^j) 值域在[0,1]，sum(f(i))==1
  * 求导：会用到商法则。对于A类的概率输出，对A类的导数为Pa(1-Pa)，对B类的导数为-PaPb
* 损失函数：交叉熵损失 Cross-Entropy Loss = -log(p) 其中p是Target类别的输出值；再对每个样本计算求和。此函数对于p=0产生大值，而MSE最大只有1
* model.predict_proba 对于多分类，依次返回各个类的概率（二分类就会返回2个）

其它多元分类方法：

* OvR(One versus Rest) 为每个类别训练一个分类器，此类别为正，其余所有类别都是负。缺点：类别数据不平衡。最终判断时一个样本可以属于多个分类。sklearn现只用multinomial
* OvO 组合为每两个类别训练一个分类器，使用时为类别投票
...
* 距离度量：连续型有闵可夫斯基距离、余弦相似度、皮尔逊相关系数（= cov(X,Y) / 二者标准差）。离散型有汉明(编辑)距离、杰卡德Jaccard相似系数（= 1 - 二者相同的项/总项）
...
* 贝叶斯定理：P(A|B) = P(B|A) * P(A) / P(B)。P(A|B,C) = P(B|A) * P(C|A) * P(A) / (P(B)*P(C))
  ...
  * 对于分类任务，分母不知道也不必计算，对于A的每个取值，计算分子，比大小即可
  * 数学上，P(A|B)叫后验概率，P(B|A)叫似然，P(A)叫先验概率，P(B)叫边际概率
...
* 投票分类器：在同一个数据集上训练多个 不同算法 模型，选择出现次数最多那个类别（众数）。绝对多数投票：必须大于1/2；相对多数：就是最多的；软投票：输出概率，选平均概率高的。如果是回归，一般用均值
* stacking：也是组合不同算法，将不同模型的输出作为新特征，再训练一个元模型聚合输出
...
* boosting提升：每个模型接受上一个模型的输出，修复它的问题（减少欠拟合/训练误差），顺序
  * AdaBoost：每个预测器不断改变样本的权重
  * GradientBoost：没有调整样本权重，而是使用前一个预测器的残差作为标签进行训练
  * XGBoost：实现上进行了并行优化，支持分布式训练。能自动处理异常值和缺失值等。实测完全不调参与RF差不多。后期出了hist版内存占用更小。https://neptune.ai/blog/xgboost-everything-you-need-to-know
...
选择K的值的方式：肘方法，当K增加时，Loss（集群内距离总和）一开始会迅速减小，之后减小的幅度平缓了，取拐点的K。\
...
* 用非ML术语回答“我要达成什么目标？”
* 先验证当前的非ML解决方案。如果没有，可以用“启发式”(heuristic)（又叫探索式），看起来基本就是按经验选几个feature，用固定方式计算，不保证全局最优
* 比较ML在质量上的改进、费用和维护
...
df.corr()  皮尔逊相关系数，默认对每两列依次计算，产生大小为cxc的矩阵；对于具体两列中的一行，会自动不计算存在NaN的。假定前提：两个变量之间是线性关系，或正态分布
...
* 其他库：yellowbrick图形化，mlxtend工具类，dtreeviz可视化，scikit-optimize，m2cgen把模型转换为其它语言，featuretools，Sacred能保存各种参数用于复现
  * 自动机器学习，自动调超参数：https://auto.gluon.ai 基本只要去掉id，指定lable就行。https://github.com/nidhaloff/igel
...
* 文本的数值表示
  ...
  * Word2Vec：基于DL的无监督学习模型，稠密

### 词袋模型BoW

1. 词汇表：dict{单词:序号} 按字母顺序
2. 特征向量：对于每个文章或句子，将其中的词汇聚合生成 list[(序号，出现次数或称为单词频率即词频)] 或 隐含序号list[频率]，长度等于词表长度，称为MultiHot

缺点：不考虑词序特征、文法、句法特征。顺序丢失（无聊不好玩 与 好玩不无聊 的BoW完全相同）。忽略语义（苹果手机 与 吃苹果 的苹果意义不同）。忽略单词之间的距离（手机 互联网 苹果 前两者距离应更近）。比较稀疏，词表内容多时会维度爆炸（其中一种解决办法：将出现次数少的都归类为OOV类别）。

sklearn：feature_extraction.text.CounterVectorizer(stop_words='english', max_df=.1, max_features=5000).fit_transform(data) 删除频率大于0.1的单词，保留频率前5000个

**n-gram**：上面的BoW称为1-gram或unigram。2-gram对于分词后的结果，两两按顺序组合编码。\
如对于原文ABCD，若仅按观察到的作为特征，则创建(AB, BC, CD)；若基于所有可能的，则创建(AA, AB, BA, ...) 共4x4=16个，非常稀疏。

**词频-逆文档频率TF-IDF矩阵**：某些词在各个文章中经常出现，则它通常不包含有判别性的信息。本方法相当于对某些单词加权，更突出“领域”词。\
对于一个词，tf-idf = tf * idf。tf就是BoW里的词语t的出现次数。idf是log( 语料库文档总数 / (包含词t的文档数+1) )。之后再将向量L2归一化。\
例如：某文章，“且”和“机器学习”出现次数相同，则它们的tf相同。再看语料库，如果每篇文章都出现“且”，则它的idf是log1等于0。

主题聚类/情感分析：超参指定n个主题个数。对于一些无label的文章，将它们归到k个类中，能输出每个类中最关键的n个单词。\
常用算法： **LDA潜在狄利克雷分配**，一种概率模型，原理很复杂，需要贝叶斯推理知识，实现用了EM算法将BoW分解成 文档主题 和 单词主题 矩阵，使得二者相乘能尽量还原输入矩阵。\
sklearn：decomposition.LatentDirichletAllocation(n_components=10).fit_transform(X) 能自动解析出10个主题（仅有标号），以及BoW中所有单词在各主题中的重要性升序
...
信息过滤系统，自动帮你挑选。排序系统，预测并排序出用户最有可能感兴趣或采取行动的项（用户建模、行为预测）。
与搜索系统的区别：用户发起查询，精确匹配、满足意图。也可以结合。

优化目标（评价指标）：
离线：准确率、F1等
在线：点击率CTR、转化率CVR、留存率DAU、交易总额GMV。常用A/B测试和多目标建模
用户测试：问卷调查，展示结果让用户打分排序、是否满意

非个性化（基于统计）：所有新闻的TopN、湖南人喜欢吃辣
个性化推荐：基于内容、协同过滤、基于知识、基于关联规则
实时性：离线推荐、实时推荐

流程（流水线）
1. 用户与物品数据采集
  * 用户数据：注册信息、点击记录、浏览行为、收藏、购买、评分、停留时长、搜索词。用户画像构建：静态属性（性别、年龄）、行为偏好（短期、长期）
  * 物品数据：内容标签、价格、类目、发布时间、热度、描述文本、图片特征
  * 上下文数据：访问时间（小时、节假日）、地点（GPS、IP）、设备类型、网络环境、天气
2. 召回/候选集生成：从几十亿缩小到几百个。决定上限。方法：内容匹配、协同过滤、Embedding向量检索，用ANN如Faiss和Annoy、基于规则、热门。多路召回
3. 评分、排序。建模目标：CTR预测、评分预测、行为多目标预测如点击+购买。模型：LR、GBDT、NN、多任务学习模型如MMoE、PLE。训练指标：LogLoss、AUC、NDCG
4. 重排序/干预：移除用户不喜欢的，避免重复的，提高新内容得分，广告

术语：内容项（也称文档）是系统推荐的实体。查询（也称上下文）是用于提供建议的信息。

基于内容的过滤（物品本身的属性）：用户看了两个猫视频，则推荐可爱动物。
设计一些tag（需要领域知识），如电影有导演，音乐有流派。计算重合数（Jaccard指数）。特征提取方式：关键词、TF-IDF、文本分类、图像识别
优点：不需要用户历史行为数据。

知识图谱：结构化语义网络，记录 “人 - 物品 - 属性 - 关系” 等多种信息，用于捕捉复杂兴趣关联与语义信息。如用户喜欢某电影->导演是X->推荐同导演其他电影

### 协同过滤 Collaborative Filtering

* 基于用户行为：显式喜欢（进行评分）、隐式喜欢（播放，不包括一秒钟就退出的）
* 分类（理论假设）
  * 基于用户的CF：两个用户相似，一个喜欢A，则向另一个推荐A。能发现用户的潜在偏好、新兴趣
  * 基于物品的CF：一用户喜欢A，另一用户喜欢A和B，则向前者推荐A。物品数量通常比用户数量稳定，且物品之间的相似度可以提前计算并存储
* 问题：冷启动效果差，新用户和新物品 没有历史行为。用户-物品评分矩阵 数据稀疏性，用户打了分的物品很少。可扩展性，用户和物品太多
* 步骤
  1. 计算用户相似度。基于用户对**相同物品**的评分，可用余弦相似度、皮尔逊相关系数（其实就是向量-均值再按余弦相似度算）
  2. 选择TopK个相似的邻居
  3. 对目标用户尚未评分的物品，用邻居用户的评分加权平均进行预测，选取预测评分最高的若干个物品作为推荐结果。计算方式：用户自身已有的平均评分 + 邻居用户对该物品的“加权偏离”即 sum(相似度*(具体评分-均分)) / sum(相似度)
