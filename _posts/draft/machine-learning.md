...
流程：召回/候选集生成（从几十亿缩小到几百个）、评分、重排序/干预（移除用户不喜欢的，提高新内容得分）

术语：内容项（也称文档）是系统推荐的实体。查询（也称上下文）是用于提供建议的信息。

候选集生成方法：
    基于内容的过滤（物品本身的属性）：用户看了两个猫视频，则推荐可爱动物。设计一些tag（需要领域知识），如电影有导演，音乐有流派；计算重合数（Jaccard指数）。优点：不需要用户历史行为数据。
    协同过滤CF：基于用户行为，两个用户相似，一个喜欢A，则向另一个推荐A；也有基于物品的：给喜欢A的用户推荐其他同时喜欢A和B的用户的B。显式喜欢（进行评分）、隐式喜欢（播放）。先找到相似用户，再找相似且未推荐过的物品。优点：能捕捉隐含偏好，可能发现新兴趣。缺点：冷启动效果差，包扩新用户和新物品；可解释性差
