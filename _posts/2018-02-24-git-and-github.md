...
* rebase的edit选项，修改后要add, commit --amend, continue，与冲突不同

### 冲突

* 发生冲突时，文件状态为unmerged changes，像是staged和local的中间态
* 解决冲突后需要add，或rm，然后git merge --continue，不用加分支名，无需commit
...
* GPL
  ...
  * 带有FOSS许可例外的GPL：https://zhuanlan.zhihu.com/p/38444122 如果自己开发的项目用了OSI批准的开源协议则可以自己选，否则那个库将视为GPL
...
* git init --bare xxx.git：用作同步中心，不包含工作区，不能使用平常的git命令。可以**接受**push，如果客户端能通过ssh访问某主机，在主机上用此命令，客户端就可以指定路径推送了。命名按照习惯以.git结尾，实际下下来的就是`.git`文件夹
