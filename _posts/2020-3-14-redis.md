...
* 不要用作复杂的消息队列
  * MQ：确认机制、持久化、事务支持、优先级、分区
  * Redis：容忍小概率消息丢失，通过定时任务/手工触发达到最终一致。消息堆积概率低，有相关的报警监控
  * PUB/SUB即发布-订阅模式的生产者和消费者是1-M的关系，一条消息会被多个消费者消费，解决的是广播的问题。消息会在当时就处理并删除，如果当时没有订阅，消息也不会留着。没有ACK
  * PUSH/POP没有ack机制，消息取出后消费失败依赖于client记录日志或者重新push到队列里面
    * 改进：先BRPOPLPUSH到另一个列表中，成功处理后LREM，添加监视程序每一段时间把另一个列表中的放回原队列中因为它们代表失败，但这又可能出现重复消费的情况，因此要实现幂等性，保证重复消费结果一致
  * Redis Stream
...
* Win版，非官方
  ...
  * https://github.com/CacoCode/LeadingCode.RedisPack/tree/master/attrs 实测7.2-rc会报STATUS_ACCESS_VIOLATION用不了
  * https://github.com/icetech233/windows-msys-redis 7.0
  * https://github.com/hemnstill/StandaloneTools 实测7.2.3会报STATUS_ACCESS_VIOLATION
...
daemonize yes # 默认no，Debian配置默认yes。使用systemd时无效。Win下有效
supervised auto # 对systemd的支持，默认no
pidfile /run/redis.pid

loglevel notice #【默】
logfile /var/log/redis/redis-server.log # 或用syslog-enabled

maxclients 10000 #【默】Win下实际最多大概3200
timeout 0 #【默】客户端空闲多久后关闭连接。0表示禁用超时
tcp-backlog 511 #【默】还受内核影响
tcp-keepalive 300 #【默】多久没收到客户端ACK视为掉线

io-threads 4 # 建议遇到性能问题时设为核心数-1，超过8意义不大。仅在输出给客户端时使用 TODO: 还需io-threads-do-reads yes？
...
* 配置：maxmemory 100mb 存在主从复制时还要给系统保留一些、maxmemory-policy xxx。active-expire-effort在后台用多少CPU主动检测过期数据
...
* no-enviction：默认，不淘汰数据，如果内存不够就报错，仍可读。Active-Active Geo-Distributed CRDBs一直用此项
...
dir /var/lib/redis # RDB和AOF写入的文件夹。若用相对路径则是相对于CWD的
dbfilename dump.rdb

appendonly yes # 默认关闭
appendfilename "appendonly.aof"
appenddirname "appendonlydir" # Redis7+会生成多个aof文件，放在单独的文件夹里
...
2. 当master出现故障的时候，会自动选一个slave作为master顶上去，原master后续如果恢复了会自动变为slave。
3. 与集群相比没有分区，数据冗余太多

配置文件写`sentinel monitor 任意名字 主节点ip 端口 1`，运行时加--sentinel，一些其它配置会自动生成。最后一个值表示有几个哨兵认为主节点挂了才切换。端口默认26379。Jedis在使用时传入那些哨兵的地址，从那里得到master节点。
...
* Streams（5.0）：只读、只附加的日志，参考了kafka，可以被多个消费者读取，有消费者组，有ACK。只是一种数据结构，与分布式无关。https://redis.io/docs/data-types/streams/ https://www.zhihu.com/question/51634494
* disque：没有完成开发的消息队列模块
...
DbSize：Key的数量
FlushDb：清空当前数据库所有Key；FlushAll：清空所有数据库的Key。都有ASYNC选项
```
...
* dragonfly：兼容redis和memcached，在单机上比redis更好
