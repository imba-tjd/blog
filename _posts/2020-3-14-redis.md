...
* 热Key：可能存在单点瓶颈、主从复制压力。解决：①识别、监控。②多级缓存。③打散
...
* 两者都由后台fork的子进程完成，共享内存COW无需加锁就能读取。如果宿主机内存不够可能会失败，但其实不会消耗很多内存，可用`echo 1 > /proc/sys/vm/overcommit_memory`
...
Shutdown：保存并关闭server，和在bash中的kill服务端效果相同；Save：把数据保存到rdb中会阻塞主线程，BgSave：异步保存，设置里的save其实是它；恢复数据：把rdb放到Config Get dir中即可，BgRewriteAOF：异步写AOF；Auth：验证密码，Quit：退出当前cli，Time：返回服务器当前时间
