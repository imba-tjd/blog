...
* 集合运算，如共同好友
* 一些简单数据的存储，低频改动，但是会被频繁读取。比如首页推荐的产品列表。要做高可用
...
* 是否要设置过期时间。如果设置，过期了要怎么做；如果不设置，容量越来越大怎么办
* 热Key：可能存在单点瓶颈、主从复制压力。解决：①识别、监控。②多级缓存。③打散（同一个数据加_xx后缀分到不同节点）
...
* 默认无限内存 + 不淘汰数据
* 配置：maxmemory 100mb 包括临时缓冲区在内的总大小，主从复制时尤其考虑
* active-expire-effort在后台用多少CPU主动检测过期数据

maxmemory-policy：

* no-enviction：不淘汰数据，如果内存不够就报错，仍可读。Active-Active Geo-Distributed CRDBs一直用此项
  * 设置volatile系时，如果没有过期的，内存又满了，还是会报错
* allkeys-lru：从数据集（server.db[i].dict）中挑选最近最少使用的数据淘汰
* volatile-lru：从已设置过期时间的数据集（server.db[i].expires）中挑选最近最少使用的数据淘汰
* volatile-ttl：从已设置过期时间的数据集中挑选将要过期的数据淘汰；如果ttl一样就选最近最少使用的
* volatile-random：从已设置过期时间的数据集中任意选择数据淘汰
* volatile-lfu：从已设置过期时间的数据集中选择最近最不常用的数据淘汰。普通LFU存在问题：很久以前大量使用增加了很多频次，但后来一直没用了；Redis做法是按一定时间减少频次数
* allkeys-random、allkeys-lfu：略
* lfu与lru的区别：当执行淘汰时假如一个不常用数据最近刚好用过，那lru下可能会淘汰另一个常用但最近没用过的数据；但lfu资源消耗更大一点

过期删除策略：1.惰性删除，客户端访问某个过期Key时删除。2.定时删除。即使上面设为不淘汰仍然会删除。

### 持久化

* RDB方式文件紧凑，保存的是数据，而AOF是命令操作日志。适合隔一段时间持久化保存，恢复速度相对快，可能丢失数据
* AOF方式默认每秒fsync且性能仍不错，因此最多丢失一秒的数据（主从复制时还可能丢两者未同步的）。当文件过大时会自动重写压缩不过仍比RDB大，在处理巨大的写入载入时性能差一些
  * 复制备份aof文件时可能因为正在重写而失败，应先暂停
  * 重写原理：如set a 1之后又设为2，则保留2
* 两者可以同时启用，此时默认混合持久化，AOF文件里前半部分内容是RDB格式
* 两者都由后台fork的子进程完成，共享内存COW无需加锁就能读取。如果宿主机内存不够可能会失败，但其实不会消耗很多内存，可用`echo 1 > /proc/sys/vm/overcommit_memory`。另一种选择是主节点只用AOF，从节点RDB冷备
* 启动redis-server时会自动读取dir中持久化的数据，如果数据太大就会花很久
* 如果AOF也损坏，可备份原有文件，用redis-check-aof进行修复，用diff -u对比
* 命令
  * Save：把数据保存到rdb中，会阻塞主线程
  * BgSave：异步保存，设置里的save其实是它
  * BgRewriteAOF：异步写AOF
...
* 主从是异步复制的。wait命令可等待多副本写入（或用哨兵模式指定min-slaves-to-write）

```conf
replicaof <masterip> <masterport> # 作为slave连接master
masterauth <master-password>
replica-serve-stale-data yes #【默】当slave失去与master的连接时是否继续响应客户端请求

shutdown-timeout 10 # master关闭时等待从的最长时间
```

#### 哨兵模式

* 原理
  * 监控master和slave是否正常运行（每秒PING），以及哨兵之间也会相互监控。只用于监视和故障转移，不负责主从复制、反代转发数据
  * 当master挂了，会自动选一个slave作为新master顶上去。原master后续如果恢复了会自动变为slave，且全量同步新master数据
  * 当一个哨兵发现master挂了，称为“主观下线”。它会向其他哨兵询问，如果其它（超过阈值）哨兵也认为master挂了，称为“客观下线”。那个下判断为客观下线的哨兵称为Leader，负责完成主从切换，即将其他slave设为跟踪新master，并用pubsub通知客户端
* 配置
  * 最小架构：三个可用区，一个Master，两个Replica，分别再在每个可用区里加一个哨兵。即哨兵最少3个
  * 配置文件：`sentinel monitor 任意名字 主节点ip 端口 2`。运行redis-server时加--sentinel，一些其它配置会自动生成
    * 最后一个值表示有几个哨兵认为主节点挂了才切换
    * 可以监视多个不同的redis实例
  * 如果运行在docker里，要特别设置announce-ip和port
* 客户端使用：客户端填哨兵地址和端口（默认26379），从那里自动得到master节点
* min-slaves-to-write和min-slaves-max-lag：需合理配置以防止脑裂，小于阈值时master拒绝写入
...
* 创建
  ...
  * redis-cli --cluster create 节点地址1 地址2
    * 运行后会交互式确认创建，然后就会退出
    * 也可以再给master配slave，用--cluster-replicas 1 表示每个m配一个s。但若有密码则replica要用另外的配置文件，加masterauth，否则会大量尝试，CPU飙高，出现大量TIME-WAIT连接
* 配置
...
* 对应object/model的用法，但不能嵌套。如果经常读写对象的某一个属性，则可用；如果每次都全读，不如普通的String+序列化
...
* 使用跳表skipList；元素数量少时用zipList压缩列表
...
* 每个bitmap支持约40亿位（数组长度），且允许稀疏
* 适合布尔值。能进行一些统计。其中key按唯一标识，下标按最小分隔单位，如 用户id 每一天是否签到
* 如果业务有多个字段，如 详细时间、关联另一实体的ID，则不能使用
...
* Exec提交，中间不会插入其它连接的指令。如果提交后出现错误，已执行的仍有效；如果提交前命令存在语法错误，则不会执行
* Discard取消
...
Shutdown：保存并关闭server，和在bash中的kill服务端效果相同；Auth：验证密码，Quit：退出当前cli，Time：返回服务器当前时间
...
* redis-benchmark -n 10000 -q 性能测试，同时执行一万个请求。新工具：memtier_benchmark

## 其它项目

* dragonfly蜻蜓：兼容redis（到5.0）和memcached，在单机上比redis更好
...
* apache/kvrocks
* https://github.com/dicedb/dice 兼容
* nalgeon/redka：Go，以sqlite为储存的兼容redis协议。但不支持lua
* https://hazelcast.com/ Java，高性能实时数据处理，也有类似于kafka的功能
* Java客户端
  * Redisson 基于netty。不仅仅是客户端，更像是架构解决方案、分布式中间件。代码中拿到RMap、RList。还支持分布式锁
    * EmbeddedRedisRunner，封装了jedis-mock，纯java实现
  * lettuce 基于netty，是spring默认的
  * jedis 在redis官方组织下，线程不安全但池化后就可以了
