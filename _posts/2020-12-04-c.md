...
* C23
  * 二进制字面量0b1010。glibc2.35的printf支持%b
  * 数位分隔符'
  * 有符号整数必须用补码表示
...
* C99的designated initializer初始化指定索引的元素：`int arr[4] = { [1] = 123, 456 }`，其中456赋给arr[2]，其余都初始化为0
  * C允许在前面定义后面的内容：`{[2]=5, [0]=1, 2}`，C++不支持。GNU扩展范围初始化：{[0 ... 2] = 1}
* sizeof(int[3])支持
* VLA
  * 必须是自动存储类别，定义时不能初始化
  ...
  * sizeof能在运行时正常计算大小
  * 不能用在结构体中
* 作为参数时会退化为指针，可省略最高维长度，可自增改变；传二维数组时必须声明长度，如`int arr[][3]`或`int (*arr)[3]`，也可用VLA`int cols, int arr[][cols]`；结构体中的定长数组不会退化，但会完整复制一遍；表达式中数组转换成的指针不是左值。C++用auto&不会退化，17用std::size代替ARRLEN宏
...
* 严格来说void*不能进行+等数学运算
* 数组与指针不同之处：数组名只读、sizeof、字符串常量、高维数组或取&、解引用读或赋值（数组就好像普通变量一样，而指针要将值加载到寄存器后再解一次引用）、声明而不初始化（数组会分配对应长度的空间，可以给内容赋值，指针会初始化成NULL，不能解引）
* nullptr：C23
...
* 匿名联合/结构体：`struct S{int a; union { int b; struct C c; struct {short d;} };}`。一般用起来类似于继承：`S2{S1; extra_field;}`
* 复合字面量 (Point){1,2}
  * 可取地址是左值，用于避免值传递大对象又不想创建局部变量再&
  * 可return、赋值给已有对象变量
  * 成员设计器：可命名和不按顺序 {.x=1, .y=1}
  * 给一个结构体变量清零：s = (typeof(s)){0}，编译器优化了不会生成临时对象，比memset更好
  * C++不支持不按顺序和取地址，要求是“聚合类型”
  * 初始化时的 Point p={1,2} 不是复合字面量
* 对齐/布局
  * 结构体成员相对于起始地址的偏移量是其*对齐大小*的整数倍，如不能则在前一个成员后补充字节，再占用sizeof的空间；0被认为是任何数的整数倍
    * 基元类型的对齐大小(alignof)不一定等于其内存占用(sizeof)大小，但x64下目前都等于
    * stdalign.h：C11，提供alignof(type)获得对齐大小、alignas(type/n)更改单个成员的对齐大小。C23后变为关键字
      * 如{int, alignas(16)char, char, int}的内存占用大小是32，其中第一个int后空12字节，第二个char紧挨着第一个char因为它的对齐没改变，第二个int前空2字节，后空8字节
  * 结构体整体的内存占用大小是其成员中最大的对齐大小的整数倍，如不能则在总后面补充字节；整体对齐大小就是成员最大对齐大小
    * 结构体中的结构体：将内层结构体视为整体
    * #pragma pack(1)指定所有成员连续储存。若设为其他数n，对齐量按min(n,最大成员大小)
  * 结构体变量的首地址必须是结构体变量中“最大基本数据类型成员所占字节数”的整数倍。这条看起来不影响计算结构体大小，但堆上怎么对齐呢？存疑
...
* 优先级：算移比位逻赋：!、*/%、+、>>、>=、==、&^|、&&、?:、=
  * (n&1) == 1
  * a+b>>1 等价于 (a+b)>>1。a>>1+b 不等价于 (a>>1)+b
  * cout << (a == b)
* 与、和、或 运算符有顺序点，`p && *p++`可防止空引用
...
* a%b 可用 a-(a/b)*b 来计算。a和b的最大值可用 (a+b+abs(a-b))/2
...
* 右移是向下舍入，数学上-3除以2等于-1.5，C的-3>>1等于-2，-3/2等于-1。右移且向上舍入：`(x+(1<<k)-1)>>k`
...
* mode
  ...
  * w只写不读，w+可读写，永远创建新文件。wx和w+x（标准没有规定wx+）已存在时打开失败，py无需加w；完全不是“独占打开”，fopen_s的w和a才是独占
...
## 生存期、存储期、linkage、作用域、命名空间

* 处于生存期中的对象保有地址和值，超出生存期访问对象是UB。除了分配到堆上的对象外，生存期等于存储期。复合字面量拥有临时存储期
...
* 块和原型作用域变量只具有空链接；块作用域变量可具有auto、register、static存储期，其中static存储期在程序执行期间一直存在，不会进入寄存器，若有初始化必须用编译期常量
* 文件作用域永远具有静态存储期；默认为外部链接，会导出符号，使用static可改为内部链接，则在每个翻译单元里会产生单独的代码；如果想在别的文件中使用，别的文件中必须加extern作为声明，否则相当于重复定义(或加到头文件里)；函数和函数声明本身属于此；`int a; main(){extern int a;}`使用的就是前面的a，只能引用全局变量且会覆盖同名局部变量，数组可不加长度且不能和指针互换
...
LLVM、Google都是花括号不换行，但默认都是2空格缩进。Google的指针符号跟随类型。当if内容很短时，Google会放在同一行。
