...
* C23：二进制字面量0b1010。数位分隔符'。有符号整数必须用补码表示。能指定位数的大整数_BigInt(n)及其字面量后缀wb和uwb。constexpr
...
* 对齐/布局
  ...
  * long double在x86上的对齐量是4
* 位域：如`union{struct{uint8 a:1;b:1;c:1;d:1}; unit8 data;}`，data=2相当于b=1，即与声明顺序一致
...
* 边界检查函数`_s`无意义：GCC和Clang没有实现，MSVC的实现与标准有少量不同，MinGW64实现了
* EOF
  * 为int类型的负值，如-1
...
* mode
  ...
  * w只写不读，w+可读写，永远创建新文件；wx和wx+当文件已存在时打开失败，完全不是“独占打开”（TODO:也可能是当系统支持时本程序x，其他程序r会失败）
...
* offsetof：不是关键字，是宏，位于stddef.h中。size_t offsetof(struct, field)
...
* strict-aliasing严格别名：假定不相似类型的指针会指向不同对象，或相同内存地址不会放不同类型的数据，则可以用restrict那样的优化。如int和unsigned就是相似的，但int和float不相似。但存在char* 和void* 时除外，比如函数参数存在char*，对此变量赋值，会假定另一个参数可能被更改
