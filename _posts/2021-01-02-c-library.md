...
* 环境变量：char* getenv(const char*)。POSIX定义了setenv()、putenv()、unsetenv()
...
* 已经从流读取了数据，则在写之前要调用定位函数；已经写入了数据，在读取之前要用fflush或定位函数
...
* clock_t clock()：返回从程序开始执行起处理器所消耗的时间（不是经过的时间），除以CLOCKS_PER_SEC就得出消耗的秒数。目前返回值实际为long
* time_t time(NULL)：unix时间戳，即UTC的1970年1月1日0点0分0秒开始到现在经过的秒数。标准未规定time_t的实现方式，目前实际为long，2038年时会不够
* char* ctime(const time_t*)：将time_t解析为类似于`Sun Jul 4 00:00:00 1976\n\0`，下一次再调用这个字符串会被覆盖。实际上可能以asctime(localtime(time_value))实现
* double difftime(time_t t1, time_t t2)：计算t1-t2，结果为秒
* struct tm* gmtime(const time_t*)：把time_t转换为UTC(GMT)
* struct tm* localtime(const time_t*)：把time_t转换为当地时间
* char* asctime(const struct tm*)：参见ctime，参数不同
* size_t strftime()：把tm格式化成字符串，参数略因为以后大概不会用到，其中一个和printf类似
