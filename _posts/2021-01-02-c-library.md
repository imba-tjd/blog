...
* atexit(void(*func)(void))：把一些函数注册为退出函数，当程序正常终止时（exit或main结束），那些退出函数将按照注册的反序被调用；不要在那些函数里使用exit。Ctrl+C不会调用
...
* struct tm* gmtime(const time_t*)：把time_t转换为UTC(GMT)
  * localtime() 转换为当地时间
* strftime(outbuf, cnt, pattern, tmp)：把tm格式化成字符串
...
* handler：void h(int){...}
  ...
  * 从handler返回会从中断的地方继续运行
* 当一个已经设置了handler的信号发生时，系统首先恢复对该信号的缺省行为，这是为了防止handler内部也引发这个信号死循环（但允许内部手动再次注册），然后再调用handler。所以如果想多次捕捉同一个信号，要在handler返回前再用一次signal()
* 如果信号是异步的，即不是由abort()或raise()引起的，handler几乎不能调用库函数，除了abort()和quick_exit()，否则未定义。可以调用stdatomic.h里的函数、给静态volatile sig_atomic_t的变量赋值
  * sig_atomic_t是一种CPU可以原子访问的整数类型。一般C11后用atomic_int等类型，用_Atomic修饰自定义类型
* 引发信号后，进程不会立即处理，而是在恰当时机处理，比如内核态返回用户态之前
* 不能用于多线程程序，不要求它是线程安全的
...
将文件映射为进程地址空间的一块内存，写入时是写Page Cache，由内核管理生命周期。写入速度相当于写内存。即使程序挂了，只要内核没挂，数据依然会被写入磁盘。

要求地址空间是连续的。适合大文件随机读写。不太适合小数据反复追加，因为无缓冲（系统会处理页缓存但性能不如进程内的）；不适合扩/缩容，因为会重新映射。如果文件读写失败，普通函数会返回错误码，而mmap会触发信号。
