...
1. content
2. Formatting Model：position / top/right/bottom/left / float / display / overflow / z-index
3. Box Model：border / margin / padding / width/height
4. Typographic：font / line-height / text-align / word-wrap
5. Visual：background / color / transition / list-style / shadows / gradients
...
* 猫头鹰选择器（* + *）：选中除第一个以外的子元素，类似于前面提到的a+a，一般用于设置堆叠元素之间的margin-top，使得第一个元素没有上间隔。一般前面还要再加 > 因为一般不需要递归应用
...
* 继承性
  * 一部分和字体相关的属性和list-style等默认会继承自父元素，优先级非常低。如a有浏览器的颜色，就不会用继承的
  * 对某种属性可设置inherit值强制继承，且此时与优先级不冲突，如放在行内可覆盖原本其它只选中自己的优先级低的选择器的值而改为继承
  * 有的属性默认不会继承，如width、margin、border、display
  * initial为属性的默认初始值，不是浏览器的样式，不看元素。如display:initial就等于inline，即使是在div上也一样
* 优先级
...
* HTML5中对于某个HTML元素，已经不再有块级元素和行内元素这种说法了。对于CSS，有块盒/块容器
* display:none隐藏后不会占位置，visibility:hidden会保留位置
* 边距margin和padding
  * 外边距折叠：同一个BFC里，两个上下挨着的块元素的margin会合并，父子的相邻上下也会，效果是都以最大的那一个偏移。只在垂直上发生，不影响左右
    * 避免父子折叠：给容器加边框或内边距（一般有padding就不给子元素加margin了）、给容器加新BFC
  * inline盒的margin和padding对左右(水平)有效，对上下(垂直)不影响布局（不占据空间），仅影响背景。border同理，上下能显示边框但不占空间
  * 滚动条占用padding。outline和box-shadow占用margin，但不会改变盒子大小
* 边框border
  * border: 1px宽度 solid/dashed样式 颜色默认按字体的; 前两者也是简写，可以分别设置上下左右
  * border-radius: 50%变为圆形 100%表示1/4圆角
  * box-shadow 可用来做按钮hover时的阴影
  * border-boundary
* display:inline-block
  * 内部为块元素，本身为行内元素。可以设置宽高
  ...
  * img video表面上display是inline，但实际行为是inline-block
* BFC块格式上下文：内部的元素不受外界影响也不影响外界（包括里面元素的margin及折叠 padding 溢出）
  * overflow: 除visible
...
* inline盒不能设置宽高，只能用 字体大小、行高 间接调整
* 包含块(containing block)
  * 绝大部分情况下，盒子的包含块是其父元素内容盒
    * 块盒：一个块盒的总宽度必须刚好等于包含块宽度
  * 如果某些属性被赋予一个百分值，它的计算值是由这个元素的包含块计算而来的。即100%指的是父元素content的大小。但如果父元素的大小反而依赖于子元素就无效，如未设置height
* auto是默认值，表示使用所有剩余空间
  * 其实margin:auto也是这个意思，只不过默认值是0
...
* fit-content：平常说的收缩就是这种行为，如float inline-block。在不超过父级盒子宽度的情况下，尽量撑开盒子满足自己的需求
* stretch：平常的块元素就是这种行为，手动使用它主要用在inline-block上。Chrome138后直接支持，FF用-moz-available。但好像对body高度无效
* 如果html和body都用height:100%，能撑起高度，但默认body有margin
* 允许用户调整大小：resize:both 还需设置overflow
  * aspect-ratio: 16 / 9 用户调整时保持指定比例
* 图片（可替换元素）保持比例
  * 单个图片不同时设置宽高，默认就会用原始比例
  * 多个图片固定相同大小一齐展示时裁切多余部分：object-fit:cover; object-position: 显示哪一部分如left
  * 指定宽高且愿意让border内的地方留空：object-fit:contain
* box-sizing:border-box; 让border和padding算在height和width之内，也是IE的奇异模式。默认的content-box宽高仅指content

## 布局（视觉格式化模型）

* 浮动双栏布局：父元素建立BFC，可选指定宽度。容器内元素浮动，宽度50%
...
* absolute 以最近一个设置了position(除static)的内容为基准偏移，Fallback为viewport首屏。不占用原来的位置；宽度自动收缩；里面的元素自动视为display:block。一般设置父元素为relative，自身再设置top:0;right:0。当想把元素重叠时用到，如将图标放在特定位置
...
* 最好只用于图片嵌入文字，不要用于布局
  * 一种用途：div>div.left{跳转}+div.right{>}
* 脱离标准流，宽度自动收缩。同一方向多个元素之间无空格
* 仍在文档或容器中占据位置，把文档流和其它float元素向左或向右挤，并可能导致换行
* 想把容器撑起来：在末尾加一个clear:both的空div（或用伪类）。另一种方法：建立BFC
...
* 百分比：不能用在字体处吗？
* vh vw：viewport高度宽度。100vh就是整个网页内容窗口，但body默认有margin，且可能有滚动条，且会被手机端可隐藏导航栏覆盖。dvh：除去导航栏的高度，且当导航栏隐藏后重新计算，但在显隐过程中不更新导致高度突然变化。理论：如果允许滚动，则首屏用svh，后续满屏内容用vh。如果不允许滚动，用dvh
* vmin vmax：vw和vh中较小/大的那个
* ex：说是以字体的小写x的高为基准，实测不是，会受父元素字体大小影响；不同字体大小不同。ch：以0的宽为基准
...
* font-size: 10px;
  * 需要在Win显示的中文内容，字号应不小于12px
* font-weight: 400和700等价于normal和bold关键字，前者一般匹配Regular字重
...
* text-align: center、right。设为justify时可进一步调text-justify
* line-height：行内元素的高度，当确实用于设置行高时，用无单位值如1.5，会乘以文字大小
* letter-spacing
* text-shadow：内/外阴影/发光
* 禁止鼠标拖黑文字：user-select:none
* 容器内文字过多
  * overflow[-x/-y]: visible直接溢出出去 hidden没有滚动条，多的就不显示了 scroll永远出现滚动条 auto溢出时出现滚动条
  * 单行文本溢出时裁切或添加省略号：text-overflow:clip/ellipsis; white-space:nowrap; overflow: hidden
* 转换
...
* color：名称、井号加十六位数字、rgb(255 255 255 0.5)、hsl(0到360色相 0到100%饱和度 0到100%明度 0到1不透明度)适合创建一组相似的颜色风格。还有一种oklch，表示鲜艳度
* background
...
* 渐变：background: linear-gradient(方向 起始颜色 结束颜色)
  ...
  * 必须用bg，不能是bgc
* opacity：背景和文字的不透明度，且影响子元素（整体透明度）
* 给body设置bgc，在没给html设置时，会形成给html设置的效果
* colorsafe.co：根据背景色提供可读性强的文字颜色
* 自动根据系统设置调亮暗模式：:root{color-scheme:light dark;}，在设置颜色的值时用light-dark(亮颜色, 暗颜色)。如果不管系统，只用网页按钮调，就只留light或dark

## flexbox

* 给元素添加display:flex，称为弹性容器，默认宽度100%。它的直接元素变成弹性子元素
* flex-flow: row/column nowrap/wrap/wrap-reverse
* 直接子元素默认水平从左到右排列排列且无空白，高度默认自动伸展(align-items: stretch/center)；如果所有直接子元素的宽度的和超过父元素，子元素的宽度会自动收缩
...
* https://web.dev/articles/animations-guide?hl=zh-cn
* 只有确定性的值才能transition，如用相对值则不行

## 其它

* @import 'custom.css' 导入其他css，支持其他域名的url，配合媒体查询可有条件导入。但好像会等到HTML加载完才执行，导致无样式内容闪烁
* 媒体查询：@media screen and (max-width:799px){ 对宽度小于800的设备的CSS }、(prefers-color-scheme: dark)暗色模式。link元素media="(min-width: 40em)"只在满足条件时加载
...
* General Purpose
  ...
  * https://blog.fontawesome.com/introducing-web-awesome/ 好像是 https://github.com/shoelace-style/shoelace
* 非常轻量的(10KB)
...
* Material Design
  ...
  * material-components/material-web：谷歌官方，进入维护阶段了
  * muicss/mui：国产，目标是模拟手机端UI，最后更新时间2020年。好像要用react
  * MDUI：国产，WebComponents
  * https://forge.tylerdev.io Star数极少
...
* utility-first
  ...
  * Tailwind：封装了媒体查询支持响应式，一级支持grid，能自动清除无用的类减少体积。其它价值：https://www.bilibili.com/video/BV1e9MBzuEdY
  * unocss
  * twind 贡献者相对少
...
* https://creatie.ai/ 说是完全免费
* Lunacy， a decent free version https://www.softwareadvice.com/graphic-design/lunacy-profile/
* Penpot 开源，Web版 https://penpot.app/blog/penpot-ui-design-software-a-beginners-guide/
* Quant-UX 开源的 https://www.quant-ux.com/
* https://marvelapp.com/ basic版免费
* https://miro.com/pricing/
* https://pixso.net/pixso-vs-figma/
* https://motiff.com/figma-to-motiff
* https://www.invisionapp.com/product/prototype
* https://www.uxpin.com/pricing
* https://moqups.com/pricing/ 在线的
* https://www.justinmind.com/pricing
...
* https://getbem.com/ 对于命名的规范

---
https://marksheet.io/css-position.html

完整教程：https://web.dev/css?hl=zh-cn
...
* transition: property duration timing-function delay 伪类之间的切换没有过渡，用此属性设置平滑动画。如给元素(非伪类)设置transition: background-color 0.2s;。也可以直接设置时间
will-change
...
https://github.com/sdras/cssgridgenerator 不更新
