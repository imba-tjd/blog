...
* 结构伪类(CSS3)
  ...
  * :first-of-type 选中当前元素在其父元素下第一次出现的元素
...
* 调整列数
  * 默认一列，相当于竖向排布
  * grid-template-columns: 1fr 1fr 1fr 三列宽度平分。可指定px
  * 自动列数指定最小宽度：grid-template-columns: repeat(auto-fill, minmax(250px, 1fr))
...
* 异步加载：在link中加media="print" onload="this.media='all'"
