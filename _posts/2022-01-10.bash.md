...
* 扩展
  ...
  * ?、*、[...]：文件扩展，只有文件确实存在时才会扩展。其中星号不匹配点文件，只匹配当前文件夹下；启用globstar后用**/可递归匹配
...
* 换行符和转义
  ...
  * `echo "1\n2"`会输出1\n2
  * `echo -e "1\n2"`才有换行效果
  * 无法用\n字面量表示普通编程语言中的`"\n"`，在双引号内手动换行可以，之后用`echo "$s"`能显示换行，不必-e，而`echo $s`则以空格分隔显示
  * 一行的末尾加反斜杠能使得在下一行继续输入
  * `echo '\\'` -> `\\`，`echo "\\"` -> `\`，`echo $'\''` -> `'`
* printf：格式参数与C类似，不会自动换行；如果实参比格式参数多，会重用格式；如果缺少实参，%s会为空格，%d会为0
