...
* msys的终端，选择就是复制，中键粘贴
* Tab补全时不再区分大小写：bind "set completion-ignore-case on"
...
* https://github.com/rupa/z 快速跳转到过去cd到的地方，有fzf感觉不需要了。ajeetdsouza/zoxide：rust的实现，配合fzf提供tui消歧义

## zsh

1. sudo apt install zsh
2. 修改默认shell：chsh -s /bin/zsh或修改`/etc/passwd`（但其实上一步会自动修改所有的）。
3. 安装[Oh My Zsh](https://ohmyz.sh/)：`sh -c "$(curl -fsSL https://raw.github.com/robbyrussell/oh-my-zsh/master/tools/install.sh)"`。
4. 主题：`ZSH_THEME="ys"`或agnoster。
5. [高亮语法插件](https://github.com/zsh-users/zsh-syntax-highlighting)：`git clone https://github.com/zsh-users/zsh-syntax-highlighting.git ${ZSH_CUSTOM:-~/.oh-my-zsh/custom}/plugins/zsh-syntax-highlighting`，然后在.zshrc的plugins里加一行`zsh-syntax-highlighting`。
6. [解决`Insecure completion-dependent directories`](https://github.com/robbyrussell/oh-my-zsh/issues/6835)：对于非root，可以用它提示的`compaudit | xargs chmod g-w`，但是root就只能把`ZSH_DISABLE_COMPFIX=true`加到环境变量里了。
7. 添加GitHub仓库的alias和各个盘符的alias。
8. 禁用自动更新检测，加快启动速度：去掉`DISABLE_AUTO_UPDATE="true"`前的井号；手动更新可以用：`upgrade_oh_my_zsh`。静默自动更新：`DISABLE_UPDATE_PROMPT=true`。
