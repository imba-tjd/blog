...
```html
<meta charset="utf-8">
<meta name="viewport" content="width=device-width"> 适配手机端
<title>略</title>
...
使用emoji作为图标：<link rel="icon" href="data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 100 100%22><text y=%22.9em%22 font-size=%2290%22>🎯</text></svg>">
...
* target=_blank在新窗口打开，还可指定iframe的name。默认是_self
...
* section
  * 适合按功能分块、希望这个元素的内容体现在文档的提纲(outline)中时。如书的章节回目、多tab对话框的每个tab页、论文以数字编号的小节、报纸的板块（头版、国际新闻等）。网站的主页可能分成介绍、最新内容、联系信息等section
  * 一般需要有heading
  * 不是“语义化的div”
  * 如果原来是h1>h2，语义等价于h1>section>h1(原来的h2)
* aside：侧边栏，附加信息，如作者简介、相关链接、广告。与main同级
* footer：放版权信息。也可放在article中，放上下一篇文章的链接
* address：一般放在footer或section里

## video、audio

* 一般直接设置src属性，一般要设置宽高，一般要设置controls显示播放按钮进度条等
* 一般浏览器不允许自动播放，但允许静音自动播放：autoplay muted
* 其他属性：loop preload="auto/metadata" poster="p.png"
* 多格式：不写src，在里面放`<source src="v.mp4" type="video/mp4">`，字幕放`<track kind="subtitles" src="sub_en.vtt" srclang="en">`。如果没指定type，会从服务器检索mime TODO:测试
* 里面放p和文字为不支持H5时显示

### img、picture的srcset、media

* 响应式图片：picture用于不同的“艺术方向”，比如对于人物肖像，窄屏时只显示半身照，宽屏时使用人物两边有背景的图片；总之是两张不同的图。img的srcset和sizes用于同一图片的不同分辨率。另一种用处：使用了浏览器不支持的图片格式的回退，如AVIF
...
* 表单本身有name属性，控件也有
  * 前者作为document.forms[]的key，一般用不到可用id代替。后者可作为form对象.elements[]的key，且也会直接作为`form对象.key`
  * 后者，每个需要提交的输入字段必须设置name属性，否则就不会提交（还有其他规则，disabled的也不会提交）。将name作为key，控件的value作为值，发送给服务器
* 表单不能嵌套
* 小部件可以用在表单外，但行为要自己写
* action：提交时使用的URL
...
* progress：进度条，会产生“进行中”的动画，可不写value表示不确定进度。meter仪表用于静态的数据如cpu负载、磁盘使用率。max表示最大值，value为当前值
...
* output：存放本表单的控件的内容计算后的值

```html
<form>
  <label for="uname">Name:</label> # for属性可与指定id的控件关联，效果是点label会focus对应的控件；那个控件也可以放到label里面
...
* ol>li的value属性影响序号值，ol的reversed按相反顺序编号，start定义从哪个数字开始。li里可以有多个子元素如a+ul，样式上a的内容就是当前列表的，a“下”有一个子列表
  * dl、dt、dd：后两者类似于k v，放在dl中。默认样式是dd有一个tab缩进。语义上用于文献中的术语定义
...
* html：根元素，HTML5规定可以不写
* pre：不会进行空白折叠。实际上是CSS的white-space:pre。外面一般再加code
* map：设置name，在img中usermap="#name"。里面加area
* 避免出现 文字+img，推荐给文字加span，方便设置样式
...
* lang：该元素内部使用的文字是哪一种自然语言。简体中文用cmn-hans

## 其它概念

* 空格实体：`&nbsp;`，且是U+00A0 Non-breaking space，与普通空格U+0020不同
...
* lorem：生成一段假文。关键是可以作为重复元素的子内容展开。lorem1只生成一个单词
...
* 测加载速度：https://pagespeed.web.dev/ https://tools.pingdom.com https://www.webpagetest.org https://gtmetrix.com/
* https://www.wappalyzer.com/ 检测网页使用了哪些技术
* 测错：https://validator.w3.org/
* 安全：https://cheatsheetseries.owasp.org/ https://html5sec.org/ https://thecopenhagenbook.com/
* 压缩：https://github.com/terser/html-minifier-terser
