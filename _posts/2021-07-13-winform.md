...
* 内容需要new ListViewItem()，代表一行，设定Text（相当于第一列）、SubItems.Add()（其他列），再添加进本控件的Items属性中。其中SubItems[0]就是Text
* 控制表头：Columns.Add(new ColumnHeader())，AllowColumnReorder 允许用户拖动表头的列改变列顺序，HeaderStyle 可设为允许点击 不允许点击 不显示
* GridLines：行列之间显示网格线
* SelectedItems：当前选中的，SelectedIndices：当前选中的索引，MultiSelect：默认为false
* CheckBoxes：在每行开头显示复选框，CheckedItems：选中复选框了的集合，CheckedIndices：索引集合
...
* TODO: 只有ListBox和ComboBox有DataSource属性，其他的是DataBindings？
