...
* 默认情况下，一个winform窗体在不同分辨率屏幕下UI表现不一致。用Dock、Anchor、上面两种Panel这些可以缓解
...
```c#
// FX的设置方式。Main中调用，等于设置系统级别的感知，不支持XP
[System.Runtime.InteropServices.DllImport("user32")]
static extern bool SetProcessDPIAware();
SetProcessDPIAware();

// 另一种设置方式，app.config(不是manifest)中：
<configuration>
  <System.Windows.Forms.ApplicationConfigurationSection>
    <add key="DpiAwareness" value="PerMonitorV2" />
  </System.Windows.Forms.ApplicationConfigurationSection>
</configuration>
// PM2宣传说4.7开始支持，会同时启用多个单条目如EnableWindowsFormsHighDpiAutoResizing，理论上对于新项目不用自己处理某些控件了。但实际一直到.NET8还在改进
// 还必须在manifest中supportedOS启用Win10
...
* 资源
  ...
  * cs中用 Properties.Resources.属性名 或 .ResourceManager.GetObject强转/GetString/GetStream 获取
    * 另一种读取方式：Assembly.GetExecutingAssembly(); foreach(as.GetManifestResourceNames()){as.GetManifestResourceStream(name)}
...
* 指定运行时的FX版本，可以让高target运行在低rt上，也可以反过来：程序名.exe.config `<configuration><startup><supportedRuntime version="v4.0" sku=".NETFramework,Version=v4.6.1"/></startup></configuration>`，如果要加载某些fx2的库，再用useLegacyV2RuntimeActivationPolicy
...
* https://roslynpad.net/ 必须装CoreSDK
* https://www.linqpad.net/Purchase.aspx https://linqpad.cn/Download.aspx 闭源，有免费版但无补全。o.Dump()在结果中打印o，且在代码中原样return o
* https://github.com/tareqimbasher/NetPad Electron写的，未来可以用WebView。界面与linqpad类似
