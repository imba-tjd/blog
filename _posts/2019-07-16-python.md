...
* hex(16) -> '0x10'，bin(7) -> '0b111'。int('123'或b'123') -> 123，int('0x10'或'10',base=16) -> 16，base=0自动按前缀解释。ord('A') -> 65，chr(65) -> 'A'
...
* subprocess.run(['ls', '-l'])
  ...
  * text=True：默认stdin stdout input都是字节序列，二进制流不转换编码和行尾，设定此项后可用字符串，Win下以gbk解码。另一种方式：encoding参数
  * env：dict
...
logging.debug、info、warning、error、critical # 第一个参数字符串为pattern，用其余参数格式化
...
* queue：Queue（FIFO的）, LifoQueue, PriorityQueue
