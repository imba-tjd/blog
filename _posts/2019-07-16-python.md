...
t = asyncio.create_task(f1()) // 调度执行
r = await t
r = await asyncio.gather(f1(), f2()) // 批量等待并获取结果
t.result() // 不具有等待功能，如果未完成，会抛InvalidStateError异常
t.exception() // 同上
t.add_done_callback()

asyncio.wait()不同于一般意义上的wait
try:
    await asyncio.wait_for(eternity(), timeout=1.0)
except asyncio.TimeoutError:
    print('timeout!')

asyncio.to_thread(blocking_io) // 其中blocking_io是普通函数，此方法相当于把threading包装成async

loop = asyncio.get_running_loop() // 不推荐用get_event_loop()
...
* https://stackoverflow.com/questions/49330905/how-to-run-a-coroutine-and-wait-it-result-from-a-sync-func-when-the-loop-is-runn
