...
* 服务端优先使用socketserver.ThreadingTCPServer/ThreadingUDPServer，创建时传((IP,PORT), MyHandler)，其中MyHandler继承StreamRequestHandler/DatagramRequestHandler，def handle(self):读取self.rfile，写入self.wfile，两者都是file-like-obj
* 源地址用('',0)会使用操作系统的默认行为
* shutdown()：应在close()前使用，关闭本端的连接。socketserver在serve_forever()中时要在另一个线程使用它
...
s.bind(('', 8080)) # 接收
bytes_data, client_ip_port = s.recvfrom(1024) # 最大65507
...
t.daemon = True # 默认为前台线程，即使主线程结束了，也会等待它结束
t.start()
