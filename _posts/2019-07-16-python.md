...
# coding: u8
# 末尾用\可换行（如果在括号内就无需），单行用分行可有多条语句，单行注释无法在\换行符后
...
* 类型转换：int('123') -> 123，ord('A') -> 65，chr(66) -> 'B'
...
* or和and可用在对象上：or返回第一个为真的操作对象，都为假就最后一个；and返回第一个为假的对象，都为真就最后一个。`-5 or ... -> -5`，`0 and 1 -> 0`，`1 and 2 -> 2`。注意取不存在的属性和方法时是抛的异常而不是返回None，因此无法和这几个连用。优先级and更高，但都比+低
* a = b if b is not None else c：三元运算；和数字比较时要用!=
* 1 < 2 < 3：支持连续比较；False is False is False -> True，这说明它执行的其实是A op B and B op C，而不是(A op B) op C
...
* 成员函数：replace（可指定最多替换次数）、translate（传递一个字典用于替换）、strip/lstrip/rstrip(按字符去除)、removeprefix/removesuffix(去除字符串)、count(指定字符串出现的次数)、find/rfind(即indexof)、split、startswith(支持元组指定多个)、isalnum(所有字符都是字母或数字)、center/ljust/rjust(30)（居中/左对齐/右对齐）
...
f'{0x11111:_x}' # 按16进制输出且每4位加下划线，但没有直接输出0x前缀的方法
f'{3.14:10.3f}' # 宽度为10，3位小数，不加f就是有效数字，对于字符串就是截断
r'\' # SyntaxError: EOL while scanning string literal；因为解释器太差了，仅仅是放行反斜杠及其后一个字符，这还导致r'\''是有效的
```
...
(a,b) == (1,2) # 表示a==1 and b==2

# 命名元组，可以通过名称（点）访问，也可以用[offset]访问；仍是不可变的，不能赋值；可用_asdict()以有序字典表示内部信息
...
* 类型转换：int、float、str、bool；其中int()支持base参数指定输入的进制，默认为十进制，int('010')就是10，int('0x10')会抛异常，指定base为0才会按前缀解释
...
* sorted：从小到大排序，可设置reverse=True，key参数为单参数函数，(Timsort)排序是稳定的，反向排序的结果不等于正向排序后再普通反向、reversed->迭代器、any/all->bool序列是唯一参数，评估对象是否为True
* pow：支持第三个参数取模、round：四舍六入五成双/凑偶(0.5为0，1.5为2)，第二个参数为保留几位小数，且也可为负，-1就是保留到10，但因为浮点精度问题round(-1.225,2)为-1.23、divmod：同时获得除数和余数、abs：支持整数和浮点、max、min、hex
* sum：第二个参数为起始值且不能是字符串，sum([[1,2],[3]],[])可以解包一层列表，但每次都会浅拷贝一遍，10个以下可用
* math.sqrt、factorial、log（默认以e为底，小心别用==比较）、sin、hypot（勾股定理）、ceil、floor、degrees（弧度转角度）、radians、pi、fsum（对于浮点数比sum精度高）、copysign(x,y)x的值y的符号
* assert：其实是关键字，可不用括号而用空格，第二个参数可加信息
* range
* ascii：把非ascii字符变成转义序列，如`ascii('你好')`->`'\\u4f60\\u597d'`
* eval：把字符串解析为表达式，不能有赋值语句和流程控制语句，`:=`也不行，但却可以是返回None的函数；剩下两个参数可指定函数内临时的global和local字典，如果都为空就会使用当前作用域的。exec：可执行语句，没有返回值，可以有赋值语句且会影响locals()。execfile废弃了

## 类

* 以单下划线开头的成员对象是私有的，但这只是一种约定；双下划线开头会以固定规则修饰其名称，达到用原本名称访问报错的目的，此时在类外对其赋值仍会成功，但这是创建了一个新的对象
* 访问一个属性时，如果对象属性不存在，找类属性，还不存在就找父类属性。但对于赋值就会在自己范围内创建新对象：对象属性不存在时，修改类属性会影响对象属性，创建对象属性后就无关了
* 类方法用@classmethod修饰，作用于整个类，一个参数cls是类本身，可被调用即为构造函数且继承后没硬编码为父类；类外调用时用类名.方法名()，也可用对象名，但仍是与类绑定
* 对于不需访问类的任何属性的方法，用@staticmethod
* 多继承，若有同名方法左边优先，所有的直接父类都没有再向上找；super()会按照类的mro()顺序访问，菱形继承时左边父类的super返回的是右边的父类而不是父类的父类
* 还有个元类metaclass的特性，太复杂了

```python
class Student(parent): # 继承，默认object，本类声明完成后即使parent被重新赋值也不会变
    count = 0  # 类属性，类中所有实例都能访问同一个，仍要加self；或在类方法里用cls.count访问

    __slots__ = ('__name', '__score') # 限制该class的实例能添加的属性，子类要重新定义一遍，如果有weakref目标也要放进去；如果是为了省内存，实例小于1万不值得使用
    ...
    def __init__(self, name, score): # 构造函数
        ...
        self.__score = score
        Student.count += 1 # 类属性；如果用=，不提前声明也可以，但每次实例化都赋同样的值没任何意义
...
f = StringIO('hello') # 当作文件用，类似于stream和StringBuilder
...
shutil.copy()dest可以是目录，就与cp一样,copy2()尽力保留元数据,move(),copytree(),rmtree(),make_archive(),unpack_archive()
```
...
logging.disable(logging.INFO) # 关掉所有此级别及以下的输出
```

### 日期和时间

* calendar.isleap()：是否是闰年
* datetiem模块：date用于日期，time用于时间，datetime用于时间和日期都有，timedelta用于间隔，tzinfo为时区信息；另外还有一个单独叫做time的模块；3.9出了zoneinfo模块
...
* timeit.timeit('f()',globals=globals())，number默认100万
...
* re.search(pattern, str)：以Match对象返回匹配到的第一个str，无匹配返回None，m.group(n=0)获取匹配到的str，n=1获取对应的组，m.span(n=0)返回匹配到的起始和结束位置，m.groups()获取子捕获组，等价于`(m.group(1),...)`；re.match()：从字符串开头进行匹配
...
* deque(seq,maxlen)：双端链表队列，也适合做栈，满的时候自动去掉另一端的。pop去掉并返回最右边的，poplift去掉最左边的，extendleft，rotate当参数大于0时将最右边的元素移到最左边
* OrderedDict：可以按字典中元素的插入顺序来输出，但构造函数初始化时的仍是无序的，必须要插入才行。move_to_end(),popitem()
* defaultdict(int)：取任何不存在的key时都返回一个默认值，构造参数需要是一个函数，可为lambda，传int就是0，传`list`就是`[]`
...
* product多个可迭代对象的笛卡尔积，repeat=2相当于把前面的内容多写一遍，permutations可迭代对象的元素的全排列，combinations组合
...
* filter：相当于Where，但不快，不如用列表生成式。内置。filterfalse作用相反
* 偏函数（固定参数的函数）：int2 = functools.partial(int, base=2)
* operator包含一系列对应于操作符的函数，就不用自己写lambda了；可以和partial结合使用
* map：基本相当于Select，只能迭代一次所以组合使用可能出问题。不同在于它可以接受多个序列，例如map(pow, [1,2], [3,4])结果是[1, 16]，迭代到如果后者更长则前者就会为None。更适合函数已经定义好了：map(str, range(20))，与(str(n) for n in range(20))
...
def check_ip_port(host,port): # 是否能连接
...
* 对象必须是pickleable的
* 不受GIL的数据结构：Queue(), Array(), Value()，或用with Manager同一管理
...
* struct.Struct('i?f').pack(23, False, 42.0).data：用于与C/C++的struct交互，帮助把它们转换成Python数据类型；第三方库也有一些但star数都很少
...
* hashlib：md5('你好'.encode(encoding='u8'))只接受bytes；hexdigest()变为16进制字符串
...
* random：randint(from,to)闭区间；.random()浮点[0.0, 1.0)；uniform(a,b)b小于a也可的闭区间浮点；.choice(seq)随机返回一个元素，choices放回采样多个元素（可重复），.sample不放回随机获取多个元素；.shuffle改变原有序列顺序
* copy.deepcopy()深拷贝
...
* memoryview.cast,tolist；stuct：操作二进制数据
* queue：Queue，LifoQueue，PriorityQueue。多线程编程时使用，满的时候锁住不丢弃，直到另外的线程移除某个元素，适合控制活跃线程的数量
...
* [i async for i in aiter() if i % 2]；[await fun() for fun in funcs if await condition()]
