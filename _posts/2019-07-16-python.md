...
* ssl.create_default_context() 客户端可以直接用
  * Purpose.CLIENT_AUTH不是用作客户端的，恰恰相反，此选项是用于服务端的。其余几个可选参数是证书，默认加载系统的；会根据Purpose只加载用于服务器或客户端验证的
...
* 算法
  ...
  * heapq堆排序：以数组为底层储存，小顶堆，想要大顶堆可对值取负。heapify(seq)，heappush(seq,elem)/heappop，merge(l1, l2)合并多个排序好的序列，nlargest/nsmallest(n,seq)最大/小的几个数。另见queue.PriorityQueue
