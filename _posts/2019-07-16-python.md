...
* iter(可迭代对象) 获得迭代器，实现了`__next__()`，可用next()手动消费，没有更多的元素时会抛出StopIteration。迭代器也实现了`__iter__()`，即也是可迭代对象，但它什么也不做，就返回迭代器本身
...
* uuid：3.12。uuid4是纯随机；5根据提供的两个参数生成，相同时会相同。uuid7（未实现）能根据时间排序
