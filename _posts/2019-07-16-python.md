...
* 事务隔离isolation_level：默认是DEFERRED，会在DML前隐式BEGIN开启事务，提交时在Py中commit()即可。设为None后底层仍为autocommit模式，每一句都自动提交，但执行BEGIN或SAVEPOINT后就不会自动提交，直到COMMIT/ROLLBACK/RELEASE。所以DEFERRED并没有改底层模式，只是自动BEGIN了
* connect设置uri=True能启用URI Filenames，设置detect_types=sqlite3.PARSE_DECLTYPES能根据类型声明解析数据
* 参数化用?占位，用元组传入；命名参数用:xxx并传dict；不能用'?'因为那是字面量
* 默认等待解锁超时为5秒，可认为不会有锁住的问题
* 默认check_same_thread为True，使得连接不能传入另一个线程使用
