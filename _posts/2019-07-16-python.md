...
* 转义
  * '\u4f60\u597d' -> '你好'，\U后接八位字符可表示所有Unicode字符
  * 与字面量形式的转换：'你好'.encode('unicode_escape') -> b'\\u4f60\\u597d'，'\n'.encode('unicode_escape') -> b'\\n'，再decode()就能变成'\\n'；反过来用encode().decode('unicode_escape')，或用codecs.decode(str,'unicode_escape')可直接传str
  * 遇到本应是bytes的str：'\xe6\x88\x91'.encode('latin-1') -> b'\xe6\x88\x91'，再decode()
  * ascii('你好') -> "'\\u4f60\\u597d'"，ascii('\n') -> "'\\n'"，类似于repr()
* bytes
...
* 如果信息从头到尾没有被任何handler处理，会以默认format输出WARNING级别的信息；注意调过过basicConfig后就不是这样了
...
logger.setLevel(...) # 支持'INFO'这样的大写字符串，level属性就只能是logging.INFO这样的数字常量了
...
subparsers = parser.add_subparsers(); create_parser = subparsers.add_parser(); create_parser.add_argument() 子命令
```
...
* hashlib.md5('你好'.encode()).update(b'继续添加内容').hexdigest()返回16进制字符串，也是Linux的md5sum命令的结果。BLAKE2算法速度稍快，32位用blake2s，但现在出了3且3.10的标准库还没集成
* hmac.new(key, data, 'sha256').hexdigest()返回16进制字符串；hmac.digest()返回bytes。hmac.compare_digest()比较两个str/bytes并避免时序攻击
