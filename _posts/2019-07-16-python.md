...
* setblocking(False)：启用非阻塞模式，可配合select或selectors模块使用，但太复杂了
...
s.bind(); s.listen() # 接收。有便捷函数create_server()，支持设置双栈和复用端口
...
* 仅用于IO绑定，无法利用多核
* 同步用的对象
  * Condition条件：用于生产者消费者，或等待队列-唤醒，避免busy-waiting。一般wait_for(predicate函数)，生产者调用notify()或notify_all()，关键是需要消费者提供函数自己检查条件是否成立
  * Semaphore信号量：用于保护数量有限的资源，或用作计数器。BoundedSemaphore：若资源量增加到高于初始值时会抛异常
...
* 指针：POINTER(c_int)。若为数组，在Py侧遍历时用切片
* HANDLE：ctypes.wintypes.HANDLE
* 结构体和位域：创建class继承STRUCTURE，再用某种规则创建`_fields_`字段
...
cdll.ucrtbase.xxx; windll.kernel32.xxx # windll自动按stdcall，A和W版本不会自动选择。已知shell32和ntdll也是预定义的，其它的用windll.LoadLibrary()
pythonapi # 预定义的Python C API

i = c_int(42); i.value = 24 # 可变
libc.scanf(b'%d', byref(i)) # 如果传Py对象的指针，用pointer(obj)，用contents属性返回指向的c_xxx对象，它也可以取下标
c_char_p('Hello, World') # 还有c_wchar_p
p = create_string_buffer(b"Hello", 10) # 两个参数都可只用其中一个，用于被外部函数写入，p.value视为以\0结尾的str解析
f.argtypes = [c_double] # 声明参数，类似于原型。什么都不做时传Py对象，bytes能转换成char*，整数转换成int，其余的都会报错
f.restype=c_double # 返回类型默认都是int，必须手动改
```
