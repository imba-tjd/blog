...
* 两个整数用`/`除仍是浮点数：9/3 -> 3.0；整除/地板除/向下取整用//，3//2 -> 1; -3//2 -> -2（C是-1）；-3%2 -> 1，即符号与后者一致，而C与前者一致，是-1；这样C与Py都能做到i == (i//j)*j + (i%j)
...
* a = b if b is not None else c：三元运算
...
* 加b表示bytes：`'我'.encode(['u8'])` -> `b'\xe6\x88\x91'`，反过来用decode()；bytes不可变，但bytearray(bytes)可变；字符串只有ascii码可直接加b，无需encode
...
* 检查参数类型：isinstance(x, int | str)，x是第二个参数的子类也能通过，type()就没有后者的效果
...
* encoding默认是gbk，坑。启动Py的时候加-X utf8或设置环境变量PYTHONUTF8=1可启用纯UTF-8模式
...
exists(),is_file(),is_dir(),samefile(),iterdir()类似于os.scandir,glob()以shell规则匹配文件和目录,rglob()相当于自带**/,resolve()把相对路径变成绝对路径
...
* subprocess.run(['ls', '-l'])
  ...
  * capture_output=True：设定后可用.stdout获取输出，否则默认会继承父进程即默认直接输出到屏幕上；设定了此项就不要再设定stdout和stderr了
  * stdout=subprocess.DEVNULL可忽略输出
  * stdin：可设为另一个命令的stdout达到管道的效果
  * text=True：默认stdin和stdout都是字节序列，设定此项后可用字符串
  * env：dict
...
* con也有execute等，且文档建议就是直接用它，会返回对应的cur
* 能创建自定义的函数和聚合函数，本身为python函数，但是能在sql语句中执行
* executescript可执行多条sql语句（含有分号）
...
cur.fetchone()/fetchmany()/fetchall() # 每一个row再是一个list
...
* heapq.heapify(seq).heappush(seq,elem)、merge(lst1, lst2)：堆排序
