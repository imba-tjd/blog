...
* 算法
  * bisect.bisect_left：二分查找返回索引，如果target不存在，仍会返回使得插入后保持有序的索引而非-1。insort二分插入
  * heapq堆排序：以数组为底层储存，小顶堆，想要大顶堆可对值取负。heapify(seq)，heappush(seq,elem)/heappop，merge(l1, l2)合并多个排序好的序列，nlargest/nsmallest(n,seq)最大/小的几个数，seq[0]就是最小的数相当于peek。另见queue.PriorityQueue
