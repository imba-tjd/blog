...
* 3.7后规定遍历顺序按插入顺序。但用==判断时不按顺序，需要这个也有序可用collections.OrderedDict
...
* hmac.digest(key, data, 'sha256') -> bytes。想返回文本要么base64要么hmac.new().hexdigest()。compare_digest比较两个str/bytes并避免时序攻击
...
* random：randint(from,to)闭区间，相当于randrange(from,to-1,1)，random()浮点[0.0, 1.0)，uniform(a,b)b小于a也可的闭区间浮点，choice(seq)随机返回一个元素，choices放回采样多个元素（可重复），sample不放回随机获取多个元素，shuffle原地改变原有序列顺序；secrets模块：生成安全性更高的随机数；os.urandom()使用系统提供的随机数，无法用自己的seed，secrets和uuid模块内部使用了它
* copy.deepcopy()深拷贝
