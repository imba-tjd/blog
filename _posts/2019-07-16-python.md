...
# 命名元组，可以通过名称（点）访问，也可以用[offset]访问；仍是不可变的，不能赋值；可用_asdict()以有序字典表示内部信息，自带slots
...
* pow：支持第三个参数取模、round：四舍六入五成双/凑偶(0.5为0，1.5为2)，第二个参数为保留几位小数，且也可为负，-1就是保留到10，但因为浮点精度问题round(-1.225,2)为-1.23，round(2.675,2)为2.67、round(2.685,2)为2.69、divmod：同时获得除数和余数、abs：支持整数和浮点、max、min、hex
...
* 在类外访问一个对象的属性时，如果对象属性不存在，找类属性，还没有就找父类属性。在类外赋值会在自己范围内创建新对象：对象属性不存在时，修改类属性会影响对象属性，创建后就无关了
...
* 多继承/菱形继承的查找顺序按照类的mro()，左父类的super()返回的是右父类而不是父类的父类，类外自动查找对象顺序也一样
...
f = StringIO('hello') # 当作文件用或者类似于流，可以readline
...
* re.search(pattern, str)：以Match对象返回匹配到的第一个str，无匹配返回None，m.group(n=0)获取匹配到的str，n=1获取对应的组，m.span(n=0)返回匹配到的起始和结束位置，m.groups()获取子捕获组，等价于`(m.group(1),...)`；re.match()：从字符串开头进行匹配；finditer()返回Match迭代器，即相当于多次search
* re.sub(pattern, repl, str)：替换，其中repl可以是str或可调用对象，第三个参数可指定替换最大次数，第四个参数才是Flags
