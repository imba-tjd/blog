...
* 镜像：https://mirrors.huaweicloud.com/python/
* mingw版：https://github.com/msys2-contrib/cpython-mingw Actions里有。或下msys2的ucrt包，还要复制libgcc_s_seh-1.dll
* 支持win7版：https://github.com/adang1345/PythonWin7
* 构建了security版：https://gregoryszorc.com/docs/python-build-standalone/main/running.html https://github.com/adang1345/PythonWindows
* embedded版添加pip：修改python311._pth去掉import前的注释，下载运行 https://bootstrap.pypa.io/get-pip.py 用python -m pip运行。另一种绿色版：https://www.nuget.org/packages/python
* 绿色版：https://winpython.github.io/
...
* 商店的Python Install Manager：pymanager install 3.13 --update

### VSC

* python.analysis.inlayHints：variableTypes:true functionReturnTypes:true callArgumentNames:partial
* python.analysis.completeFunctionParens
* debug.inlineValues:on
* https://docs.basedpyright.com/ 是Pyright的fork，添加了一些pylance的功能
* https://github.com/astral-sh/ruff An extremely fast Python linter, written in Rust. 代替Flake8 Black isort。LSP：ty
* https://pyrefly.org/ FB出的LSP，也有Lint功能
...
* 功能
  ...
  * miniconda只包含了conda、python、和一些必备的软件工具；实现上是个包含了二进制数据的sh，大概130MB，交互式安装。anaconda包含了数据科学和机器学习要用到的很多软件
    * 它俩都禁止用于公司大于200人的商业用途。conda虽然开源，但anaconda的仓库禁止商业用途，但conda-forge可以用
  * conda用来安装conda package二进制包，大部分是python的，但也支持了不少其他语言的依赖项如mkl、cuda。可以安装gcc且不需要root权限。有些包只能用conda，比如rdkit。包的总数远少于PyPI
...
* miniforge：类似于miniconda，也是用来安装conda和mamba的sh，预设了默认channel为conda-forge。曾有Mambaforge但现在合并了
...
* 两个整数用`/`除仍是浮点数：9/3 -> 3.0；整除/地板除/向下取整用//，3//2 -> 1; -3//2 -> -2（C是-1）。-3%2 -> 1，即符号与后者一致，相当于+mod成正数再取余；与C不同，C与前者一致，是-1；这样C与Py都能做到i == (i//j)*j + (i%j)
* 赋值可以`a,b=b,a`，其实是元组；可以连续赋值`a=b=1`，计算最右边的式子后从左到右赋值；不具有返回值，即不能`if a=True:`，但可用`:=`
* REPL中下划线保存了上一个表达式的值
* 0bxxx是二进制数字，0o八进制，0x十六进制，中间可以使用下划线提高可读性
* hex(16) -> '0x10'，bin(7) -> '0b111'，k进制np.base_repr(n,k)。int('123'或b'123') -> 123，int('0x10'或'10',base=16) -> 16，base=0自动按前缀解释。ord('A') -> 65，chr(65) -> 'A'
...
* or和and可用在对象上：or返回第一个为真的操作对象，都为假就最后一个；and返回第一个为假的对象，都为真就最后一个。`-5 or ... -> -5`，`0 and 1 -> 0`，`1 and 2 -> 2`。注意取不存在的属性和方法时是抛的异常而不是返回None，因此无法和这几个连用
...
* 链式赋值的推进问题：a = b = c，会先给c求值，然后从左往右推进。例如合并两个有序链表中p = p.next = head1会出错，要写成p.next = p = head1才行
* 1.0 / 0：抛异常。其它大部分语言是∞
* 优先级
  * `-1**2 == -1`，因为乘方优先级更高
  * 位运算符优先级大于比较运算，==和>=的优先级相同。都与C不同
  * not > and > or，但都比+低。C的一元逻辑非优先级很高
...
* locals()和globals()分别返回一个局部/全局空间内的字典，前者在3.13时只收集一次，修改用sys._getframe().f_locals字典；全局字典中还含有魔术对象
* 变量只是名字/标签，b=a后id(b)等于id(a)
...
* 引号前加r为raw string；末尾不能是反斜杠如`r'\'`，因为解释器仅仅是放行反斜杠及其后一个字符，这还导致`r'\''`能有效表示一个反斜杠加一个单引号的字符串
...
* bytes
  ...
  * bytes(4) -> b'\x00\x00\x00\x00' 无意义，不是把int转bytes的。可用int.to_bytes()和from_bytes()或struct包
* codecs
  * utf-8-sig带有BOM
...
* str成员函数
  ...
  * split：无参时把多个空格视为一个且去掉开头结尾的空格，' '.split()->[]；若手动传' '结果中可能出现''，优点是使其与join互逆。不支持split('')，用list()即可。另有splitlines()
  * startswith：支持元组指定多个
  * isalnum：所有字符都是字母（或中文）或数字，即没有英文标点或空格
  * center/ljust/rjust(n)：居中/左对齐/右对齐
  * zfill(n)：在数字字符串左边填充0直到包括负号和小数点后n个字符
* textwrap.dedent('''\换行)：去掉多行字符串前面的空白
...
for name in names: # in range(n)可产生顺序序列达到类似C的for的效果；in 1,2实际上是遍历元组。变量在出循环后仍有效
for i, value in enumerate(['A', 'B', 'C'], starti): # 把序列变成索引-元素对的序列。若正常结束，i不会等于长度，而是等于最后一个索引，若要处理一般配合else
...
* 把列表推导式最外面改成小括号是生成器generator表达式，是惰性的。for和list+=的时候会消费，只能消费一次；转换成list要用`list()`而不能用`[]`
  * in子句在声明时绑定对象，条件子句和结果则是在消费时执行
  * 作为函数参数时可不加括号
* 函数中yield也是生成器
  * 函数外send()可以给函数内的yield设定返回值，然后继续next()的效果。yield(yield)就是返回内部send的值
  * 存在yield的函数中遇到了return会停止迭代并抛StopIteration。手动迭代时调用方可在e.value中获得return的值，在list()中时会自动吃掉；一般这种情况是想返回序列，那再for一遍yield就好了，或用yield from
  * for x in y: yield x 等效于 return (x for x in y)。二层for yield等效于外层for: yield from (内层for)
* 可迭代对象Iterable实现了`__iter__()`，可以for in。包括两类：集合数据类型(list, tuple, dict, set, str)和生成器
* iter(可迭代对象) 获得迭代器，实现了`__next__()`，可用next()手动消费，没有更多的元素时会抛出StopIteration，或第二个参数指定默认值。迭代器也实现了`__iter__()`，即也是可迭代对象，但它什么也不做，就返回迭代器本身
* 判断对象是否是可迭代的：isinstance([], collections.Iterable)，或`hasattr(x, '__iter__')`
...
* itertools
  ...
  * compress：将序列按第二个参数提供的mask来filter
  * accumulate：最终结果与reduce一样，区别是此函数会迭代返回每一次的结果，且默认回调为加法
  * batched：3.12，将序列分隔成固定大小的元组
* functools
  ...
  * @lru_cache：根据参数值缓存最近几次返回值，默认大小128；@cache等于无限大小，@cached_property需3.8
...
* 字符串转dict可用eval()，但必须是python的dict；或用JSON
...
dict(a=1, b=2) # 这样key必须不能加引号，但value正常
...
min(d, key=d.get) # Bob，即根据key参数进行比较，返回字典的key
```
...
* sorted
  * 非原地从小到大排序返回list
  * key=lambda x:x.p 表示按p属性排序，-x 表示从大到小排序，str.lower 忽略大小写排序
  * 多字段排序：返回元组。复杂逻辑，或像传统比较函数那样接受两个参数：cmp_to_key(lambda a,b: a.x-b.x)。或类中定义`__lt__(self,other)`
  * 算法为Timsort是稳定的，reverse=True不等于正向排序后再反向
* pow：支持第三个参数取模。divmod：同时获得除数和余数。abs：支持整数和浮点。max、min：有重载，既支持多参，也能传iterable且可指定默认值否则序列空时抛ValueError
...
* range
* reversed->迭代器
* eval：把字符串解析为表达式，不能有赋值语句和流程控制语句，`:=`也不行，但却可以是返回None的函数；剩下两个参数可指定函数内临时的global和local字典，默认会使用当前作用域的，设为{}去掉环境，但仍可用builtins的东西，设为`{'__builtins__':None}`会安全不少。exec：可执行语句，没有返回值，可以有赋值语句，在模块里运行会影响globals()，但在函数里运行对变量赋值，结束后不会改变locals()。execfile废弃了。`eval(compile("5 + 4", "<string>", "eval"))`执行编译后的字节。安全版：pydantic/monty
* issubclass
...
* 把第一个参数为self的函数绑定到对象，不过一般直接放到类里不用这么做：o.fun=types.MethodType(fun, o)
...
* super(M,self)
  ...
  * super实际上是个类，无参调用时Py3黑魔法自动传递了它所在的类和对象。也可以在类外普通地使用
* new
  * 第一个参数还是类本身，调用`self=super().__new__(cls)`产生的就是类实例，之后必须有return
...
* enter、exit：使对象支持with语句，其中enter一般返回self，exit有多个参数
  * 如果对象自己没支持with但是存在close()，可用contextlib.closing()
  * 使得函数支持with：@contextlib.contextmanager，中间的yield就是返回给调用者的对象执行用户代码：`<setup> try: yield <value> finally: <cleanup>`
  * 管理多个ctx：contextlib.ExitStack
* 自定义eq时不会再自动继承hash
...
class Color(Flag): # 3.11增加boundary=STRICT拒绝无效值
    Black = 0
    Red = auto(); Green = auto(); Blue = auto() # 自动按二进制1递增
    White = Red | Green | Blue # 7

@unique # 定义期检查无重复值
class Weekday(IntEnum):
    Sun = 0; Mon = 1; Tue = 2 # 注意不是逗号
    Fri = 5; Sat = auto() # 按最高值+1
```
...
move()不会自动新建文件夹,copytree(),rmtree()类似于rm -rf但只用于文件夹,unpack_archive()
```
...
* subprocess.run(['ls', '-l'])
  ...
  * check=True：非0退出抛CalledProcessError as e; e.stderr；不启用时也可用.check_returncode()手动抛
  ...
  * text=True：默认stdin stdout input都是bytes，二进制流不转换编码和行尾，设定此项后可用字符串，Win下以gbk解码。另一种方式：encoding参数
  * env：dict
  * stdout/stderr/stdin：设置流，一般不用
  * 可执行文件不存在时抛OSError。Win下后缀如果省略，只支持exe，不支持bat
  * 会等程序执行完。如果要不等待，用Popen
* subprocess.call、check_output、check_call：3.5之前使用；getoutput、getstatusoutput是更旧的函数
* os.system()：该函数相当于创建了一个新的shell；os.popen().read/readlines()：创建管道，fork子进程，获取输出，基于流，stdout用完了要关闭，不如使用asyncio.create_subprocess_exec。另外system('')能在cmd和老ps下开启虚拟终端，使得颜色转义生效
...
* 流程
  ...
  2. Level默认NOTSET，会继承第一个不为NOTSET的父级的级别直到root。
  3. 如果信息级别低于自己的Level，直接忽略信息。
...
logging.debug、info、warning、error、critical # 第一个参数字符串为pattern，用其余参数格式化，若含有换行会转义成可见字符
exception：只能在异常处理块中使用，自动记录捕获了的异常的traceback，自己只需给自定义信息，级别error

logger = logging.getLogger(__name__) # 某一名字的记录器创建以后就一直存在，多次调用也返回同一个。小心别写成logging.Logger()
logger.setLevel(...) # 支持'INFO'这样的大写字符串；level属性只能用数字常量logging.INFO
handler = logging.FileHandler('output.log', encoding='u8') # StreamHandler(sys.stderr)、RotatingHandler自动归档
handler.setFormatter(logging.Formatter(format)) # handler也有个setLevel，作用与logger不同。style='{'支持改为fmt风格
logger.addHandler(handler)
logging.disable(logging.INFO) # 关掉此级别及以下，优先级比logger.disabled高；默认值为CRITICAL
优化性能：if logger.isEnabledFor(logging.DEBUG): logger.debug(expensive_fn())
...
* datetime模块：date，time，datetime，timedelta，timezone固定时差的时区。zoneinfo模块(3.9)处理按城市指定的时区，tzinfo是两者的抽象父类
  * hashable、pickleable
  * 相同类型可直接用 < > 比较
  * 不可变，那些修改的函数都会返回新对象
* date.today() .day/.month/.year
* datetime.now()简单型本地时间 / now(timezone.utc)感知型UTC时间 / now().astimezone()感知型本地时间 / 构造函数 / combine(date,time)
  * dt.replace(hour=xx) 改变参数中指定的字段，其余保持不变
  * 时间精度：微秒
* 时区
  * 推荐用法：没有时区信息（简单型）则默认表示本地时间。若要表示UTC或其他时区的时间，必须带时区（dt.tzinfo不为None，感知型）
    * 不考虑非本地时区的简单型，其实并不正确，相当于仅存在时刻了，非时刻的时间是有意义的，不过感觉只对纯time常见
  * dt.astimezone(tzinfo) 若为简单型，先附加（不改变时间）本地时区成感知型，再（对于感知型）调整时区（和时间，不改变时刻）到指定的
  * 修复（违反了推荐用法的）简单型但含义上不是本地时间的dt，保持时间，直接设置时区：dt.replace(tzinfo=时区)
  * 获得UTC+8时区：timezone(timedelta(hours=8))、ZoneInfo('Asia/Shanghai')
  * date不存在时区问题
  * dt.timetz() 获得带有时区的time对象
* timedelta
  * 表示间隔，是d1-d2的结果类型，可以为负
  * 可主动调用构造函数，用于把时间标准化为days、seconds属性，用.total_seconds()再把days也换算进秒
* 字符串和datetime互转
  * dt.ctime() -> 'Tue Oct 19 12:12:33 2021' C语言风格
  * dt.isoformat() -> '2021-10-19T12:12:33.653925' ISO 8601格式
    * str(dt) -> 类ISO，但无T
    * datetime.fromisoformat() 能读取二者。还允许省略-和: '20201209T160953Z'；还允许省略时间（或部分时间），则默认为0
  * dt.strftime(pattern)、datetime.strptime(date_str, pattern)、'{:pattern}'.format(dt)
    * %Y-%m-%d %H:%M:%S
    * 有几个指示符能根据locale产生不同的风格，如%c英文下对应ctime，实测中文下也都是。调整用setlocale(locale.LC_TIME, 'en_US')
  * wsgiref.handlers.format_date_time(time.time()) -> 'Tue, 19 Oct 2021 04:35:34 GMT'，RFC1123或RFC2822日期表示，用在邮件中
* calendar.isleap()：是否是闰年
* 第三方日期库：https://github.com/arrow-py/arrow pendulum scrapinghub/dateparser解析人类可读的日期 dateutil。评测：https://dev.arie.bovenberg.net/blog/python-datetime-pitfalls/
* TODO: https://realpython.com/python-datetime/

### time模块（不是datetime.time）

* 计数
  * time.perf_counter()或time.monotonic()二者完全相同：返回性能计数器的值，会算sleep的时间。基本上就是一个程序开始后就自动运行的stopwatch。基准未定义，一般要获取两次相减计算时间差
  * time.process_time()：消耗CPU的时间，不包括sleep的时间。使用也是相减。少量运算这个值甚至都不会变，连续运算一般是上者的一半左右
    * time.thread_time() 仅当前线程
  * time.time() -> 1634617609.0561507：unix时间戳秒数
  * 以上三个方法虽然时间单位是秒，但类型为float，有很多位小数，是从ns(也可能是微秒)转换而来的，有精度损失。用_ns后缀的函数会返回int类型的纳秒
  * time.clock()在3.8中移除了
* 时间
  * time.ctime(可选sec) -> 'Tue Oct 19 12:00:00 2021'：把时间戳转换为C语言风格字符串
  * time.localtime() time.gmttime()：把时间戳转换为struct_time命名元组（具有tm_year等属性，另外月份从1开始，与C不同）
...
* re.search(pattern, str)：以Match对象返回第一个匹配到的部分，无匹配返回None
  * re.match()：从字符串开头进行匹配，没啥用。其他语言中的match一般为py的search
...
* Flags标志：re.I大小写不敏感，re.X忽略字符串中的空白和井号注释（除非两者前再有反斜杠或`[#]`），re.ASCII让\w \d这些只匹配ASCII字符集。未来必须用命名参数
* Match对象
  ...
  * 如果模式中不存在指定的组，会抛异常；如果模式中存在但没捕获到（如加了?）则返回None；对于`*`，可能成功捕获''
...
* Counter(seq)
  ...
  * 取不存在的key时不会报错，而是返回0。也可以ctr[不存在的key]+=1。类似于defaultdict
* deque(seq,maxlen)：双端链表队列，也适合做栈，满的时候自动去掉另一端的
  * pop去掉并返回最右边的，poplift去掉最左边的，extendleft，rotate当参数大于0时将最右边的元素移到最左边
  * `consume=deque(maxlen=0).extend` 一种不额外消耗而执行生成器的方式
...
* 如果类作为容器类，继承Generic[T]。3.12允许在名称后加[T]，且允许约束，如T:(str,bytes)代替AnyStr
...
collections.abc.Iterable, Mapping只读, MutableMapping, Sequence
collections.abc.Callable[[...]，ReturnType]  可调用对象的参数和返回值

类型别名，3.12：type Vector = list[float]
Never(等价于NoReturn)  用于返回值时表示会抛异常，用于type guard时表示bottom type
...
* socketserver.ThreadingTCPServer
  * 适合服务端，比socket更高层。但实测没有做IPv6的支持
  * 创建时传((IP,PORT), MyHandler)，with s: serve_forever()
  * MyHandler继承StreamRequestHandler/DatagramRequestHandler
    ...
    * 每次请求的self都不同，不是Spring那种单例
  * allow_reuse_address、timeout
  * http.server类似，有ThreadingHTTPServer和BaseHTTPRequestHandler，用法：self.send_response(200); self.end_headers(); self.wfile.write(b'ok')。SimpleHTTPRequestHandler的do_GET和do_HEAD默认返回目录浏览网页，一般不用
  * 发生异常不会退出程序，而是捕获输出到stderr
* 地址用('',0)会使用操作系统的默认行为，即0.0.0.0或::和随机分配端口，但仅限对应协议。底层的getaddrinfo要用None，会返回v4和v6但顺序不保证 TODO:测试socketserver
* 断开连接
  ...
  * 默认情况下close()是异步的。改为强制关闭：可以先shutdown，设置so_linger，设置短超时
  * 不要多次close()同一个socket，因为这之间可能产生了值相同的新socket，就会错误关闭另一个
* setblocking(False)：启用非阻塞模式，可配合select或selectors(Linux下自动为epoll，Win下为select)模块，后者基本可看作创建事件循环和注册回调，不如用asyncio
* settimeout()、setdefaulttimeout()：默认None，阻塞模式，会等待直到系统返回错误。设为正数时为超时模式，会抛TimeoutError，可以catch并忽略。0为非阻塞模式，报错BlockingIOError: 无法立即完成一个非阻止性套接字操作，也可以catch然后sleep
* 接收大小推荐用2的相对小的幂，如4096。UDP最大65507
* gethostbyname()：相当于V4的DNS解析
* sendfile()：高性能发送二进制文件对象，Win下自动用普通的send()，只支持TCP和阻塞模式
* getsockname()：本地绑定的地址元组；getpeername()：远端连接到的地址
...
* Win下recv()时用Ctrl+C无法中断，参见threading的处理办法。asyncio可能在3.11后可以，未测试
* listen(backlog半连接数，默认为合理值)
* sendmsg
  * 重要功能一：零拷贝拼接，msg是个数组，内部自动合并，而不需要用户先把已有的多个数据拷贝到一起。如header+body的模式
  * 功能二：指定控制消息 ancillary data，一般是给操作系统处理的元数据，如指定从某个网卡发出去（还有记录从网卡发出的瞬间）、临时调整opt等；接收者读取IP头中的特殊选项。另一种用法：多进程共享fd，普通fd是进程独有的，用UDS和它内核会复制一份
  * 不支持Win。原生WinAPI有WSASend和WSASendMsg，前者相比send支持IOCP和msg数组，相比后者不支持辅助消息；后者不支持TCP
...
s.bind(('', 8080)) # 接收
data, addr = s.recvfrom(1024) # 若包大于buf的大小，会抛异常，Linux下可指定MSG_PEEK截断，Win不支持
# 客户端也能用connect，之后可以send()和recv()。服务端一般不recv因为不知道客户端地址无法发回去了

# TCP。连接成功后双方都可以收发
with socket() as s # 参数默认是 AF_INET(仅限v4), SOCK_STREAM
...
# TCP Server。有便捷函数create_server(addr, family=socket.AF_INET6, dualstack_ipv6=True)且已设置了SO_REUSEADDR
s.bind(); s.listen()
client_sock, client_ip_port = s.accept()
with client_sock:
  while True: # Client也是这么接收的
    try:
      data = s.recv(1024)
      if not data: break # 当客户端断开后调用recv会一直成功返回空
    except ConnectionResetError: break # 异常断开
...
* ssl.create_default_context() 客户端可以直接用
  * Purpose.CLIENT_AUTH不是用作客户端的，恰恰相反，此选项是用于服务端（想验证客户端身份即mTLS）的
  * ctx.minimum_version = ssl.TLSVersion.TLSv1_3 最低默认1.2。当握手失败时会说SSLV3_ALERT_HANDSHAKE_FAILURE，其实并不是SSL3
  * ctx.check_hostname 客户端默认True
  * 查看支持的密码套件：ctx.get_ciphers()
  * 证书默认加载系统的，会根据Purpose只加载用于服务器或客户端验证的
  * 额外加载证书和私钥：ctx.load_cert_chain()
  * 客户端加载服务端ca证书，即用于验证对方的：ctx.load_verify_locations()
* socket连接成功后，实际类型为SSLSocket，增加了version() cipher()实际使用的密码套件 等成员
* 忽略证书验证：ctx.check_hostname=False; ctx.verify_mode=ssl.CERT_NONE; urlopen(..., context=ctx)
* 弃用的：TLSVersion下除了1.2和1.3的、PROTOCOL_除了TLS_CLIENT和SERVER的
...
* 同步用的对象
  * Condition条件：用于生产者消费者，或等待-唤醒队列，避免busy-waiting（反复 加锁、检查不成立、解锁）。消费者wait_for(predicate函数)（相当于while(!predicate)cv.wait()），生产者执行生产逻辑后调用notify()或notify_all()。重点在于双方必须在with cv中调用wait和notify，隐含互斥锁，断言成立后状态不会被其他线程修改。可理解为Event和Lock的合并，单纯Event（即使某些语言会自动Reset）无法保证断言成立后不被另一个线程修改状态
  * Semaphore信号量：保护数量有限的资源，或用作计数器，同样用于生产者消费者和等待-唤醒队列。BoundedSemaphore：若资源量增加高于初始值时会抛异常
  * Event事件：调用.wait()会阻塞，直到set()。不会自动变回False，直到clear()。简单来说就是一个bool状态。用于（可一对多）就绪/终止通知；不依赖P
  * Barrier栅栏：能wait()N次。用于固定数量的线程相互等待，直到所有线程都调用了wait()后同时释放。和信号量刚好相反，后者有值时不阻塞，无值时阻塞
* with Lock()普通锁。RLock()可重入锁，本线程可多次acquire，减少死锁可能
* Timer定时器：是Thread的子类
* queue模块：Queue（FIFO的）, LifoQueue, PriorityQueue
  * 空的时候get()阻塞，直到另一线程put()；可选非阻塞而抛异常、指定等待时间
  * 适合控制活跃线程的数量、生产消费者模型。消费者处理完后用task_done()，生产者用join()，则相当于WaitGroup
  * 不支持in运算符
  * 如果只要数据结构而非多线程交换消息，用collections.deque。还有个SimpleQueue是用List模拟的
...
```py
# 线程池、进程池。不考虑使用multiprocessing.Pool
from concurrent.futures import ProcessPoolExecutor, as_completed
with ProcessPoolExecutor() as executor: # 默认max_workers=T是cpu核心数+4，P是cpu核心数
    for result in executor.map(fib, NUMBERS): # 批量提交，第二个参数是序列，如果很长且执行时间短，推荐设置chunksize(默认1)
        print(result)

    # 单独提交，用于函数不同或可能抛出异常
    futures = []
    for num in NUMBERS:
        futures.append(executor.submit(fib, num))
...
* 取消或异常
  * 如果有异常，在await时会抛出，或用t.exception()
  * t.cancel()会在t内部的执行的co函数上引发CancelledError，t本身没事，其实和其他异常区别不大。只不过有t.cancelled()指示是否取消了
  * CancelledError一般不进行捕获（属于BaseException），它会自动向上传播。执行清理逻辑一般用finally
* python -m asyncio可在REPL中直接await而不必run，IPython更是直接可以
* 异步生成器：在async函数中用yield返回值，不支持yield from。消费时用async for，外面可能还有一层async with  aclosing。支持推导式，相当于自动await了，所以也要在async函数中用
* 同步原语：不能用于系统线程同步。其中Semaphore推荐用with。一般优先用队列，相比于普通的，get且处理完后要task_done()。或如果单向传递可用异步生成器
* 子进程：create_subprocess_exec('xxx', 'args', stdout=asyncio.subprocess.PIPE)
* DEBUG模式：检查未被等待的协程、记录花费时间过长的IO和回调
* contextvars.Context：必须声明成全局变量，一次只是一个对象，在某个函数里set后，当前函数中的后续其他函数调用都能get。可以跨async和sync函数用
* 与线程的异同：都要保存栈(局部变量)和寄存器。协程在用户态调度，由开发者决定切换，不会被抢占，如果不主动让出，其他的没有机会执行
* 千万不要pip装asyncio，那是2017年的不在标准库里的包
* aiohttp
* aiofiles、aiofile：前者纯线程池实现，拉完了。后者Linux下用caio（uring和aio的绑定），其它系统用线程池
* property不是awaitable的。有个async-property库，但仍不支持setter
* 一些内置函数的async版：https://github.com/maxfischer2781/asyncstdlib

```py
# 正常使用
import asyncio

async def f1(): # 定义coroutine函数。单纯f1()后返回co对象，不会运行内容，如果不await就应用create_task才会开始执行
    print('Hello ...')
    await asyncio.sleep(1) # 执行co对象
    print('... World')

asyncio.run(main()) # 在最外层或同步方法中调用，产生eventloop。终止时忽略异常信息：包一层KeyboardInterrupt:pass

t = asyncio.create_task(f1()) # 把co对象放进evloop待执行，返回代表它的task对象，await t获得f1的值
t.add_done_callback(tsk_set.discard) # 如果将t保存到别的地方，要在t成功执行完后清除引用。回调是非async函数

# 并发执行
lr = await asyncio.gather(f1(), f2()) # 并发调用并获取list[结果]。某一任务被取消不会导致其余任务被取消，gather本身被取消会全部取消；可用return_exceptions=True获取异常结果
async for t in asyncio.as_completed(tasks): await t # 获得的t是最先执行完的tasks中的原有的元素(3.13)，只要获得结果
# 3.11另一种并发调用和等待的方式
# 如果里面有任务出现了异常，其余任务会被取消，那些异常组合抛出ExceptionGroup
...
try:
    ...
    async with asyncio.timeout(10): ... # 3.11，类tg处理多个任务
except TimeoutError: # 3.11之前是asyncio.下的
    print('timeout')

# 替代threading，在单独的线程中执行普通函数。如果函数本身是no-gil的则可并行执行。会使用默认的ThreadPoolExecutor
asyncio.to_thread(blocking_io_fun, arg1, arg2)

# 不应直接执行cpu-bound的代码，否则会阻塞主循环。应指定另一个进程执行器
loop = asyncio.get_running_loop()
with concurrent.futures.ProcessPoolExecutor() as pool:
    result = await loop.run_in_executor(pool, cpu_bound)

asyncio.run_coroutine_threadsafe()：主线程创建了evloop，另一个手动使用threading的线程创建了某个coro，则使用此函数可以把coro放进evloop执行。也可以用来在threading为主的代码中调用async函数。返回concurrent.futures.Future，线程可以做其他事，之后再wait。而如果利用asyncio.run()的返回值，那线程就阻塞了，必须等evloop关闭。另有django出第三方库asgiref.sync;@async_to_sync
```

#### Stream

* 服务端：server = await asyncio.start_server(cb, port=8080)
  * cb的参数为(r:asyncio.StreamReader, w)
  * async with server: await server.serve_forever()
* 客户端：asyncio.open_connection(ip,port)后获得reader和writer
  * 也可用域名，加ssl=True
* Writer
  * w.write() 如果w已关闭，会抛ConnectionResetError
  * await w.drain() 等待写入完成
  * w.close(); await w.wait_closed() 关闭时会drain？好像相当于把socket关了。如果不用等待完成再继续就不用后者。多次close()没问题，但socket不行，最好别依赖
  * transport属性：返回socket
  * 客户端ip:port：get_extra_info('peername')[0]
  * write_eof：完全看不出有什么用
* Reader
  * d = await r.read(512); if not d: return 一般不用无参的，否则只有等对方关闭连接才能继续
  * readline() 读取直到\n。非阻塞，如果流关闭了且缓冲区读完了，会返回空
  * 支持async for消费，相当于readline，但是无限阻塞的直到关闭
  * readexactly(n) 如果没读到就关闭了会抛异常
  * readuntil('\r\n') 会将参数算进返回值。如果没读到就关闭了、缓冲区不够（默认64K） 会抛异常
...
parser.add_argument('file', help='xxx') # 之后储存到args.file中，用--dest储存到不同名字的属性。--a-b对应args.a_b
...
* quoting策略
  * 默认QUOTE_MINIMAL：当数据包含逗号和换行符时会在那一项外面加双引号，数据包含双引号时会在外面加双引号并且用两个双引号表示数据中的一个双引号
  * QUOTE_NONNUMERIC：读取时会把所有未加双引号的转换为float，写入时会给所有非数字加双引号
* dialect：默认excel。可选unix，则所有字段都由引号包围，用\n换行

```py
f=open('data.csv', newline='') 第二个参数固定使用
for row in csv.reader(f): row为list[str]
csv.writer(f) .writerow([col1, col2]) .writerows(行列表)
csv.DictReader(f) 自动把文件中的第一行看作header，如果没有必须自己指定一下，之后每遍历一行返回一个dict
csv.DictWriter(f, [h1,h2]) .writeheader() .writerow(dict) .writerows(行列表)  必须指定header名称和顺序，且不会自动写入，而是用于映射dict
```
...
* 指针
  * POINTER(c_int); pointer(c_对象)
  * 若只为调用函数传引用（允许修改）而非使用指针对象，用byref(c_对象)更快（指定了argtypes时可直接用c_对象，但类似于c++看不出修改了参数），它相当于右值，而pointer相当于左值，可以再对它取指针
  * 单一对象用contents属性返回指向的c_对象。若指针对象为数组，可在Py侧用切片遍历，也可以取下标
* 结构体和位域：创建class继承Structure，再写`_fields_ = [('变量名',类型,可选位宽),]`。带位域时只能通过指针传递给函数
* 回调函数：创建类型用FUNC=CFUNCTYPE(restype, argtypes)。创建可传递给C的实例用FUNC(py函数)
在正常函数上用@CFUNCTYPE()声明
* 访问库的导出变量：c_int.in_dll(lib, 'name')
* ctypes.wintypes.HANDLE, HWND, DWORD, LPVOID;
* 处理报错：if not ret: raise ctypes.WinError()。手动：err = ctypes.GetLastError(); raise OSError(None, ctypes.FormatError(err), None, err)。其中FormatError无参调用会内部调用GetLastError，但此函数用了一次会重置，第二次调用就无报错了，如果要同时获得报错内容和错误码要手动管理。或在创建WinDLL时指定use_last_error=True（默认实例未设定），之后用ctypes.get_last_error()，如果没设定，后者无报错码
* 示例：https://github.com/abetlen/llama-cpp-python

```py
from ctypes import *
libc = CDLL('libc.so.6') 或 cdll.ucrtbase.xxx 或 cdll['dllname'] # Linux下后缀必须写全。win下用第二种自动搜索系统路径中的dll，A和W版本不会自动选择。windll按stdcall但只有32位才需要，oledll假定返回值是HRESULT且失败时会自动抛OSError；pythonapi为预定义的Python C API。加载相对路径下的库用./
util.find_library('m') -> 'libm.so.6' # 只会搜索系统库，但win下也受os.add_dll_directory()影响
libc.time(None) # 对应NULL

i = c_int(42); i.value = 24 # 可变
libc.scanf(b'%d', byref(i))

c_char_p(b'Hello') # 相当于const char*，内部自动以\0结尾。value可以改，但相当于py的str只能重新赋值引用，不能把它们传给会改变其内容的外部函数。一般直接用b''即可
...
string_at(addressof(o), size=-1) 从指定地址创建bytes，size是长度，若不指定则找到\0
...
* http.server -b ip port默认8000：-b默认可能是0.0.0.0或::，后者是v6双栈。支持目录index
...
* uuid：3.12
* random：参数用单个整数则生成[1,N)，用单个小数或指定-f则生成[0,N)，用字符串或多个参数则随机选一个
...
* copy.deepcopy()深拷贝
* inspect：signature(fun)，getfullargspec(fun)查看函数参数信息，getsource(fun)返回函数实现源代码
...
* tempfile
  ...
  * gettempdir()获得临时文件目录，还可以主动设置tempdir属性来影响其它函数
  * SpooledTemporaryFile先在内存中缓存直到超过max_size
* contextvars：替代threading.local
* mimetypes.guess_file_type(path-str/url)需要读取文件内容，返回(type, encoding)；types_map['.json']单纯根据后缀猜
* atexit.register(func, *args, **kwargs)
* gc：如果确定程序不会循环引用可以disable()关闭。调试内存泄漏的程序时有选项启用详细日志
* cProfile：run('f()', '可选保存到文件中')，pstats.Stats读取文件并分析。可分析所有函数调用和花费的时间
* webbrowser.open()
* statistics：mean(data) median() variance()
* mmap
* tracemalloc：start(); take_snapshot().statistics('lineno')[:10] 第一次运行某句会产生加载函数等额外开销，应该先运行一遍再测
* 压缩
  * zlib：compress(), decompress(), crc32()
  * 更高层的gzip可以把.gz当作filelike，mode默认rb
  * bz2
  * with tarfile.open('xxx.tar.gz', mode='w:gz') as tar: tar.add()
  * with zipfile.ZipFile('a.zip', 'w') as zp: zp.write(src,dest)，with zp.open(压缩包内文件) as f，zp.infolist()显示文件和压缩情况。默认不压缩，但make_archive默认DEFLATED
* 算法
  * bisect.bisect_left：二分查找返回索引，如果target不存在，仍会返回使得插入后保持有序的索引而非-1
  * heapq堆排序：以数组为底层储存，小顶堆，想要大顶堆可对值取负。heapify(seq)，heappush(seq,elem)/heappop，merge(l1, l2)合并多个排序好的序列，nlargest/nsmallest(n,seq)最大/小的几个数（再取[-1]就是那一个数），seq[0]就是最小的数相当于peek。另见queue.PriorityQueue
  * graphlib.TopologicalSorter：拓扑排序
* 随机数
  * random：randrange(to)/(from,to,step)左闭右开整数，randint闭区间设计失误，random()浮点[0.0, 1.0)，uniform(a,b)b小于a也可的闭区间浮点，choice(seq)随机返回一个元素，choices放回采样多个元素（可重复），sample不放回随机获取多个元素，shuffle原地改变原有序列顺序
  * secrets：安全性更高的随机数。SystemRandom提供普通random的mock
