...
* metaclass元类：定义时继承type，使用时指定metaclass=M。一般在元类中重写init和call，分别影响创建类本身和实例化类对象时的行为。当继承无法解决时可以考虑使用，如存在某些类字段时才添加某些方法，或者限制允许添加的字段名称。其实本来创建类用的就是C=type('类名', 基类, 字段)，指定元类后就是C=M()了
...
* 继承时，父类的属性都限制在父类的空间中，子类用super().xxx可以访问（如果子类没有同名对象可直接访问），但不能赋值，会报AttributeError
* 函数都是“虚函数”，父类函数使用的都是子类对象的数据
* 第一基类能在类上用`__base__`获得
* 支持多继承，查找对象的顺序按类（不是对象）的mro()顺序，只读。假如M(B,C) B(A) C(A)，则M的mro为MBCA，但如果C没继承A，则为MBAC
* super(M,self)：在第二个参数的mro()中查找第一个参数之后一个对象，并把第二个参数绑定在最后调用的方法上。即`super(M,self).__init__(xxx)`等价于`B.__init__(m,xxx)`，因此B中再super(B,self)得到的是与B无关的C而非它的父类A，因为那个self对象是m。super实际上是个类，无参调用时Py3中用了黑魔法自动传递了它所在的类和对象
...
move(),copytree(),rmtree()类似于rm -rf,unpack_archive()
```
...
* 同步用的对象
  ...
  * Event事件：调用.wait()会阻塞，直到set()
  * Timer定时器
  * Barrier栅栏：能wait()N次。用于固定数量的线程相互等待，直到所有线程都调用了wait()后同时释放
...
* tempfile.NamedTemporaryFile()，mode默认w+b，支持wit
...
* ctypes：直接加载调用C的库。Linux下用cdll.LoadLibrary('libc.so')或CDLL()，当前目录下的必须加./，winAPI用windll.kernel32等。Win下CAPI能用cdll.ucrtbase.xxx但它本来就没有printf，输出会直接输出到底层而非py的stdout，注意换行或刷新缓冲区。C的字符串应用bytes，整数能直接用，其他类型都要用c_xxx()转换；返回类型默认为int，修改需改函数对象的restype属性。util.find_library()根据库名返回绝对路径，只会搜索系统库，但也受os.add_dll_directory()影响
* webbrowser.open()
