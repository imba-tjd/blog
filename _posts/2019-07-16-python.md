...
* 非MSVC版：https://github.com/msys2-contrib/cpython-mingw 下Actions里的build-cross-llvm-mingw-x86_64，运行前手动设置PYTHONHOME为bin所在目录。或者下msys2的ucrt包，还要复制libgcc_s_seh-1.dll
...
* bytes
  ...
  * bytes.fromhex('4B CE')-> b'K\xce'，忽略空白字符，bytearray有此函数，str没有此函数
...
* sorted：非原地从小到大排序返回list。key=lambda x:x.p表示按p属性排序，-x表示从大到小排序，可以返回元组表示依次按指定项排序；算法为Timsort是稳定的，reverse=True不等于正向排序后再反向
...
* range
* reversed->迭代器
* eval：把字符串解析为表达式，不能有赋值语句和流程控制语句，`:=`也不行，但却可以是返回None的函数；剩下两个参数可指定函数内临时的global和local字典，默认会使用当前作用域的，设为{}去掉环境。exec：可执行语句，没有返回值，可以有赋值语句，在模块里运行会影响globals()，但在函数里运行对变量赋值，结束后不会改变locals()。execfile废弃了。`eval(compile("5 + 4", "<string>", "eval"))`执行编译后的字节码
* issubclass
...
* 支持多继承，查找顺序按类（不是对象）的mro()顺序，只读。假如M(B,C) B(A) C(A)，则M的mro为MBCA，但如果C没继承A，则为MBAC。更混乱时有可能无法序列化出结果，会报错
* super(M,self)
...
* type和metaclass
  ...
  * new和init的参数都是`(cls, name, bases, ns)`，但new里的cls指的是元类，init里指的是要使用元类的类
...
f = StringIO('hello') # 属于file-like-obj，或类似于Stream或StringBuilder。直接写入文件用copyfileobj
...
shutil.copy() dest可以是目录类似于cp，copy2()尽力保留元数据类似于cp -p，copyfile()只复制数据不复制权限，copyfileobj()非常普通地分块写file-like-obj，并不高效但可防止一次性占内存太多
...
* 持久化字典：`with shelve.open('data') as db`，内部使用了pickle，默认最新版协议，设置writeback=True可内存缓存，只在sync()或关闭时写入；内部还使用了dbm，没必要单独用它了。第三方有sqlitedict
...
* datetime模块：date，time，datetime，timedelta，timezone时区信息，3.9出了zoneinfo模块，tzinfo是两者的抽象父类
* date.today().day/.month/.year
* datetime.now()/now(UTC)/构造函数；datetime.combine(date,time)；dt.replace()改变参数中指定的字段，其余保持不变。utcnow()为简单型不推荐用
* timedelta：表示间隔，是d1-d2的结果类型，也可主动调用构造函数，会把时间标准化为days seconds属性，还有.total_seconds()再把days换算进秒
* 相同类型可直接用< >比较
* 字符串和datetime互转
  * ctime() -> 'Tue Oct 19 12:12:33 2021'，C语言风格，无法直接转换回来
  * isoformat() -> '2021-10-19T12:12:33.653925'；fromisoformat()，它也能处理str(datetime)的
  * strftime()、strptime()；模式字符串能根据locale.setlocale(locale.LC_TIME, 'en_US')产生不同的日期名称
  * wsgiref.handlers.format_date_time(time.time()) -> 'Tue, 19 Oct 2021 04:35:34 GMT'，RFC1123日期表示
  * '{:%Y-%m-%d %H:%M:%S}'.format(dt)，注意最前面的冒号
  * %a Tue，%A Tuesday，%b Oct，%B October，%j 012一年中的第几天
...
* re.sub(pattern, repl, str)：全文替换，其中repl可以是str或可调用对象；用`r'\1'`引用第一个捕获组（不是$）但`\0`会被看作`\x00`；第四个参数可指定替换最大次数，第五个参数才是Flags
...
* 内置
  ...
  * map：与推导式相比更适合函数已经定义好了：map(str, range(20))。可接受多个序列但行为与starmap不同，是每个都取一个作为回调函数的参数：map(pow, [2,3], [2,4])->[4,81]
* itertools
  * 注意这些还是返回的迭代器，只能消费一次
...
* TypedDict：唯一用处就是标注旧的不恰当用了dict的代码
* 第三方库
...
* 事务隔离isolation_level：默认是DEFERRED，会在DML前隐式BEGIN开启事务，完成后必须在Py中commit()否则修改不生效。设为None后为autocommit模式，每一句都自动提交，但执行BEGIN后就不会自动提交，直到COMMIT/ROLLBACK
* connect设置uri=True能启用URI Filenames，设置detect_types=sqlite3.PARSE_DECLTYPES能根据类型声明解析数据？
...
```py
with sqlite3.connect('data.db'或':memory:'或'') as con: # 不是用于自动关闭连接的，而是用于处理事务：执行完成后自动commit，发生错误自动rollback，且仍会抛sqlite3.Error
...
* socketserver
  * 服务端优先使用本模块，更高层
  * ThreadingTCPServer/ThreadingUDPServer，创建时传((IP,PORT), MyHandler)，serve_forever()
  * MyHandler继承StreamRequestHandler/DatagramRequestHandler，def handle(self):读取self.rfile，写入self.wfile，两者都是file-like-obj
...
* setblocking(False)：启用非阻塞模式，可配合select或selectors模块使用，后者基本上可看作创建事件循环和注册回调，不如用asyncio
...
* recvfrom()：返回(bytes_data, client_ip_port)，一般UDP用到
* getsockname()：返回绑定的地址；getpeername()：返回远端连接到的地址
* settimeout(浮点秒数)：任何行为的超时时间，默认是无限，对服务端也有用
* setsocketopt(socket.SOL_SOCKET/socket.SOL_TCP, optname如SO_REUSEADDR, value如1表示启用)
...
with client_sock: while True: data = s.recv(1024); if not data: break
...
* 仅用于IO绑定，无法利用多核
* 同步用的对象
  * Condition条件：用于生产者消费者，或等待-唤醒队列，避免busy-waiting和反复加锁检查不成立解锁。一般wait_for(predicate函数)，生产者调用notify()或notify_all()，关键是需要消费者提供函数自己检查条件是否成立
...
* Queue（FIFO的）, LifoQueue, PriorityQueue：满的时候put()阻塞直到另一线程get()，空的时候get()同理；或用put_nowait()抛异常。适合控制活跃线程的数量，或生产消费者模型；不支持in运算符
* 没有办法杀死指定线程，多进程可以

```py
from threading import Thread
t = Thread(target=f, args=(xxx,))
t.daemon = True # 默认为False前台线程，即使主线程结束了，也会等待它结束，因此当它阻塞时无法结束
t.start()
...
* 优先使用imap和imap_unordered，是迭代器；普通的map要等所有任务执行完才一并返回；若待处理的序列很长，应设置chunksize
...
* async with
* 流：基本上就是TCP编程。服务端不如socketserver简单，不学；客户端基本上就是asyncio.open_connection(ip,port)后获得reader和writer
* 同步原语：不能用于OS线程同步
...
lstr = await asyncio.gather(f1(), f2()) # 并发调用并获取结果列表；asyncio.as_completed()行为类似，返回迭代器，自身不必await
t.add_done_callback(cbk)

async with asyncio.TaskGroup() as tg: # 3.11代替gather等待多个任务的方法
...
* python -m unittest 会自动寻找文件名以test开头的py，默认支持发现根目录下的命名空间包。加以点分隔的对象只运行指定的测试，-f只要有一个失败就快速失败中止测试，--locals在traceback中显示局部变量
...
* Linux下后缀必须写全，libc.so.6无法简写成libc
* 加载本地库用./xxx
* printf等输出会直接输出到底层而非py的stdout，注意换行或刷新缓冲区，且只能在控制台下工作
* 数组：IntArr=c_int*3; IntArr(1,2,3)
* 指针：POINTER(c_int); pointer(c_对象)。若只为传引用而非使用指针对象，用byref(c_对象)更快。单一对象用contents属性返回指向的c_对象。若指针对象为数组，可在Py侧用切片遍历，也可以取下标
* HANDLE：ctypes.wintypes.HANDLE
* 结构体和位域：创建class继承STRUCTURE，再写`_fields_ = [('变量名',类型,可选位宽),]`
* 回调函数：在正常函数上用@CFUNCTYPE()声明类型
* 访问库的导出变量：c_int.in_dll(lib, 'name')

```py
from ctypes import *
libc = cdll.LoadLibrary('libc.so.6'); libc.time(None) # 或CDLL()
ctypes.util.find_library('m') -> 'libm.so.6' # 只会搜索系统库，但也受os.add_dll_directory()影响
cdll.ucrtbase.xxx; windll.kernel32.xxx # windll自动按stdcall，A和W版本不会自动选择。已知shell32和ntdll也是预定义的
pythonapi # 预定义的Python C API

i = c_int(42); i.value = 24 # 可变
libc.scanf(b'%d', byref(i))

c_char_p(b'Hello') # 相当于const char*，内部自动以\0结尾。还有c_wchar_p能传Py字符串。value也是可变的，但却不能把它们传给会改变其内容的外部函数。感觉没用
...
f.argtypes = [c_double] # 声明参数，类似于原型。什么都不做时传Py对象，bytes能转换成const char*，整数转换成int，其余的都会报错
f.restype=c_double # 返回类型默认都是int，必须手动改。基本数据类型会自动转换为Py对象，如设置返回值类型为c_char_p会自动得到字符串而非c_char_p实例
```
...
* memoryview：内置类型无需导入。支持bytes bytearray array.array，切片时不会产生新对象
...
* tempfile.NamedTemporaryFile()，mode默认w+b，支持with。gettempdir()获得临时文件目录，还可以主动设置tempdir属性来影响其它函数。SpooledTemporaryFile先在内存中缓存直到炒作max_size
