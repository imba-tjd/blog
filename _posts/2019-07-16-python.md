...
* 非MSVC版：https://github.com/msys2-contrib/cpython-mingw 下Actions里的build-cross-llvm-mingw-x86_64，运行前手动设置PYTHONHOME为bin所在目录
...
* datetime.now()/now(timezone.utc)/构造函数；datetime.combine(date,time)；dt.replace()改变参数中指定的字段，其余保持不变。utcnow()为简单型不推荐用
...
* shutdown()：如果需要立即关闭本端的连接，可在close()前调用，必须指定行为。close()不会立即关闭。socketserver有个同名但无参的，serve_forever()时在另一个线程使用用于打断
...
* tempfile.NamedTemporaryFile()，mode默认w+b，支持with。gettempdir()获得临时文件目录，还可以主动设置tempdir属性来影响其它函数
