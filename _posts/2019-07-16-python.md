...
* subprocess.run(['ls', '-l'])
  ...
  * capture_output=True：设定后可用.stdout获取输出，否则默认会继承父进程即默认直接输出到屏幕上；设定了此项就不要再设定stdout和stderr了；注意末尾一般有\n
...
* os.system()：调用该函数相当于创建了一个新的shell；os.popen().readlines()：创建管道，fork子进程，获取输出，stdout用完了要关闭
