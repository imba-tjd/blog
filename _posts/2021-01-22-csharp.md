...
* t.GetTypeInfo()是Type的子类，一些UWP的API会用到。用AsType()能转回去。本来Core1做了大改动，但2又改回来了。现在不建议主动使用TypeInfo
...
* ulong -> DWORD
* char -> char/char16_t
* string -> char* LPSTR(默)/const char* LPCSTR/char16_t* wchar_t * LPWSTR(CharSet为Unicode时默)/LPUTF8Str。不要使用`[Out]`，intern字符串不应修改；ref string（对应LPStr*）没问题
* StringBuilder -> LPSTR/LPWSTR。默认`[In,Out]`，必须先由调用方创建足够长度，能为被调用方提供Capacity+1的大小。少用因为始终会复制，优先用ArrayPool的 char[] 或 byte[] 且加 [Out]
...
* 方向特性
  ...
  * 引用类型（一般就是字符串和数组）对应C的指针，但都默认`[In]`不能改里面的值。数组要明确用`[In,Out]`才能改值。T[,]对应C的[][COL_DIM]
...
* 调用UWP的API
  * .NET5：CsWinRT。设置TargetFramework=net7.0-windows10.0.22000.0，无需设置CsWinRTWindowsMetadata，不装WinSDK虽然build时报`Failed to resolve WinRT.Runtime.dll`但能用。但CsWinRTEmbedded用不了，会产生无数报错信息，它能摆脱Microsoft.Windows.SDK.NET.dll的20MB依赖。要求存在%ProgramFiles%\Microsoft SDKs\NuGetPackages文件夹但现在好像不会写入内容
...
* builder.Query = "one=1"; builder.Query += "&two=2" 但这个Query就是个string啊
