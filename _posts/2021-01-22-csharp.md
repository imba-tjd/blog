...
* Assembly ass = Assembly.GetExecutingAssembly()/LoadFrom("lib.dll"); Type t = ass.GetType("NS.C"); Type[] ts = ass.GetTypes()
...
* ulong -> DWORD
* nint -> 根据系统32/64位，感觉类似于ssize_t，在元数据中用IntPtr实现，但支持整数运算
* char -> char/char16_t
* string -> char*/const char*/char16_t*/wchar_t */LPSTR/LPCSTR/LPWSTR/LPUTF8Str。不要使用`[Out]`，intern字符串不应修改；但修改ref string没有问题，特殊处理了
...
* 结构体：默认就是`[StructLayout(LayoutKind.Sequential)]`，还能指定CharSet，想用union时用LayoutKind.Explicit并在每个成员上指定FieldOffset(以0开始的字节数)。类默认是LayoutKind.Auto只用于托管代码。不要用Delegate类型因为它不具有参数签名
...
* DllImport
  ...
  * 未来可以换成[LibraryImport]，extern改成partial，CharSet属性改为StringMarshalling，好像没有坏处，对AOT更友好
...
* 使用回调函数或返回指针时注意垃圾回收
* 调用Win32的API：https://pinvoke.net https://github.com/dotnet/pinvoke https://github.com/microsoft/CsWin32 https://github.com/dahall/Vanara https://github.com/JeremyKuhne/WInterop 还有个不活跃且无release的pinvoke-interop-assistant
* 调用UWP的API：CsWinRT
* 生成binding：https://github.com/mono/CppSharp 但我没有看懂怎么用，文档根本没写。另一种 https://www.nuget.org/packages/ClangSharpPInvokeGenerator
...
* 时间戳：new DateTimeOffset(DateTime.UtcNow).ToUnixTimeSeconds()
* TimeSpan.FromSeconds()
* TODO: https://docs.microsoft.com/zh-cn/dotnet/standard/base-types/standard-date-and-time-format-strings

## UriBuilder

* builder.Query = "one=1"; builder.Query += "&two=2"
