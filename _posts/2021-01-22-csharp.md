...
* 如果一次性使用的using太多，还不如自己写成try finally，就只用写一层；或者用C#8的using语法
...
* 获取函数：typeof(int).GetMethod("Parse", new[]{typeof(string)})，因为有重载，只靠名字无法唯一确定；调用函数：m.Invoke(obj/null, ...)
* GetMembers()获取所有成员包括属性方法字段事件等。GetMember()仍返回数组，则可以只指定函数名字。使用要tp.InvokeMember()指定一些BindingFlags，很麻烦
* `BindingFlags.NonPublic | BindingFlags.Instance/Static`可获取私有成员，IgnoreCase忽略大小写
