...
* 闭包
  ...
  * 匿名函数在定义的时可以使用(捕获)外部变量，当定义匿名函数的作用域结束时，如果匿名函数还存在引用，则捕获的变量不会消失，称作捕获变量的延长生存期，实际上创造了一个实例容纳捕获的变量
...
* 使用回调函数或返回指针时注意垃圾回收
* 调用Win32的API：https://pinvoke.net/
* 调用WinRT(UWP)的API：https://github.com/microsoft/CsWin32
