...
* Blittable类型：托管代码和本机代码具有相同位级别表示形式。bool不属于，char和string在指定编码为Unicode时属于，含有一般的C#数组的结构体不属于。sizeof(char)是2，Marshal的是1；sizeof(bool)是1，Marshal的是4
...
* 本机类型为结构体，C#中也为结构体；本机类型为指向结构体的指针，C#中可为ref结构体或类；二级指针，C#用ref类。int* n，C#可用out int n，int** arr，C#可用ref IntPtr
* 回调函数：一般先声明一个delegate，extern函数中使用此类型，调用时传C#函数
* 使用回调函数或返回指针时注意垃圾回收
* 调用Win32的API：https://pinvoke.net/ https://github.com/microsoft/CsWin32
...
* 访问结构体成员要用->
* 函数不要返回指针，因为不固定
* 函数指针：`delegate*<T, T, T>`，等价于Func，无返回值最后一个参数用void。在static函数上使用&获取地址。可在星号后加unmanaged关键字指示调用约定
...
* 重定向输出流后给OutputDataReceived事件添加处理函数，进程启动后启用BeginOutputReadLine()，之后每输出一行就会自动触发事件了。如果只执行一次也可p.StandardOutput.ReadToEnd()
...
doc.RootElement...
对象：GetProperty()。EnumerateObject()返回JsonProperty(k-v)列表，TryGetProperty()
