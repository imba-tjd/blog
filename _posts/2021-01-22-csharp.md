...
* -3>>1 -> -1，其它语言一般是-2
* 不支持1.表示1.0，其它语言一般支持。只有Java和C#支持d后缀表示double

### 字符串

* 引用类型，不可变对象，修改时会创建一个新的
* 静态方法：string.Join string.Compare
  * string.Create：需Core2.1。以前从SB创建字符串相当于复制了一份，此函数能操作字符串内部的char[]。若从char[]创建，则不如new
...
* 翻转：一般做法是 ToCharArray() Array.Reverse() new，但无法处理代理对
* TODO: 转义字符串，如把`"\n"`变成`"\\n"`。Regex.Unescape()可以解转义，但不能用Escape()进行转义，因为它会把正则的元字符都转义掉，如点

#### CultureInfo

* CultureInfo实现了IFormatProvider，决定某些格式化说明符的含义
  * 当前区域：System.Globalization.CultureInfo.CurrentCulture、Thread.CurrentThread.CurrentCulture
  * 固定区域：CultureInfo.InvariantCulture
* 涉及安全相关的字符串比较时，优先用StringComparison.Ordinal二进制比较。Invariant几乎用不到，会允许如`a + ̊  等于 å`。另外OrdinalIgnoreCase比先将二者都大写或小写更快
...
* 不能初始化字段或属性，除非是static或const的
* 构造函数必须为所有字段赋值
* 可以有静态构造函数。没有析构函数
* 一直存在隐式构造函数，且不能显式定义无参构造函数
* 不能继承，但可以实现接口
* 总是隐式密封的，不能用protected、internal、abstract、virtual、sealed。成员不默认public（与类一样）
* 预定义简单类型都实现为结构体
...
* 初始化时保证不为null：`event D e = delegate {};`，或用e?.Invoke()
...
* 避免catch中再引发新的异常，比如记录日志的时候出现IO异常。同理还有Dispose和析构器、异常的when、事件处理程序（存在多个handler时中间出现异常会中断，不调用后续；如果想对于某具体异常特殊处理后仍继续一般异常的处理，应catch一般异常，里面用switch模式匹配子类型）
...
* try与yield无法组合使用
* HResult
  * 列表：https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-erref/705fb797-2175-4a90-b5a3-3918024b10b8
  * 获得详细描述：Marshal.GetExceptionForHR(int hr).Message;
...
* 如果确实在泛型类中，即使为静态的，也必须先具体化那个类。静态非泛型方法也一样。这点与Java不同，优势是可以使用泛型类上的T，因为本质上不同封闭类型对应不同的方法
...
* 同步等待/在同步方法中调用异步方法，在UI线程上可能死锁：t.Result、t.Wait()、t.GetAwaiter().GetResult()、t.WaitAll()、t.WaitAny()、Thread.Sleep()；其中GetAwaiter()后对其操作不会抛AggregateException而已。解决办法见下面
...
* 在UI线程中用了t.Wait()或t.Result等会死锁，因为在另一个线程的异步方法要到回到UI线程，但UI线程阻塞了无法处理消息：`async Task f() => await Task.Delay(1); f().Wait()`
...
* 获取函数：typeof(int).GetMethod("Parse", new[]{typeof(string)})，因为有重载，只靠名字无法唯一确定
  ...
  * 构造函数：Activator.CreateInstance(type)、GetConstructor() 根据测试，前者无参时比后者快，相比于new大概4倍和10倍；前者有参时显著慢，不知道跟后者比如何。代替：LambdaExpression，运行多了后跟new差不多，但编译有很大开销，根据测试调用3500次后收益才超过
...
* Assembly ass = Assembly.GetExecutingAssembly()/LoadFrom("lib.dll")/t.Assembly; Type t = ass.GetType("NS.C"); Type[] ts = ass.GetTypes()
  * internal的类可用这种方法获取
  * Assembly.GetExecutingAssembly() 与 this.GetType().Assembly 的区别：如果一个方法F分别使用了它们，在另一个程序集里反射调用F，前者指的是调用者的程序集，后者指F的程序集。如果要读取F的资源应使用后者
...
* 结构体
  ...
  * 类：默认是LayoutKind.Auto，只用于COM。TODO：接口呢？
  ...
  * static和const成员不会自动marshal，也不计入sizeof大小
* SAFEARRAY：COM用到
...
* 在非unsafe块下，只能用于几个基元类型编译期求值。在unsafe下才能用于struct，或对于泛型加where T:unmanaged
* 不用unsafe的方式
...
* ReadByte、Read16/32，Write...，Copy，ReadIntPtr 根据 指针(+偏移量) 读写某类型的数据
...
* DllImport
  ...
  * SetLastError=true后可用Marshal.GetLastWin32Error()获取（缓存的）错误值，一般传进new Win32Exception，不会自动抛异常。.NET6会清除错误，另推荐GetLastPInvokeError()，完全一样只是重命名因为反映跨平台
...
* 调用Win32的API
  ...
  * CsWin32源生成：在NativeMethods.txt里指定要使用的函数，Windows.Win32.PInvoke.XXX()调用。会隐式启用AllowUnsafeBlocks
  * 装Microsoft.Windows.SDK.Win32Metadata，引用Windows.Win32.winmd，得到Windows.Win32.XXX命名空间，在各个命名空间的Apis静态类里找函数和结构体
  * https://github.com/PWagner1/Windows-API-CodePack-NET 一些高层封装。似乎原本是微软开发的，后来不更新了
* 调用UWP的API
  * .NET5：设置TargetFramework=net7.0-windows10.0.17763即可（对应1809，最高22621）
  * FX：装Microsoft.Windows.SDK.Contracts。手动引用：下载并引用Windows.winmd后再引用自带的System.Runtime.WindowsRuntime。无法使用$(SystemRoot)\System32\WinMetadata里的，没有Windows.Foundation.FoundationContract.winmd
  * .NET5消费非UWP的winmd（包括Win32）：CsWinRT，必须装Windows SDK。CsWinRTEmbedded能摆脱Microsoft.Windows.SDK.NET.dll的20MB依赖
* 自动生成binding：https://github.com/mono/CppSharp 在CI里下二进制，大概100MB。另一种[ClangSharpPInvokeGenerator](https://github.com/dotnet/clangsharp/#generating-bindings) 大概50+MB
...
* 可以new出来，设置StartInfo属性，然后Start；也可以用静态的Start方法，传ProcessStartInfo实例；基本不用字符串重载因为默认会弹窗
...
* 异步读取输出：RedirectStandardOutput=true，给OutputDataReceived事件添加处理函数，进程启动后启用BeginOutputReadLine()，之后每输出一行就会自动触发事件了
* 一次性读取输出：启用重定向，p.StandardOutput.ReadToEnd()，会自动等待进程结束。一定不能先调用WaitForExit()，因为进程输出时会填充stdout缓冲区，如果调用者直接等待子进程退出，没有消费缓冲区，就死锁了
...
* DateTimeOffset 包含时区Offset的DateTime，推荐优先使用。不是指两个DT的差
  * 改变时区（时刻不变）：dto. ToUniversalTime ToLocalTime ToOffset
* DateTime 用于抽象的日期时间，或不关注现实结果的日期运算
  * 具有Kind属性，方便用户不学习时区，就能处理常见的本地和UTC时间任务
  * new出来的Kind为Unspecified。调用ToLocalTime()会把原有值视为UTC转换为本地时间，调用ToUniversalTime()会把原有值视为本地时间转换为UTC
  * DateTime.Now和UtcNow创建出来的Kind分别是Local和Utc，如对utc_dt.ToUniversalTime()会什么也不做
  * 运算时会忽略Kind：Local-Utc 得到的是8，而DTO得到的是0
* 修改：dt.AddDays、AddDuration(timespan)但其实支持运算符
* dt.ToString("yyyy-MM-dd HH:mm:ss")
  * ISO格式说明符："o" -> "2024-12-15T15:27:02.7028663+08:00"；精度：比毫秒多1位。简单版："s" -> "2024-12-15T15:27:28"
  * RFC1123："r"
  * 符合区域的："F" -> "2024年12月15日 15:32:22"；"f"无秒。"D" -> "2024年12月15日"；"d" -> 2024/12/15。无参或"G" -> "2024/12/15 16:05:33"
  * 其它格式化说明符：https://docs.microsoft.com/zh-cn/dotnet/standard/base-types/standard-date-and-time-format-strings
* Parse
  * DateTime.Parse("datestr")比较宽容，几种格式都接受，也因此不太可靠，比如无时间时默认为0，无日期时默认为当天。有选项允许存在空格System.Globalization.DateTimeStyles
  * DateTime.ParseExact("datestr", "o", null)
  * 二者无时区时Kind都为Unspecified
  * TryParse
* 时间戳：DateTimeOffset.UtcNow.ToUnixTimeSeconds() 返回类型是long，无2038问题。秒数转日期：DateTimeOffset.UnixEpoch.AddSeconds()
* TimeSpan：时间间隔，或反映一个时间而不引用特定的一天。TimeSpan.FromSeconds()、new TimeSpan(时,分,秒)
* TimeZoneInfo：完全代替TimeZone
* .NET6：DateOnly TimeOnly（18点+8->2点，而TimeSpan会超过24）
...
* 在fx时期支持多AppDomain，负责加载程序集、安全边界，是隔离单元
  * 程序集就是exe或dll但内容是IL
* Core不支持那些功能，优先用AppContext。加载程序集用AssemblyLoadContext
  * 只在以下情况（事件）下用AppDomain：AssemblyResolve、UnhandledException、ProcessExit

## 其他语法

* 单例模式和延迟初始化：`static Lazy<T> _singleton = new(()=>new T()); public static T Instance => _singleton.Value`，只有在第一次用到Value时才会初始化，且线程安全。或用内部类的静态字段
* 退出程序
...
* System.Runtime GCSettings.LatencyMode = GCLatencyMode.LowLatency、SustainedLowLatency 低延迟应用避免GC影响时使用，会增大内存占用，后者容忍度相对高一些。前者只用于WorkstationGC
