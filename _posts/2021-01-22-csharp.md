...
* 在非unsafe块下，只能用于几个基元类型编译期求值。在unsafe下可对struct求值，或对于泛型加where T:unmanaged。以下两个函数都不需要启用unsafe
...
* 调用Win32的API
  ...
  * 装Microsoft.Windows.SDK.Win32Metadata，引用Windows.Win32.winmd，得到Windows.Win32.XXX命名空间，在各个命名空间的Apis静态类里找函数和结构体z
* 调用UWP的API
