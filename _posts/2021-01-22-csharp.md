...
* 同步等待/在同步方法中调用异步方法（在UI线程上会死锁）：t.Result、t.Wait()、t.GetAwaiter().GetResult()、t.WaitAll()、t.WaitAny()、Thread.Sleep()；其中GetAwaiter()后对其操作不会抛AggregateException而已
...
* 一种解决方式是直接在UI线程中Task.Run：`Task.Run(()=>xxxAsync()).Result`
...
```c#
CancellationTokenSource cts = new(); // 可指定超时
