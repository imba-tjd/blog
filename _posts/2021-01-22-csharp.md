...
* Equals()
  ...
  * 实现时，一般调用Object.Equals加上&&。如果手动实现且重载了==，小心循环调用自己而爆栈，和null进行比较时最好用is null
...
* Specialized：强类型的集合，其中和字符串有关的一些也许有用
...
* 无法更改函数的指向。原生有个SwapBody但只能用于动态创建的程序集，一些简单的第三方代码都依赖于运行时的实现，也许Release或换个版本就不能用了。但有个第三方库 https://github.com/pardeike/Harmony 也许可以
