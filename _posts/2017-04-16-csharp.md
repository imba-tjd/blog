...
* c/c++的普通函数指针只能指向静态函数，成员函数指针调用时要手动传入对象；而委托可以指向静态和**实例方法**，因为委托同时封装了对象和方法。不仅储存对方法的入口点的引用，同时也储存对调用方法的对象实例的引用
...
* 委托是函数的封装，事件是委托的封装
* 事件不是委托实例，只是成对的+=和-=，类似于属性；默认实现利用的就是多播委托，但我们也可以自己实现访问器
* 事件只能在声明它的类中调用
* ~~事件不能是静态的~~ 可以，我不知道为什么很多文章说不行，也许以前不行
* 函数返回值不能是事件
* 委托可以是局部变量
* 事件在类外不能普通赋值，因为这样会影响其它订阅者
* 事件在类外可以直接+=而不必先=，类内和委托都不行，因为一开始的时候是null
...
* 不能重写static方法
* 必须有相同的参数和返回值
* new：原本类里有一个函数，后来非自己控制的父类出现了同名函数，两个函数其实是无关的。转换为基类时，这样的同名函数可能会有完全不同的作用。一般改名比较好
...
* Flags特性的唯一用处是枚举实例.ToString()时为友好信息而不是数字值，不能检测重复值
...
* StartsWith, EndsWith
* Trim, TrimStart, TrimEnd // 注意并不是删除字符串，而是删到第一个不在参数列表里的字符打止：`"three features in Python".Trim(" Python".ToCharArray())`会得到`ree features i`
...
class JsonFileProductService {
    ...
    public void SaveProducts(IEnumerable<Product> products) {
        ...
        JsonSerializer.Serialize(new Utf8JsonWriter(outstream), products); // 可用Async方法
    }
}
