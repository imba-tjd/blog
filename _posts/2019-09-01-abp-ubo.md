...
* $important：能忽略对应的@@
* $csp=script-src: 'none'：向对应请求添加csp响应，不属于拦截规则；`@@...$csp`白名单所有HTTP头中的csp注入，也可白名单指定的csp内容，会精确匹配不影响其它csp
...
* remove-attr/ra：移除指定的HTML attr。第一个参数是属性名，后一个是css选择器
* remove-class/rc：移除行内class。第一个参数是指定的class名，用竖线分隔，第二个参数是css选择器，第三个参数允许持续监听DOM更改
