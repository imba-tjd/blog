...
* remove-class/rc：移除行内class。第一个参数是指定的class名，用竖线分隔，第二个参数是css选择器，第三个参数允许持续监听DOM更改。基于remove-attr/ra，移除HTML属性。这两者都必须紧跟在##后
...
* `javascript:window.external.msAddTrackingProtectionList('https://easylist-msie.adblockplus.org/easylistchina+easylist.tpl','EasyListChina+EasyList')`
