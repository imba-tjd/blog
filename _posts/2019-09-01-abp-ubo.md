...
## [静态规则](https://help.adblockplus.org/hc/en-us/articles/360062733293-How-to-write-filters)

默认情况下开头和结尾隐式有一个通配符，即`ad`与`*ad*`一样，任意位置含有ad就会匹配。但如果只有域名没有路径（或为基本文件名带后缀），uBO行为不同，会当作“hosts”。

匹配开头和结尾用|，其中开头用|需要写协议，区分http和https，且要把子域写完；而开头的||只匹配域名，且是顶级域名反向往前匹配，无需写子域和协议。ABP的`||example.com`或`example.com`会匹配上`example.com.org`，uBO不会，所以一般加^。
...
* `$<type>`：当url匹配成功时进一步匹配类型，如`$script`匹配js。多个条件并列用逗号分隔。白名单用~，`$~script`匹配除js以外的所有类型
  * $document/$doc：直接访问该网页时匹配。允许其它页面引用它。都屏蔽：$all
* $match-case：区分路径大小写
...
* 适合“安装后就忘记”的用户
* 不会发出任何网络请求，内置规则的更新靠扩展更新
* 不支持scriptlet、dynamic。只能自定义元素过滤规则，可能支持Cosmetic，至少添加没报错
* 自定义路径规则：必须用DNR规则。原规则会在内部自动转换
* 受浏览器影响，合并后的规则数最多3万条
* 同为MV3的ADG好像可以直接用原本的规则

### DNR规则

配置项：https://developer.chrome.com/docs/extensions/reference/api/declarativeNetRequest?hl=zh-cn#build-rules
...
condition:
  requestDomains:
    - example.com
  resourceTypes:
    - main_frame # 代表$document。不指定时会屏蔽除了main_frame以外的。没有all选项
---
下一组规则
---
condition:
  urlFilter: ||example.com^  # 会匹配子域名；如果顶级域名后不加^或斜杠，会匹配.company
---
action:
  type: redirect
  redirect:
    transform:  # 还支持 url即简单重定向、正则替换（condition要用regexFilter）
      queryTransform:  # 能调整URL的各个部分，但大部分情况只能指定写死的
        removeParams:  # 仅它还支持addOrReplaceParams
          - tn
其他功能：增删改 请求、响应 头。然后基本就没了
```
