...
```json
{
  ...
  "ip6tables": true, // 还需"experimental": true
...
* docker run --net="bridge/host/none/container:容器名"
  * 默认为bridge模式：宿主机默认创建docker0网卡，ip例如为172.x.x.1，容器中的网卡ip为172.x.x.2
  * container模式：完全共享网络环境，具有相同的ip，连回环都能直接访问，k8s的Pod就是这样
  * host模式：容器的网络栈不会和主机隔离，并且容器不会分配自己的IP，和主机自己的程序或其它该类型的容器监听同一端口会发生冲突，对于需要处理大量端口时有性能帮助
  * --add-host=host:ip 添加条目到hosts文件中
  * --link过时了。overlay用于swarm的
* 端口映射
  * EXPOSE是容器之间互相访问的端口，且在--icc=true的时候即使不EXPOSE也能互相访问 TODO: 好像docker engine默认就是icc=true的，不知道手动创建bridge时是不是，如果是，EXPOSE感觉就没用了
  * PUBLISH是-p、-P和compose的ports，是容器与宿主之间的访问，且会隐式EXPOSE
  * 可以有EXPOSE了但不PUBLISH的端口
  * -p 宿主IP:宿主端口:容器端口[/udp]，端口可以写范围如8001-8010，宿主端口可省略，则会随机
  * -P是自动映射所有EXPOSE了的端口到宿主机的随机端口
* IP指定*或0.0.0.0或::或不指定效果都是监听所有IP且是双栈；127.0.0.1或::1就只是本地单栈
* 一般情况不需要指定容器IP地址，因为容器间不用IP直接通讯，而是用容器名、服务名、网络别名，不过默认的bridge只能用IP(TODO:测试。应该是容器之间用容器名访问，容器外访问容器里用PUBLISH的如localhost，容器里访问同一台宿主机的本地网络用host.docker.internal)，自定义的可以用容器名
