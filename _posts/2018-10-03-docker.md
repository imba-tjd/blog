...
* image转文件：`docker save alpine | xz > alpine.xz`
* 文件转image：`docker load -i alpine.xz` 或 < alpine.xz
* 一条命令完成映像迁移：`docker save image_name | xz | ssh dest 'cat | docker load'`
...
* docker build --rm -t *image_name* *path*（一般是.）
  * 点是上下文路径，对应COPY的第一个参数的点，可不与Dockerfile所在路径相同
