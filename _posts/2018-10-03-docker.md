...
* Docker容器只是一个进程，用映像提供的rootfs提供了调用所需的userland库支持，使得进程可以在受控环境下运行。容器中的进程会直接向宿主内核发送syscall，而不会直接或间接的使用任何宿主系统的userland的库
* namespace隔离机制：包括进程、网络、挂载点（文件）等
  * 默认未启用 用户隔离。容器内的root就是容器外的。启用后，容器内不变，但对于挂载宿主机目录后，宿主机看到的文件所有者是普通用户UID
* cgroups：限制CPU 内存 磁盘IO 网络带宽
* 默认启用了BIND CAP，非root用户也能绑定1024以下端口

### 虚拟化级别

1. 硬件虚拟化
  * 第一类Hypervisor：裸金属型，直接运行在物理硬件上，如VMware ESXi, Microsoft Hyper-V
  * 第二类Hypervisor：寄居型，运行在宿主操作系统上
  * 实现基础：Intel VT-x
  * 不支持不同指令集。要用模拟(Emulation)技术
  * hypervisor的功能：管理运行在VM中的代码，处理中断，截获并正确路由外部通信。管理VM的创建和销毁，监控资源利用情况
2. 操作系统虚拟化：Docker
3. 应用虚拟化：沙盒
...
https://docker.nju.edu.cn 似乎限制校内了
...
docker.m.ixdev.cn image.cloudlayer.icu
自建：https://github.com/sky22333/hubproxy
...
* 查看容器被运行时的命令行：pip3 install runlike
...
* docker volume create my-vol [-o type=tmpfs -o device=tmpfs -o o=size=2m] [-o type=nfs -o addr=192.168.1.1,rw -o device=:/path/to/dir]
...
# 如果不指定，用的就是root。最好手动指定UID 999。因为默认以1000开始，且默认未启用用户Namespace，导致当挂载目录时宿主机看到的文件所有者是宿主机上相同UID的用户创建的，引起混乱和不安全
...
* 在容器中无法运行systemctl，可用`systemctl.py`替代部分功能：`curl https://raw.githubusercontent.com/gdraheim/docker-systemctl-replacement/master/files/docker/systemctl3.py > /usr/bin/systemctl && chmod +x /usr/bin/systemctl`；或pip install docker-systemctl-replacement再用systemctl.py。journalctl无日志
...
* gosu：https://zhuanlan.zhihu.com/p/151915585 原生可用setuidgid代替。su-exec是C的重新实现，只有10KB
...
* https://github.com/bcicen/ctop 不维护了。https://github.com/jesseduffield/lazydocker
