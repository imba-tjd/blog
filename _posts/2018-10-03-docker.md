...
* 临时使用：`DOCKER_BUILDKIT=1 docker build .` 或 docker buildx build
* 永久使用：在daemon.json里设置"features":{ "buildkit": true } 或 docker buildx install 或用docker23
* 支持并行的多阶段构建、更好的缓存管理、更已读的日志；支持secret mount，无需root权限
* 可以配合containerd使用，release里有二进制
...
* 命令基本上和docker一样。没有restart功能，rmi后会被k8s自动拉起。没有构建和push功能，但能pull
* containerd的命令行为ctr
