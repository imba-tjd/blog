...
* containerd包含runc（曾用名libcontainer），实现了cri标准，用于被kubectl调用容器引擎。实现了cri的还有crun（c语言轻量，兼容runc）、cri-o、youki(rust)
...
* crictl
  ...
  * 命令基本和docker一样。没有restart功能，rmi后会被k8s自动拉起。没有构建和push功能，但能pull
    * ps不会显示暴露的端口
* containerd的命令行叫ctr
  * 默认不会显示k8s创建的，需加--namespace=k8s.io
...
* nerdctl：Docker-compatible CLI for containerd，也是containerd官方出的
  ...
  * 手动配置网络插件：下载https://github.com/containernetworking/plugins/releases 放到/opt/cni/bin里
