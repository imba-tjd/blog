...
* docker run --net="bridge/host/none/container:容器名"
  ...
  * --add-host=host:ip添加条目到/etc/hosts文件中
  * --link过时了
* 端口映射
  * EXPOSE是容器之间互相访问的端口，且在--icc=true的时候即使不EXPOSE也能互相访问
...
* 一般情况不需要指定容器IP地址，因为容器间不用IP直接通讯，而是用容器名、服务名、网络别名，不过默认的bridge只能用IP(TODO:测试)，自定义的可以用容器名
