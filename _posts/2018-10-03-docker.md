...
* docker run -it image_name *程序* *参数*：创建并运行新容器，基本相当于docker create加上docker start；如果image不存在会自动下载；i表示打开并保持stdout，t表示分配一个tty终端；--name指定容器名
...
* -p 宿主IP:宿主端口:容器端口[/udp]，端口可以用范围8001-8010，IP和宿主机端口都可省略，前者默认为*，后者用随机的；-P 则是自动映射，将所有定义 EXPOSE 的端口，随机映射到宿主的某个端口
