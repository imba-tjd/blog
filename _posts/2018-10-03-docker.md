...
* docker run --net="bridge/host/none/container:容器名"指定容器的网络连接类型，其中容器名直接共享lo；--add-host=host:ip添加条目到/etc/hosts文件中；--link过时了
...
# ADD 也是复制但功能更多，1.允许src是url，2.如果src是可识别的压缩文件如files.tar.xz且第二个参数以斜杠结尾是目录，会解压后复制进去。所以在不主动需要这俩功能时最好用copy。不过好像不允许自动解压来自于url的压缩文件
...
### [安装](https://rancher.com/docs/k3s/latest/en/installation/install-options/server-config/)
...
* 有的命令加不加s都一样
* `alias k="kubectl"`；`touch ~/.kube/config; chmod 600 ~/.kube/config`
* server默认会创建`/etc/rancher/k3s/k3s.yaml`，使用kubectl前需要`export KUBECONFIG=$(pwd)/kubeconfig`或指定`--kubeconfig`或放到`~/.kube/config`中且把里面的localhost改为server的url

```
-n <ns>
k get xxx -o wide可以得到更多的信息
k create/get node/deployment/ns/issuer/pod/service
k create/apply/delete -f xx.yml
k config current-context # default
k get componentstatus
k version
k cluster-info
k logs [-f] podname
k create -f https://kubernetes.io/docs/user-guide/nginx-deployment.yaml
k proxy
```

### TODO

* k3d：https://felixwiedmann.de/k3d-manage-k3s-clusters/
