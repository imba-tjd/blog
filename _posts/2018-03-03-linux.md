...
* Ctrl+Alt+F2切换到命令行界面，Ctrl+Alt+F7返回图形界面，Ctrl+Alt+T打开终端
...
* adduser可以交互式添加用户，更友好；useradd -m(创建home文件夹) -g 组 用户名，userdel -r删除用户
...
* sudo -i：加载目标用户的.bashrc，并跳转到目标用户的home，-s和sudo su不做后一项
