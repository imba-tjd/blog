...
* mkfs.ext4 /dev/sda1：格式化分区，实际调用的是e2fsprogs
  ...
  * ext4：稳定。存在inode数量限制（仅此一家）。不能在线扩容
  * f2fs：三星，适合ssd
...
* mount命令
  ...
  * 查看此命令的报错：dmesg
* discard选项：适用于ssd，删除文件时同步trim，可能影响性能。如果不开，要定期手动运行fstrim -a；也可能已存在fstrim.service自动定期运行
...
* SO_REUSEADDR
  ...
  * 功能二：使得TIME_WAIT的Socket能立即使用，无需等到超时过期。理论上对于任何服务端都应该用，是安全的
  * Win即使不设置也能立即使用TIME_WAIT的Socket，设置了会类似于也设置了REUSEPORT
* SO_REUSEPORT：用于解决惊群问题，由内核将每个连接均匀分配到绑定了同一个Socket的各个进程上。只有Linux有，本意是允许完全相同的ip,port成功绑定，每一个Socket都要设置此选项
...
sysctl vm.overcommit_memory=默认0表示基本不允许，1表示允许，2表示要配合overcommit_ratio较复杂不考虑
```
...
net.ipv4.tcp_max_syn_backlog=128 #【默】处于SYN_RECV的最大连接数。还需一同增大somaxconn和socket listen时的backlog参数
...
cp：不会修改文件的inode，且非原子。如果一个程序读了一半某文件，它被修改了，程序可以继续读，且会出问题。而mv只对应一个rename系统调用，且会修改inode，不影响程序读原文件。
