...
* SO_REUSEADDR使得通配地址和具体地址能同时绑定，如可以两个Socket同时分别绑定0.0.0.0:80和127.0.0.1:80，且已存在的不必设定此选项，只要后来的绑定时指定了就行。附加效果是使得TIME_WAIT的Socket能立即使用，无需等到超时过期，理论上对于任何服务端都应该用，是安全的。Win即使不设置也能立即使用TIME_WAIT的Socket，设置了会类似于也设置了REUSEPORT
