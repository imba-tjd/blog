...
* SO_REUSEADDR用于使得TIME_WAIT的套接字能立即使用，无需等到超时过期。SO_REUSEPORT用于解决惊群问题，由内核将每个连接均匀分配到绑定了同一个套接字的各个进程上
...
https://www.rajualluri.net/2015/01/29/authbind-vs-iptables-aws/
